...
import {Form, useNotify, TextInput, UpdateParams, SaveButton} from "react-admin";
        ...
        if (params && user.id) {
            try {
                ...
                const formParams: UpdateParams =
                    {id: user.id, data: formData, previousData: user}
                const response = await dataProvider.update('public_accounts', formParams)
        ...
        <Card style={cardStyle}>
            <Form onSubmit={submitForm}>
                <Typography variant={"h5"}>
                    Détails utilisateur {!editable ? <><Button type={"button"}
                                                               onClick={toggleEditableForm}>Editer</Button></> :
                ...
                <Grid container spacing={1} sx={{mt: 4}}>
                    <Stack spacing={3} direction={"row"} style={{width: "100%"}}>
                        <Grid item xs={4}>
                            <p>Nom</p>
                            <TextInput id="user-lastname" type={"text"} label="" variant="standard"
                        ...
                        <Grid item xs={4}>
                            <p>Prénom</p>
                            <TextInput id="user-firstname" type={"text"} label="" variant="standard"
                        ...
                        <Grid item xs={4}>
                            ...
                            {user.email && <>
                                ...
                                <TextInput source={"email"} id="user-email" label="" variant="standard"
                                           defaultValue={user.email}
                                           disabled={!editable} style={{width: "100%"}} type={"email"}/>
                    ...
                    <Stack spacing={3} direction={"row"} style={{width: "100%"}}>
                        ...
                        <Grid item xs={4}>
                            ...
                            <TextInput source={"phoneNumber"} id="user-phone-number" label="" variant="standard"
                                       defaultValue={user.phoneNumber ? user.phoneNumber : ""}
                                       disabled={!editable} style={{width: "100%"}}/>

                            <Stack width={"60%"} sx={{mt: 2}} spacing={2} textAlign={"left"}>
                                <Tooltip
                        ...
                        <Grid item xs={4}>
                            ...
                            <TextInput id={"user-date-birth"}
                                       source={"dateOfBirth"}
                                       label={""}
                                       defaultValue={user.dateOfBirth ? Moment(user.dateOfBirth).format('YYYY-MM-D') : ""}
                                       disabled={!editable} style={{width: "100%"}} variant={"standard"} type={"date"}/>
                        </Grid>
                        <Grid item xs={4}>
                    ...
                    <Stack spacing={3} direction={"row"} style={{width: "100%"}}>
                        ...
                        <Grid item xs={4}>
                            ...
                            <TextInput id={"user-id-number"} type={"text"} label={""} variant={"standard"}
                                       defaultValue={firstIdCheckHistory && firstIdCheckHistory.technicalDetails && firstIdCheckHistory.technicalDetails.identificationId ? firstIdCheckHistory.technicalDetails.identificationId : ""}
                                       source={"idPieceNumber"} disabled={!editable}/>
                        </Grid>
                        <Grid item xs={3}>
                            <p>Adresse</p>
                            <TextInput id="user-address" type={"text"} label="" variant="standard"
                        ...
                        <Grid item xs={2}>
                            <p>CP</p>
                            <TextInput id="user-postal-code" type={"text"} label="" variant="standard"
                        ...
                        <Grid item xs={3}>
                            <p>Ville</p>
                            <TextInput id="user-city" type={"text"} label="" variant="standard"
