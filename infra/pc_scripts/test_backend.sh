...
function test_backend {
    ...
    RUN='docker stop pc-postgres-pytest;
        ...
        docker exec pc-flask bash -c "cd /opt/services/flaskapp/src/ && rm -rf static/object_store_data/thumbs/* && RUN_ENV=tests pytest tests/routes/native/v1/account_test.py"
        '
}
