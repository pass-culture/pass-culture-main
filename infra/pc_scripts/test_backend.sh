...
function test_backend {
    ...
    RUN='docker stop pc-postgres-pytest;
        ...
        docker exec pc-flask bash -c "cd /opt/services/flaskapp/src/ && rm -rf static/object_store_data/thumbs/* && PYTHONPATH=. pytest --durations=5 --color=yes -rsx -v $PYTEST_ARGS"
        '
}
