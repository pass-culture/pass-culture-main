 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const St="FirebaseError";class T extends Error{constructor(t,n,r){super(n),this.code=t,this.customData=r,this.name=St,Object.setPrototypeOf(this,T.prototype),Error.captureStackTrace&&Error.captureStackTrace(this,v.prototype.create)}}class v{constructor(t,n,r){this.service=t,this.serviceName=n,this.errors=r}create(t,...n){const r=n[0]||{},a=`${this.service}/${t}`,s=this.errors[t],i=s?yt(s,r):"Error",c=`${this.serviceName}: ${i} (${a}).`;return new T(a,c,r)}}function yt(e,t){return e.replace(Tt,(n,r)=>{const a=t[r];return a!=null?String(a):`<${r}?>`})}const Tt=/\{\$([^}]+)}/g;/**
 * @license
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const yn="firebase-heartbeat-database",Tn=1,F="firebase-heartbeat-store";let j=null;function xe(){return j||(j=Ke(yn,Tn,{upgrade:(e,t)=>{switch(t){case 0:try{e.createObjectStore(F)}catch(n){console.warn(n)}}}}).catch(e=>{throw re.create("idb-open",{originalErrorMessage:e.message})})),j}async function An(e){try{const n=(await xe()).transaction(F),r=await n.objectStore(F).get(Ue(e));return await n.done,r}catch(t){if(t instanceof T)O.warn(t.message);else{const n=re.create("idb-get",{originalErrorMessage:t==null?void 0:t.message});O.warn(n.message)}}}async function me(e,t){try{const r=(await xe()).transaction(F,"readwrite");await r.objectStore(F).put(t,Ue(e)),await r.done}catch(n){if(n instanceof T)O.warn(n.message);else{const r=re.create("idb-set",{originalErrorMessage:n==null?void 0:n.message});O.warn(r.message)}}}function Ue(e){return`${e.name}!${e.options.appId}`}/**
 * @license
 * Copyright 2021 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
