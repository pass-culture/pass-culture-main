...
from repository.user_queries import filter_users_with_at_least_one_offer, \
                                    filter_users_with_at_least_one_validated_offerer_validated_user_offerer
from sandboxes.scripts.utils.helpers import get_offer_helper, \
                                            get_offerer_helper, \
                                            get_user_helper, \
                                            get_venue_helper

def get_existing_pro_validated_user_with_at_least_one_offer():
    query = User.query.filter(User.UserOfferers.any())
    query = query.filter(User.validationToken == None)
    query = filter_users_with_at_least_one_offer(query)
    user = query.first()
