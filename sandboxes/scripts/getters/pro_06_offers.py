...
from sandboxes.scripts.utils.helpers import get_offer_helper, get_pro_helper
...
def get_existing_pro_validated_user_with_at_least_one_visible_activated_offer():
    ...
    query = query.join(VenueSQLEntity).join(OfferSQLEntity).filter(OfferSQLEntity.isActive == True)
    user = query.first()
