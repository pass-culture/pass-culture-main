                ...
                FROM (
                       SELECT (
                                SELECT coalesce(sum(criterion."scoreDelta"), 0) AS coalesce_1
                                 ...
                                 WHERE criterion.id = offer_criterion."criterionId" AND
                                       offer_criterion."offerId" = offer.id) AS anon_2,
                            offer."idAtProviders" AS "offer_idAtProviders",
                            offer."dateModifiedAtLastProvider" AS "offer_dateModifiedAtLastProvider",
                            offer."extraData" AS "offer_extraData",
                            offer."isActive" AS "offer_isActive", offer.id AS offer_id, offer."productId" AS "offer_productId",
                            offer."venueId" AS "offer_venueId", offer."bookingEmail" AS "offer_bookingEmail",
                            offer.type AS offer_type,
                            offer.name AS offer_name, offer.description AS offer_description,
                            offer.conditions AS offer_conditions,
                            ...
                            offer."lastProviderId" AS "offer_lastProviderId",
                  (EXISTS(SELECT 1
                          FROM mediation
                          WHERE mediation."offerId" = offer.id AND
                                mediation."isActive")) AS anon_3,
                      row_number()
                      OVER (
                        ...
                        ORDER BY
                          ...
                          (SELECT coalesce(sum(
                                               criterion."scoreDelta"), 0) AS coalesce_1
                           FROM criterion, offer_criterion
                           WHERE criterion.id = offer_criterion."criterionId" AND
                                 offer_criterion."offerId" = offer.id) DESC, random() ) AS anon_4
FROM offer
...
WHERE offer."isActive" = TRUE AND venue."validationToken" IS NULL AND (product."thumbCount" > 0 OR ( EXISTS ( SELECT 1
FROM mediation
WHERE mediation."offerId" = offer.id AND mediation."isActive"))) AND (venue."departementCode" IN ('75', '77', '78', '91', '92', '93', '94', '95') OR offer."isNational" = TRUE ) AND ( EXISTS ( SELECT 1
FROM stock
WHERE stock."offerId" = offer.id AND stock."isSoftDeleted" = FALSE AND (stock."beginningDatetime" > NOW() OR stock."beginningDatetime" IS NULL ) AND (stock."bookingLimitDatetime" > NOW() OR stock."bookingLimitDatetime" IS NULL ) AND (stock.available IS NULL OR stock.available > ( SELECT COALESCE (sum(booking.quantity), 0) AS coalesce_3
FROM booking
WHERE booking."stockId" = stock.id AND booking."isCancelled" = FALSE )))) AND offerer."isActive" = TRUE AND offerer."validationToken" IS NULL AND offer.type != 'EventType.ACTIVATION' AND offer.type != 'ThingType.ACTIVATION' ORDER BY offer.id) ORDER BY ( EXISTS ( SELECT 1
FROM mediation
...
FROM stock
WHERE stock."offerId" = offer.id AND (stock."beginningDatetime" IS NULL OR stock."beginningDatetime" > NOW() AND stock."beginningDatetime" < NOW() ))) DESC, ( SELECT COALESCE (sum(criterion."scoreDelta"), 0) AS coalesce_1
FROM criterion, offer_criterion
WHERE criterion.id = offer_criterion."criterionId" AND offer_criterion."offerId" = offer.id) DESC, random())
LIMIT 30) AS anon_1 LEFT OUTER JOIN product AS product_1 ON product_1.id = anon_1."offer_productId" LEFT OUTER JOIN mediation AS mediation_1 ON anon_1.offer_id = mediation_1."offerId" ORDER BY anon_1.anon_3 DESC, anon_1.anon_4;
