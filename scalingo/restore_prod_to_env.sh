...
DATABASE_URL_OLD="$(scalingo -a $APP_NAME env |grep SCALINGO_POSTGRESQL_URL= | sed -e s,SCALINGO_POSTGRESQL_URL=postgres://,,g)"

# EXTRACT THE USER FROM URL
PG_USER_OLD="$(echo $DATABASE_URL_OLD | grep @ | cut -d@ -f1 | cut -d':' -f1)"

# DELETE APPLICATION ADDON
echo $PG_USER_OLD | scalingo -a $APP_NAME addons-remove $PG_USER_OLD

# RE-ATTACHING APPLICATION ADDON
scalingo -a $APP_NAME addons-add postgresql free
sleep 20

# GET NEW SCALINGO DATABASE URL
POSTGRESQL_URL="$(scalingo -a $APP_NAME env |grep SCALINGO_POSTGRESQL_URL= | sed -e s,SCALINGO_POSTGRESQL_URL=postgres://,,g)"
