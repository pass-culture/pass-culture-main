...
from pcapi.scripts.add_isbn_in_product_extra_data import _extract_isbn_from_offer_id_at_providers
...
class AddIsbnInProductExtraDataTest:
    ...
    def test_does_not_update_extra_data_when_isbn_is_already_present(self, app):
        ...
        product = create_product_with_thing_type(
            id_at_providers="12345678", last_provider_id=provider.id, extra_data=extra_data
        )
    ...
    def test_update_extra_data_when_isbn_is_missing_and_product_was_updated_by_a_provider(self, app):
        ...
        product1 = create_product_with_thing_type(
            id_at_providers="8765432176124", last_provider_id=provider.id, extra_data=extra_data1
        )
        product2 = create_product_with_thing_type(
            id_at_providers="1234567809865", last_provider_id=provider.id, extra_data=extra_data2
        )
        repository.save(product1, product2)
        expected_extra_data = {"author": "author name", "isbn": "1234567809865"}
    ...
    def test_update_extra_data_on_product_and_offer_when_isbn_is_missing(self, app):
        ...
        offer1 = create_offer_with_thing_product(
            venue, product=product1, id_at_providers="8765432176124@12345678912345", last_provider_id=provider.id
        )
        offer2 = create_offer_with_thing_product(
            venue, product=product2, id_at_providers="1234567809865@12345678912345", last_provider_id=provider.id
        )
        ...
        # Then
        assert product1.extraData == {"author": "author name", "isbn": "8765432176124"}
        assert product2.extraData == {"author": "author name", "isbn": "1234567809865"}
        assert offer1.extraData == {"author": "author name", "isbn": "8765432176124"}
        assert offer2.extraData == {"author": "author name", "isbn": "1234567809865"}
