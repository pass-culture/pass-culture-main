...
class CorrectBookingUsedTest:
    @freeze_time('2019-10-01')
    @clean_database
    def test_when_booking_is_associated_to_sold_out_stock_should_update_stocks_and_bookings(self, app):
        # Given
        user = create_user()
    ...
    @freeze_time('2019-10-01')
    @clean_database
    def test_when_booking_is_associated_to_enough_stock_should_not_update_anything(self, app):
        # Given
        user = create_user()
        deposit = create_deposit(user)
