...
class UpdateConfirmationDateFromExistingBookingsTest:
    ...
    def test_data_fixture(self):
        ...
        TestDataFixture = namedtuple(
            "TestDataFixture", ["last_year", "today_at_2", "tomorrow_at_13", "user", "event_stock"]
        )
    ...
    def test_select_existing_event_bookings_without_confirmation_date(self, test_data):
        event_booking_with_confirmation_date = BookingFactory(
            dateCreated=test_data.today_at_2, stock__beginningDatetime=test_data.tomorrow_at_13
        )
        event_booking_without_confirmation_date = BookingWithoutConfirmationDateFactory(
            dateCreated=test_data.today_at_2, stock__beginningDatetime=test_data.tomorrow_at_13
        )
        ...
        assert bookings_without_confirmation_date == [
            event_booking_without_confirmation_date,
            event_booking_without_confirmation_date_from_last_year,
        ]
    ...
    def test_update_target_bookings(self, test_data):
        ...
        assert (
            event_booking_without_confirmation_date_from_last_year.confirmationDate
            == test_data.last_year + datetime.timedelta(hours=48)
        )

    def test_fill_missing_confirmation_dates_of_all_event_bookings_without_confirmation_date(self, test_data):
        BookingWithoutConfirmationDateFactory.create_batch(
            100, dateCreated=test_data.today_at_2, stock=test_data.event_stock, user=test_data.user
        )
    ...
    def test_fill_missing_confirmation_dates_of_event_bookings_without_confirmation_date_not_cancelled_not_used(
        self, test_data
    ):
        BookingWithoutConfirmationDateFactory.create_batch(
            7, dateCreated=test_data.today_at_2, stock=test_data.event_stock, user=test_data.user
        )
        BookingWithoutConfirmationDateFactory.create_batch(
            5, dateCreated=test_data.today_at_2, stock=test_data.event_stock, user=test_data.user, isUsed=True
        )
        BookingWithoutConfirmationDateFactory.create_batch(
            5, dateCreated=test_data.today_at_2, stock=test_data.event_stock, user=test_data.user, isCancelled=True
        )
