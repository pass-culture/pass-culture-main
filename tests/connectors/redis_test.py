...
@patch('connectors.redis.redis')
def test_should_add_venue_id_to_venue_redis_set_when_feature_flipping_is_enabled(mock_redis, mock_redis_list, mock_feature_active):
    # Given
    client = MagicMock()
    client.rpush = MagicMock()
...
@patch('connectors.redis.ALGOLIA_INDEXING_BATCH_SIZE', return_value=500)
@patch('connectors.redis.REDIS_LIST_OFFER_IDS_NAME', return_value='fake_list_offer_ids')
@patch('connectors.redis.redis')
def test_should_return_offer_ids_from_redis(mock_redis, mock_redis_list, mock_algolia_indexing_batch_size):
    # Given
    client = MagicMock()
...
@patch('connectors.redis.REDIS_LIST_OFFER_IDS_NAME', return_value='fake_list_offer_ids')
@patch('connectors.redis.redis')
def test_should_delete_given_range_of_offer_ids_from_redis_list(mock_redis, mock_redis_list, mock_algolia_indexing_batch_size):
    # Given
    client = MagicMock()
...
@patch('connectors.redis.REDIS_LIST_VENUE_IDS_NAME', return_value='fake_list_venue_ids')
@patch('connectors.redis.redis')
def test_should_delete_given_range_of_venue_ids_from_redis_list(mock_redis, mock_redis_list, mock_algolia_indexing_batch_size):
    # Given
    client = MagicMock()
    client.ltrim = MagicMock()
