...
from pcapi.utils.image_conversion import _crop_image_v2
from pcapi.utils.image_conversion import _resize_image_v2
...
class ImageConversionTest:
    def when_image_is_correctly_cropped_v2(self):
        # given
        ...
        # when
        cropped_image = _crop_image_v2(crop_origin_x, crop_origin_y, crop_rect_height, raw_image)
    ...
    @mock.patch("pcapi.utils.image_conversion._crop_image_v2")
    @mock.patch("pcapi.utils.image_conversion._resize_image_v2")
    @mock.patch("pcapi.utils.image_conversion._convert_to_jpeg")
    def when_standardize_image_v2_is_called_with_right_arguments(
        self, mock_convert_to_jpeg, mock_resize_image_v2, mock_crop_image_v2, mock_open_image
    ):
        # given
        crop_origin_x = 0.2
        crop_origin_y = 0.45
        ...
        mock_crop_image_v2.return_value = "B"
        mock_resize_image_v2.return_value = "C"

        # when
        standardize_image_v2(image_bytes, crop_params)

        # then
        mock_crop_image_v2.assert_called_once_with(crop_origin_x, crop_origin_y, crop_rect_height, "A")
        mock_resize_image_v2.assert_called_once_with("B")
    ...
    def when_resize_image_that_have_width_less_than_max_width(self):
        ...
        # when
        resized_image = _resize_image_v2(expected_image)
