...
def test_export_model_returns_bad_request_if_no_token_provided(app):
    ...
    response = TestClient(app.test_client()).get(
        "/exports/models/%s" % ("Venue"), headers={"origin": "http://localhost:3000"}
    )
...
def test_pending_validation_return_200_and_validation_token(app):
    ...
    venue = create_venue(
        offerer, siret=None, comment="comment because no siret", validation_token="venue_validation_token"
    )
...
def test_pending_validation_returns_offerers_venues_user_and_user_offerer_with_requested_data(app):
    ...
    repository.save(
        connexion_user, user_offerer1, user_offerer2, user_offerer3, user_offerer4, venue1, venue2, venue3, venue4
    )
    ...
    user_offerers_validation_token = [
        user_offerer_dict["validationToken"] for user_offerer_dict in response_json[2]["UserOfferers"]
    ]
...
def test_get_venues_return_200_and_filtered_venues(app):
    ...
    venue93_with_offer_before_date_range = create_venue(
        validated_offerer, name="venue93_with_offer_before_date_range", postal_code="93100", siret="12345678912310"
    )
    venue93_with_offer_after_date_range = create_venue(
        validated_offerer, name="venue93_with_offer_after_date_range", postal_code="93100", siret="12345678912311"
    )
    venue93_with_offer_in_date_range = create_venue(
        validated_offerer, name="venue93_with_offer_in_date_range", postal_code="93100", siret="12345678912312"
    )
    venue67_with_offer_before_date_range = create_venue(
        validated_offerer, name="venue67_with_offer_before_date_range", postal_code="67100", siret="12345678912313"
    )
    venue67_with_offer_in_date_range = create_venue(
        validated_offerer, name="venue67_with_offer_in_date_range", postal_code="67100", siret="12345678912314"
    )
    venue67_without_offer_in_date_range = create_venue(
        validated_offerer, name="venue67_without_offer_in_date_range", postal_code="67100", siret="12345678912315"
    )
    venue34_with_offer_in_date_range = create_venue(
        validated_offerer, name="venue34_with_offer_in_date_range", postal_code="34100", siret="12345678912316"
    )
    venue_without_siret_with_offer_in_date_range = create_venue(
        validated_offerer, name="venue_without_siret_with_offer_in_date_range", comment="here is a comment", siret=None
    )
    venue_virtual_with_offer_in_date_range = create_venue(
        validated_offerer, name="venue_virtual_with_offer_in_date_range", siret=None, is_virtual=True
    )
    venue_not_validated_with_offer_in_date_range = create_venue(
        validated_offerer,
        name="venue_not_validated_with_offer_in_date_range",
        validation_token="here is a validation_token",
        siret="12345678912317",
    )
    venue_with_not_validated_offerer_in_date_range = create_venue(
        not_validated_offerer, name="venue_with_not_validated_offerer_in_date_range", siret="12345678912318"
    )
    ...
    repository.save(user, venue_with_not_validated_offerer_in_date_range, venue67_without_offer_in_date_range, *stocks)
    ...
    activity_in_date_range1 = create_venue_activity(
        venue93_with_offer_in_date_range, "insert", issued_at=datetime(2018, 11, 30)
    )
    activity_in_date_range2 = create_venue_activity(
        venue67_with_offer_in_date_range, "insert", issued_at=datetime(2018, 11, 30)
    )
    activity_in_date_range3 = create_venue_activity(
        venue67_without_offer_in_date_range, "insert", issued_at=datetime(2018, 11, 30)
    )
    activity_in_date_range4 = create_venue_activity(
        venue34_with_offer_in_date_range, "insert", issued_at=datetime(2018, 11, 30)
    )
    activity_in_date_range5 = create_venue_activity(
        venue_virtual_with_offer_in_date_range, "insert", issued_at=datetime(2018, 11, 30)
    )
    activity_in_date_range6 = create_venue_activity(
        venue_not_validated_with_offer_in_date_range, "insert", issued_at=datetime(2018, 11, 30)
    )
    activity_in_date_range7 = create_venue_activity(
        venue_with_not_validated_offerer_in_date_range, "insert", issued_at=datetime(2018, 11, 30)
    )
    activity_before_date_range1 = create_venue_activity(
        venue93_with_offer_before_date_range, "insert", issued_at=datetime(2018, 6, 30)
    )
    activity_before_date_range2 = create_venue_activity(
        venue67_with_offer_before_date_range, "insert", issued_at=datetime(2018, 6, 30)
    )
    activity_after_date_range = create_venue_activity(
        venue93_with_offer_after_date_range, "insert", issued_at=datetime(2019, 8, 30)
    )

    save_all_activities(
        activity_in_date_range1,
        activity_in_date_range2,
        activity_in_date_range3,
        activity_in_date_range4,
        activity_in_date_range5,
        activity_in_date_range6,
        activity_in_date_range7,
        activity_before_date_range1,
        activity_before_date_range2,
...
def test_get_venues_with_params_for_pc_reporting_return_200_and_filtered_venues(app):
    ...
    validated_user_offerer_with_validated_user_with_validated_offerer_with_siren = create_user_offerer(
        validated_user, validated_offerer_with_siren1
    )
    validated_user_offerer_with_not_validated_user_with_validated_offerer_with_siren = create_user_offerer(
        not_validated_user, validated_offerer_with_siren3
    )
    validated_user_offerer_with_validated_user_with_not_validated_offerer_with_siren = create_user_offerer(
        validated_user, not_validated_offerer_with_siren
    )
    validated_user_offerer_with_validated_user_with_validated_offerer_without_siren = create_user_offerer(
        validated_user_2, validated_offerer_without_siren
    )
    not_validated_user_offerer_with_validated_user_with_validated_offerer_with_siren = create_user_offerer(
        validated_user, validated_offerer_with_siren2, validation_token="another_token"
    )

    venue_with_validated_offerer_with_siren_with_user_offerer_with_user = create_venue(
    ...
    venue_without_validated_offerer_with_siren_with_user_offerer_with_user = create_venue(
        ...
        name="venue_without_validated_offerer_with_siren_with_user_offerer_with_user",
    ...
    venue_with_validated_offerer_without_siren_with_user_offerer_with_user = create_venue(
        ...
        name="venue_with_validated_offerer_without_siren_with_user_offerer_with_user",
        siret="12345678912343",
    )
    venue_with_validated_offerer_with_siren_without_user_offerer_with_user = create_venue(
        validated_offerer_with_siren2,
        name="venue_with_validated_offerer_with_siren_without_user_offerer_with_user",
        siret="12345678912344",
    )
    venue_with_validated_offerer_with_siren_with_user_offerer_without_user = create_venue(
...
def test_get_venues_with_sirens_params_return_200_and_filtered_venues(app):
    # given
    data = {"sirens": ["123456781", "123456782", "123456783"]}
    ...
    repository.save(query_user, venue_123456789, venue_123456781, venue_123456782, venue_123456783, venue_123456784)
...
def test_get_offerers_return_200_and_filtered_offerers(app):
    ...
    offerer_93100_in_date_range_with_validated_venue_with_siret_with_offer = create_offerer(
        ...
        postal_code="93100",
        siren="123456781",
        date_created=datetime(2018, 11, 2),
    )
    ...
    offerer_93100_before_date_range_with_validated_venue_with_siret_with_offer = create_offerer(
        ...
        postal_code="93100",
        siren="123456786",
        date_created=datetime(2018, 1, 2),
    )
    offerer_93100_after_date_range_with_validated_venue_with_siret_with_offer = create_offerer(
        ...
        postal_code="93100",
        siren="123456787",
        date_created=datetime(2019, 12, 2),
    )
    ...
    stock_offer_10 = create_stock_with_event_offer(
        offerer_93100_in_date_range_with_virtual_venue_with_offer, virtual_venue_with_offer
    )
    ...
    stock_expired_offer_thing = create_stock_with_thing_offer(
        ...
        validated_venue_with_siret_with_expired_offer_thing,
        quantity=0,
    )
    ...
    repository.save(
        ...
        stock_offer_9,
        stock_offer_10,
        stock_active_offer_thing,
        stock_expired_offer_thing,
        validated_venue_with_siret_without_offer,
    )
...
def test_get_offerers_with_params_for_pc_reporting_return_200_and_filtered_offerers(app):
    ...
    user_offerer_not_validated = create_user_offerer(
        user_validated, offerer_not_validated_user_offerer, validation_token="blabla"
    )
    ...
    repository.save(
        user_querying,
        user_offerer_no_siren,
        user_offerer_not_validated_offerer,
        user_offerer_not_validated,
        user_offerer_not_validated_user,
        user_offerer_bank_information,
        user_offerer_not_active,
        user_offerer_ok,
    )

    bank_information = create_bank_information(
        bic="AGRIFRPP", iban="DE89370400440532013000", offerer=offerer_bank_information
    )
...
def test_get_offerers_with_sirens_params_return_200_and_filtered_offerers(app):
    ...
    repository.save(
        query_user, offerer_123456789, offerer_123456781, offerer_123456782, offerer_123456783, offerer_123456784
    )
