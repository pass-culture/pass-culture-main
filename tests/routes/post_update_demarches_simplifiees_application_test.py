...
class Post:
    ...
    class Returns400:
        ...
        @patch('validation.routes.bank_informations.DEMARCHES_SIMPLIFIEES_WEBHOOK_TOKEN', "good_token")
        @clean_database
        def when_refferer_is_not_offerer_or_venue(self, mock_add_to_redis_queue, app):
            # Given
            data = {'dossier_id': '666'}

            # When
            response = TestClient(app.test_client()).post(
                '/bank_informations/refferer_name/application_update?token=good_token', form=data)
