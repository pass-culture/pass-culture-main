...
class Put:
    ...
    class Returns404:
        @patch("pcapi.routes.webapp.recommendations.feature_queries.is_active", return_value=False)
        @pytest.mark.usefixtures("db_session")
    ...
    class Returns200:
        class WhenGeolocationFeatureIsDeactivated:
            @patch("pcapi.routes.webapp.recommendations.feature_queries.is_active", return_value=False)
            @patch("pcapi.routes.webapp.recommendations.create_recommendations_for_discovery")
            @patch("pcapi.routes.webapp.recommendations.create_recommendations_for_discovery_v3")
            @pytest.mark.usefixtures("db_session")
        ...
        class WhenGeolocationFeatureIsActivated:
            @patch("pcapi.routes.webapp.recommendations.feature_queries.is_active", return_value=True)
            @patch("pcapi.routes.webapp.recommendations.create_recommendations_for_discovery_v3")
            @patch("pcapi.routes.webapp.recommendations.create_recommendations_for_discovery")
            @patch("pcapi.routes.webapp.recommendations.current_user")
            @patch("pcapi.routes.webapp.recommendations.serialize_recommendations")
            @patch("pcapi.routes.webapp.recommendations.update_read_recommendations")
            @pytest.mark.usefixtures("db_session")
