...
class Post:
    class Returns201:
        ...
        def when_user_and_offerer_exists_in_database(self, app):
            ...
            payload = {"userId": humanize(user.id), "offererId": humanize(offerer.id)}

            # when
            response = TestClient(app.test_client()).with_auth(email=user.email).post("/userOfferers", json=payload)
    ...
    class Returns400:
        ...
        def when_user_id_is_not_present_in_payload(self, app):
            ...
            offerer = create_offerer(siren="123456781")
            repository.save(user, offerer)
            payload = {"offererId": humanize(offerer.id)}
