...
def test_find_all_offerers_with_managing_user_information_and_venue(app):
    ...
    user_offerer3 = create_user_offerer(user_admin1, offerer2, is_admin=True)
    user_offerer4 = create_user_offerer(user_admin2, offerer2, is_admin=True)
    repository.save(venue1, venue2, venue3, user_offerer1, user_offerer2, user_offerer3, user_offerer4)
...
def test_get_all_pending_offerers_return_requested_tokens_in_case_only_venue_not_validated(app):
    ...
    venue_not_validated = create_venue(
        offerer_validated, siret=None, comment="comment because no siret", validation_token="venue_validation_token"
    )
...
def test_get_all_pending_offerers_return_requested_tokens_in_case_only_user_not_validated(app):
    # given
    user_not_validated = create_user(
        can_book_free_offers=False, email="user@user.pro", is_admin=False, validation_token="token_for_user"
    )
    offerer_validated = create_offerer()
...
def test_get_all_pending_offerers_return_requested_tokens_in_case_only_user_offerer_not_validated(app):
    ...
    user_offerer_not_validated = create_user_offerer(
        user_validated, offerer_validated, validation_token="user_off_token"
    )
    venue_validated = create_venue(offerer_validated)
...
def test_find_filtered_offerers_with_sirens_params_return_filtered_offerers(app):
    ...
    repository.save(offerer_123456789, offerer_123456781, offerer_123456782, offerer_123456783, offerer_123456784)
...
def test_find_filtered_offerers_with_dpts_param_return_filtered_offerers(app):
    ...
    repository.save(offerer_93, offerer_67, offerer_34, offerer_00, offerer_01, offerer_78, offerer_973, offerer_2A)
...
def test_find_filtered_offerers_with_date_params_return_filtered_offerers(app):
    ...
    repository.save(
        offerer_in_date_range, offerer_20180701, offerer_20180801, offerer_before_date_range, offerer_after_date_range
    )

    # When
    query_with_date = find_filtered_offerers(from_date="2018-07-01", to_date="2018-08-01")
...
def test_find_filtered_offerers_with_has_bank_information_param_return_filtered_offerers(app):
    ...
    bank_information = create_bank_information(
        bic="AGRIFRPP", iban="DE89370400440532013000", offerer=offerer_with_bank_information
    )
...
def test_find_filtered_offerers_with_True_has_validated_user_param_return_filtered_offerers(app):
    ...
    repository.save(
        user_offerer_validated_user_1,
        user_offerer_validated_user_2,
        user_offerer_not_validated_user_1,
        user_offerer_not_validated_user_2,
    )
...
def test_find_filtered_offerers_with_True_has_validated_venue_param_return_filtered_offerers(app):
    ...
    venue_with_not_validated_venue_1 = create_venue(
        offerer_with_not_validated_venue, siret="12345678912341", validation_token="another_token"
    )
    venue_with_validated_venue_1 = create_venue(offerer_with_validated_venue, siret="12345678912342")
    venue_with_not_validated_venue_2 = create_venue(
        offerer_with_both, siret="12345678912343", validation_token="a_token"
    )
    venue_with_validated_venue_2 = create_venue(offerer_with_both, siret="12345678912344")

    repository.save(
        venue_with_not_validated_venue_1,
        venue_with_validated_venue_1,
        venue_with_not_validated_venue_2,
        venue_with_validated_venue_2,
    )
...
def test_find_filtered_offerers_with_True_has_venue_with_siret_param_return_filtered_offerers(app):
    ...
    venue_with_no_siret_comment_1 = create_venue(
        offerer_with_venue_without_siret_comment, siret=None, comment="I've no siret"
    )
    ...
    repository.save(
        venue_with_no_siret_comment_1,
        venue_with_no_siret_virtual_1,
        venue_with_siret_1,
        venue_with_siret_2,
        venue_with_siret_3,
        venue_with_no_siret_comment_2,
...
def test_find_filtered_offerers_with_True_has_validated_user_offerer_param_return_filtered_offerers(app):
    ...
    user_offerer_not_validated_1 = create_user_offerer(
        user, offerer_with_not_validated_user_offerer, validation_token="a_token"
    )
    user_offerer_not_validated_2 = create_user_offerer(user_2, offerer_with_both, validation_token="another_token")

    repository.save(
        user_offerer_validated_1, user_offerer_validated_2, user_offerer_not_validated_1, user_offerer_not_validated_2
    )
...
def test_find_filtered_offerers_with_offer_status_with_VALID_param_return_filtered_offerers(app):
    ...
    venue_with_soft_deleted_thing_product = create_venue(
        offerer_with_soft_deleted_thing_product, siret="12345678912342"
    )
    venue_with_soft_deleted_event_product = create_venue(
        offerer_with_soft_deleted_event_product, siret="12345678912343"
    )
    venue_with_not_available_event_product = create_venue(
        offerer_with_not_available_event_product, siret="12345678912344"
    )
    ...
    valid_stock = create_stock_with_thing_offer(
        offerer_with_valid_thing_product, venue_with_valid_thing_product, valid_thing
    )
    expired_stock = create_stock_with_thing_offer(
        offerer_with_expired_thing_product, venue_with_expired_thing_product, expired_thing, quantity=0
    )
    soft_deleted_thing_stock = create_stock_with_thing_offer(
        offerer_with_soft_deleted_thing_product,
        venue_with_soft_deleted_thing_product,
        soft_deleted_thing,
        soft_deleted=True,
    )
...
def test_find_filtered_offerers_with_offer_status_with_EXPIRED_param_return_filtered_offerers(app):
    ...
    expired_stock = create_stock_with_thing_offer(
        offerer_with_expired_thing, venue_with_expired_thing, expired_thing, quantity=0
    )
    soft_deleted_thing_stock = create_stock_with_thing_offer(
        offerer_with_soft_deleted_thing, venue_with_soft_deleted_thing, soft_deleted_thing, soft_deleted=True
    )
...
def test_find_filtered_offerers_with_offer_status_with_ALL_param_and_False_has_not_virtual_venues_return_filtered_offerers(
    ...
    virtual_venue_without_offer_5 = create_venue(
        offerer_with_both_venues_offer_on_not_virtual, is_virtual=True, siret=None
    )
...
def test_find_filtered_offerers_with_default_param_return_all_offerers(app):
    ...
    repository.save(
        ...
        offerer_34000,
        offerer_with_siren,
        offerer_without_siren,
        offerer_with_no_venue,
        offerer_with_only_virtual_venue,
        offerer_with_both_venues,
        offerer_not_validated,
