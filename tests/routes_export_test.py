...
def test_export_model_returns_200_when_given_model_is_known(app):
    ...
    # when
    response = auth_request.get(API_URL + '/export/%s?token=%s' % ('Venue', 'abc'))
...
def test_export_model_returns_400_when_given_model_is_not_exportable(app):
    ...
    # when
    response = auth_request.get(API_URL + '/export/%s?token=%s' % ('VersionedMixin', 'abc'))
...
def test_export_model_returns_400_when_given_model_is_unknown(app):
    ...
    # when
    response = auth_request.get(API_URL + '/export/%s?token=%s' % ('RandomStuff', 'abc'))
