from models import Offerer, PcObject, User
...
def test_13_create_venue_should_through_400_if_managingOfferer_does_not_have_SIREN(app):
    managing_offerer_data = {
        'siren': '',
        'name': 'Test offerer',
        'postalCode': '93000',
    ...
    managing_offerer = Offerer(from_dict=managing_offerer_data)
    venue_data = {'name': 'Test Venue',
                  'siret': '41816609600069',
                  'address': 'Test address',
                  'postalCode': '75200',
                  'bookingEmail': 'venue_test@mail',
                  'city': 'Paris',
                  'managingOffererId': humanize(managing_offerer.id),
                  'latitude': 48.82387,
                  'longitude': 2.35284
                 }
    ...
    user_offerer = managing_offerer.give_rights(user,
                                       RightsType.admin)

    PcObject.check_and_save(user, managing_offerer, user_offerer)

    r_create = req_with_auth().post(API_URL + '/venues/',
                                    json=venue_data)
    assert r_create.status_code == 400
    assert 'undefinedSiren' in r_create.json()