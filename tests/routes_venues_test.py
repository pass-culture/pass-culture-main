...
@pytest.mark.standalone
@clean_database
def test_get_venues_return_200_and_venue(app):
    #given
    user = create_user(password='p@55sw0rd', is_admin=True, can_book_free_offers=False)
    offerer = create_offerer()
    venue = create_venue(offerer)
    ...
    #then
    assert response.status_code == 200
    assert response.json()[0]["name"] == venue.name
...
def test_get_venues_return_error_when_date_param_is_wrong(app):
    ...
    #when
    response = auth_request.get(API_URL + '/venues?from_date=' + wrong_date)

    #then
    assert response.status_code == 400
    assert response.json()['date_format'] == ['to_date ou from_date doit être de type aaaa-mm-jj'] 
