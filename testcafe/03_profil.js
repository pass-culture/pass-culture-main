...
fixture('03_01 Modale Profil').beforeEach(async t => {
  await t.useRole(regularUser).navigateTo(`${ROOT_PATH}decouverte/AH7Q/AU`)
})

test("Lorsque je clique sur l'icône profil, la modale s'affiche", async t => {
  await t.click(rectoDivProfileButton.nth(1))
  await t
    .expect(profileModal.visible)
    .ok()
    .expect(profileModal.hasClass('active'))
    .ok()
...
test('Menu | Réglages', async t => {
  await t
    ...
    .expect(reglages.innerText)
    .eql('Réglages')
    .expect(reglages.hasAttribute('disabled'))
    .ok()
})
