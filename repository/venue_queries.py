...
def find_filtered_venues(dpt=None,
    ...
    if offer_status:
        if offer_status == 'ALL':
            ...
            query = query.join(Offer)
            can_still_be_booked_event = EventOccurrence.beginningDatetime > datetime.utcnow()
            ...
            query_1 = query.join(EventOccurrence)
            query_2 = query.join(Stock)

        if offer_status == "VALID":
            query_with_valid_event = query_1.filter(can_still_be_booked_event)
        ...
        if offer_status == "EXPIRED":
            query_with_expired_event = query_1.filter(~can_still_be_booked_event)
