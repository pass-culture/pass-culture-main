...
Feature: Signup journey
  ...
  Scenario: With a new account, create a new offerer with an unknown SIRET
    Given I log in with a new user account
    When I start offerer creation
    And I specify an offerer with a "not used" SIRET
    ...
    And the offerer validation is in progress
  ...
  Scenario: With a new account and a known offerer, create a new offerer in the space
    ...
    And I log in with a new user account
    When I start offerer creation
    And I specify an offerer with a "known" SIRET
    And I chose to add a new offerer
