import { COLLECTIVE_OFFERS_TABLE_HEADER } from '../support/constants.ts'
import {
...
describe('Adage confirmation', () => {
  ...
  beforeEach(() => {
    ...
    cy.sandboxCall(
      ...
      (response) => {
        ...
        cy.intercept({
          method: 'GET',
          url: `/collective/offers?offererId=1&collectiveOfferType=offer`,
        }).as('collectiveOffers')
        cy.intercept({
          method: 'GET',
  ...
  it('I should be able to search with several filters and see expected results', () => {
    ...
    cy.request({
      ...
      let expectedResults = [
        COLLECTIVE_OFFERS_TABLE_HEADER,
        [
          '',
          '',
          offer.name,
