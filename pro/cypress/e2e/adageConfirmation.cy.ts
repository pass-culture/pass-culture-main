  ...
  it('I should be able to search with several filters and see expected results', () => {
    ...
    cy.request({
      ...
      let expectedResults = [
        ...
        ['', '', offer.name, offer.venueName, 'DE LA TOUR', 'préréservée'],
      ]
      ...
      expectedResults = [
        ['', 'Titre', 'Lieu', 'Établissement', 'Statut'],
        ['', '', offer.name, offer.venueName, 'DE LA TOUR', 'réservée'],
      ]
      ...
      cy.wait('@collectiveOffers').its('response.statusCode').should('eq', 200)
      cy.stepLog({ message: 'Offer is now Published again' })
      cy.contains('publiée')
      expectedResults = [
        ['', 'Titre', 'Lieu', 'Établissement', 'Statut'],
        ['', '', offer.name, offer.venueName, 'DE LA TOUR', 'publiée'],
      ]
      expectOffersOrBookingsAreFound(expectedResults)
    })
  })
