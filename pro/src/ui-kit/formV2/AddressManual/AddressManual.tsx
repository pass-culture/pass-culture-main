...
export const AddressManual = ({
  ...
  const formik = useFormContext<AddressFormValues>()

  const [coords, coordsMeta] = formik.getValues('coords')
  ...
  const onCoordsBlur = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    formik.setValue('latitude', latitude)
    formik.setValue('longitude', longitude)
  }
