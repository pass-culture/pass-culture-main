...
export const SettlementTable = ({
  ...
  const isSettlementSelectable = (row: ExtendedSettlementResponseModel) =>
    row.status === SettlementStatus.EXECUTED && row.invoicesCount > 0

  const invoicesToDownload = checkedSettlements.flatMap((s) =>
    s.invoices.map((i) => i.reference)
  )

  const checkedSettlementsDescrition = `${checkedSettlements.length} ${pluralizeFr(checkedSettlements.length, 'virement sélectionné', 'virements sélectionnés')}`
  ...
  return (
    ...
    >
      ...
      <InvoiceDownloadActionsBar
        ...
        description={checkedSettlementsDescrition}
      />
    </Table>
  )
}
