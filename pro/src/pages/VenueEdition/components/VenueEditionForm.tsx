...
export const VenueEditionForm = ({ venue }: VenueFormProps) => {
  ...
  const buildedSelectOptions = buildSelectOptions(getActivities())
  // `activity` may be null if the venue wasn't yet open to public.
  // In that case, we provide a default value so the field isn't rendered "blank"
  if (methods.watch('activity') === null) {
    buildedSelectOptions.unshift({
      value: '',
