...
export function IndividualOfferTimetableScreen({
  ...
  const shouldShowTimetableTypeRadioGroup =
    ...
    openingHoursEmpty &&
    !offer.hasStocks //  TODO : can we see the difference between an offer with one thing stock or one event stock if there is no openingHours yet ?
  ...
  async function onSubmit(values: IndividualOfferTimetableFormValues) {
    ...
    if (
      ...
      !openingHours
      //  TODO : enable this condition when the previous step (Tarif) is completed
      //  || !stocks ||
      //  stocks.length === 0
    ) {
      return
    ...
    try {
      ...
      if (stocks && stocks.length > 0) {
        await api.updateThingStock(stocks[0].id, {
          price: offer.priceCategories?.find(
