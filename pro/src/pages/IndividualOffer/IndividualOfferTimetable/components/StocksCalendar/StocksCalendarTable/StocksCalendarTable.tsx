...
export type StocksCalendarTableProps = {
  ...
  selectedStocksIds: Set<number | string>
...
export function StocksCalendarTable({
  ...
  // ✅ Conditional column: Réservations (not in CREATION)
  if (mode !== OFFER_WIZARD_MODE.CREATION) {
    columns.push({
      id: 'bookingsQuantity',
      label: 'Réservations',
  ...
  // ✅ Conditional column: Actions (not in READ_ONLY)
  ...
  return (
    <div className={styles['container']}>
      ...
      <Table
        ...
        selectedIds={selectedStocksIds}
        onSelectionChange={(stocks) =>
          setSelectedStocksIds(new Set(stocks.map((s) => s.id)))
        }
        data={stocks}
        isLoading={isLoading}
        ...
        noResult={{
          ...
          onFilterReset: onUpdateFilters,
        }}
        noData={{
          hasNoData: hasNoStocks,
          message: {
            icon: '',
            title: 'Vous n’avez pas encore de dates',
            subtitle: '',
          },
        }}
      />
