...
export const validationSchema = yup
    ...
    .shape({
        timetableType: yup
            ...
            .required(),
        startDate: yup.string().when('timetableType', {
            is: 'openingHours',
            then: (schema) => schema.required('La date de début est obligatoire'),
        }),
        noEndDate: yup.boolean().required(),
        endDate: yup.string().when(['timetableType', 'noEndDate'], {
            is: (timetableType: string, noEndDate: boolean) =>
                timetableType === 'openingHours' && !noEndDate,
            then: (schema) =>
                schema
