...
>({
  ...
  // `blocker.state` has 3 states:
  // - "unblocked": Default (idle) state.
  // - "blocked": After each path (navigation) change, `useBlocker` callback is evaluated.
  //   "blocked" means this callback has returned `true`, to prevent the navigation towards this new path.
  // - "proceeding": The navigation has been blocked but is now allowed and proceeding.
    ...
    async (transformedFormValues: TTransformedValues) => {
      ...
      if (!canProceed) {
        if (blocker.state === 'blocked') {
          // Use Case:
          // 1. We tried to click on a link while the form was dirty.
          // 2. It triggered the guard dialog (`blocker.state` => "blocked")
          // 3. We clicked "Enregistrer et quitter".
          // 4. `onSubmit` callback returned `false` to indicate a submission error.
          // 5. We have to reset `blocker.state`.
          //
          // Otherwise the next click on the form submit button would redirect us to the (1) link
          // instead of redirecting us to the `afterSubmitPath`.
          blocker.reset()
        }
