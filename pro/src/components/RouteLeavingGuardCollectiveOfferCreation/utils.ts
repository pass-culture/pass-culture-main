import type { BlockerFunction } from '@/components/RouteLeavingGuard/RouteLeavingGuard'

const STEP_OFFER = 'offer'
const STEP_OFFER_EDITION = 'offer_edition'
const STEP_STOCKS = 'stocks'
const STEP_INSTITUTION = 'institution'
...
const collectiveUrlPatterns: { [key: string]: RegExp } = {
  ...
  [STEP_INSTITUTION]:
    /\/offre(\/([A-Z0-9]+))\/collectif(\/[A-Z,0-9]+)?\/etablissement/g,
  [STEP_RECAP]:
...
export const shouldBlockNavigation: BlockerFunction = ({
  ...
  if (
    ...
    (from === STEP_INSTITUTION && to === STEP_OFFER_EDITION) ||
    (from === STEP_INSTITUTION && to === STEP_STOCKS) ||
    (from === STEP_RECAP && to === STEP_INSTITUTION) ||
