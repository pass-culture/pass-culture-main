...
describe('components | BookingsRecapTable', () => {
  ...
  it('should filter when filters change', async () => {
    const bookingsRecap = [
      collectiveBookingFactory(bookingInstitutionCustom),
      collectiveBookingFactory(),
    ]
    const props: Props = {
      ...defaultProps,
    ...
    await userEvent.selectOptions(
      ...
      screen.getByRole('option', { name: 'Établissement' })
    )
    ...
    await userEvent.type(screen.getByRole('searchbox'), 'COLLEGE BELLEVUE')
    await waitFor(() => {
      // 1 line = 5
