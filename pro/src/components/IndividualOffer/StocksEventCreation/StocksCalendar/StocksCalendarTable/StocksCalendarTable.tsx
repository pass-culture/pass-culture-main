...
export function StocksCalendarTable({
  ...
  function handleStockCheckboxClicked(stock: GetOfferStockResponseModel) {
    const newChecked = new Set(Array.from(checkedStocks))
    if (checkedStocks.has(stock.id)) {
      newChecked.delete(stock.id)
    } else {
              ...
              <tr key={stock.id} className={styles['tr']}>
                <td className={styles['tbody-td']}>
                  <Checkbox
                    ...
                    onChange={() => handleStockCheckboxClicked(stock)}
                    name="select-stock"
                  />
                </td>
                <td className={styles['tbody-td']}>
                  {stock.beginningDatetime
                    ? format(new Date(stock.beginningDatetime), 'HH:MM')
                    : 'Horaire invalide'}
                </td>
                <td className={styles['tbody-td']}>
                  {priceCaregory
