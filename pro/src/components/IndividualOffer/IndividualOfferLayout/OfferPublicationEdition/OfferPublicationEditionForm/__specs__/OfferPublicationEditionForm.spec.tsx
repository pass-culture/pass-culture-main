...
describe('OfferPublicationEditionForm', () => {
  ...
  it('should pre-fill the bookability date', async () => {
    const bookabilityDate = addDays(new Date(), 2).toISOString()

    //  The bookability date can only have a time every 15 minutes : 12:00, 12:15, 12:30...
    const bookabilityDateFormatted = `${bookabilityDate.split('T')[0]}T12:00:00Z`

    renderOfferPublicationEditionForm({
      offer: getIndividualOfferFactory({
        publicationDatetime: bookabilityDateFormatted,
        bookingAllowedDatetime: null,
      }),
      onSubmit: () => {},
    })
