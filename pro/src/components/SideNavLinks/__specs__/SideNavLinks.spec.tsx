...
describe('SideNavLinks', () => {
  ...
  it('should display partner link if user as partner page to display 1st venue partner page as default', async () => {
    ...
    const link = await screen.findByRole('link', { name: 'Page sur l’application' })
    expect(link).toBeInTheDocument()
    const defaultVenueId = mockedManagedVenues.find((venue) => venue.isPermanent)?.id
    expect(link).toHaveAttribute('href', `/structures/${offerer.selectedOffererId}/lieux/${defaultVenueId}/partner-page`)
  })
