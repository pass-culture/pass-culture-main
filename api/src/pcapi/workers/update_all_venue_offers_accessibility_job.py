...
def update_all_venue_offers_accessibility_job(venue_id: int, accessibility: dict[str, bool]) -> None:
    offer_query = db.session.query(Offer).filter(Offer.venueId == venue_id)
    collective_offer_query = db.session.query(CollectiveOffer).filter(CollectiveOffer.venueId == venue_id)
    collective_offer_template_query = db.session.query(CollectiveOfferTemplate).filter(
        CollectiveOfferTemplate.venueId == venue_id
    )
