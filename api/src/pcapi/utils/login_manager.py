...
def manage_pro_session(user: users_models.User | None) -> users_models.User | None:
    ...
    last_page = datetime.fromtimestamp(flask.session.get("last_page", current_timestamp))

    valid_session = compute_pro_session_validity(last_login, last_page)

    # TODO remove 4 month after introduction
    if "last_login" not in flask.session:
        flask.session["last_login"] = current_timestamp
    flask.session["last_page"] = current_timestamp
...
def compute_pro_session_validity(last_login: datetime, last_page: datetime) -> bool:
    now = datetime.utcnow()

    if last_page + PRO_SESSION_BASE_TIMEOUT < now:
        return False

    if last_login + PRO_SESSION_LOGIN_TIMEOUT > now:
        return True

    if last_page + PRO_SESSION_GRACE_TIME < now:
        return False
