...
class OffererViewsModel(pydantic_v2.BaseModel):
    eventDate: datetime.date
    numberOfViews: int


class TopOffersData(pydantic_v2.BaseModel):
    offerId: int
    numberOfViews: int


class OffererTotalViewsLast180Days(pydantic_v2.BaseModel):
    offererId: int
    dailyViews: list[OffererViewsModel]

    @pydantic_v2.field_validator("dailyViews", mode="before")
    @classmethod
...
class OffererTopOffersAndTotalViews(pydantic_v2.BaseModel):
    offererId: int
    topOffers: list[TopOffersData]
    totalViews: int | None

    @pydantic_v2.field_validator("topOffers", mode="before")
    @classmethod
