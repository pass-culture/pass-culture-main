...
def log_offer_event(offer: Offer, booking: Booking, user: User, event_name: str) -> None:
    if "apps_flyer" not in user.externalIds:
        raise AppsFlyerMissingError("user has no apps flyer information")
    ...
    try:
        ...
        logger.error("[APPS FLYER][OFFER BOOKED] request failed", extra=extra_infos)
        return
    ...
    if response.status_code != 200:
        ...
        logger.error("[APPS FLYER][OFFER BOOKED] request returned an error", extra=extra_infos)
