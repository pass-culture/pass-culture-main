...
def get_quotient_familial(
    ...
    if response.status_code // 100 == 4:
        logger.error("Invalid query for Particulier API", extra={"response": response.json()})
        raise ParticulierApiQueryError("Invalid query for Particulier API")
    if response.status_code // 100 == 5:
        logger.error("Particulier API is not responding", extra={"response": response.json()})
        raise ParticulierApiUnavailable("Particulier API is not responding")
    elif response.status_code // 100 != 2:
        logger.error("Unexpected response from Particulier API", extra={"response": response.json()})
