...
def substract_from_deposit_because_of_past_expenses_on_another_account(user_id: int, author_id: int) -> None:
    user = db.session.scalars(select(User).where(User.id == user_id)).one()
    ...
    [
        recredit_to_delete,
    ] = [recredit for recredit in deposit.recredits if recredit.recreditType == RecreditType.PREVIOUS_DEPOSIT]

    action_history = ActionHistory(
        actionType=ActionType.COMMENT,
        authorUserId=author_id,
        user=user,
        comment=f"(PC-39377) Suppression du crédit pré-décret {recredit_to_delete.id} de {recredit_to_delete.amount} euros déjà utilisé dans le compte doublon 4245388",
    )
...
if __name__ == "__main__":
    ...
    with atomic():
        ...
        substract_from_deposit_because_of_past_expenses_on_another_account(args.user_to_subsctract_credit, args.author)
