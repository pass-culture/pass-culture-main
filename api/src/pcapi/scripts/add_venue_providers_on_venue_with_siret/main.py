...
def _move_cinema_pivot_to_destination_venue(
    ...
    if venue_provider.provider.localClass == "AllocineStocks":
        ...
        if cinema_pivot_destination:
            ...
            cinema_pivot_source.venue = venue_destination
            db.session.flush()
            logger.info(
    ...
    for venue_source in venues_without_siret_with_providers:
        ...
        for venue_provider in venue_source.venueProviders:
            try:
                ...
                logger.error("%s is already synchronized with %s", venue_destination, venue_provider.provider)
                log_fails.append(
                    (venue_source.id, f"Cette venue est déjà synchro avec le provider {venue_provider.provider.id}.")
                )
                continue
            except ProviderException as error:
                logger.error(
                    "Could not move provider %s to venue %s. \n Full error: %s",
                ...
                log_fails.append(
                    (
                        venue_source.id,
                        f"Impossible de configurer le Provider {venue_provider.provider.id} avec cette venue",
                    )
                )
                continue
