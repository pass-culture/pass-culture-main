...
from pcapi.core.providers.api import create_venue_provider
...
def main(not_dry: bool) -> None:
    ...
    for venue in venues_without_siret_with_providers:
        ...
        print(f"Venue {venue.id} will be synch. with those providers {[vp.provider for vp in new_providers]}")
        for venue_provider in new_providers:
            try:
                with atomic():
                    create_venue_provider(venue_provider.provider, venue, user)
