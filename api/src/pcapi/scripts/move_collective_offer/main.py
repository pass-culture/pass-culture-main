...
gh workflow run on_dispatch_pcapi_console_job.yaml \
  ...
  -f RESOURCES="512Mi/.5" \
  -f BRANCH_NAME=pc-42987-move-collective-offer \
...
def main(
    source_offerer_id: int,
    source_venue_id: int,
    destination_offerer_id: int,
    ...
    _source_venue = (
        ...
        .filter(
            offerers_models.Venue.id == source_venue_id, offerers_models.Venue.managingOffererId == source_offerer_id
        )
        .one()
    )
    destination_venue = (
    ...
    collective_booking.offererId = destination_offerer_id
...
if __name__ == "__main__":
    ...
    parser.add_argument("--old-offerer-id", type=int, required=True)
    ...
    main(args.old_offerer_id, args.old_venue_id, args.new_offerer_id, args.new_venue_id, args.collective_offer_id)
