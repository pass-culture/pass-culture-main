...
DATA = [95225, 95576, 95193, 89900, 95058, 89710, 95323]


def delete_items(batch_size: int, retries: int) -> None:
    try:
        for partition in [DATA[i : i + batch_size] for i in range(0, len(DATA), batch_size)]:
...
def apply_batch_script(batch_size: int, retries: int, timeout: str, dry_run: bool) -> None:
    ...
    try:
        ...
        delete_items(batch_size, retries)
