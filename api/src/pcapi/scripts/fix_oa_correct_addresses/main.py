...
DESCRIPTION = "Fix incorrect OffererAddresses (with correct Addresses) linked to several Venues"
...
def fix_items(ids_to_fix: list[int]) -> list[offerers_models.Venue]:
    ...
    incorrect_locations: set[offerers_models.OffererAddress] = set(
        db.session.execute(
            sa.select(offerers_models.OffererAddress).where(offerers_models.OffererAddress.id in ids_to_fix)
        )
    )
