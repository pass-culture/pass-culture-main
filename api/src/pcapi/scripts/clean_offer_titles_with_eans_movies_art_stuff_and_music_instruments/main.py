...
def handle_offers_with_ean_inside_title(min_id: int | None = None, max_id: int | None = None) -> None:
    rows_iterator = get_offers_with_ean_inside_title(min_id=min_id, max_id=max_id)
    for idx, chunk in enumerate(get_chunks(rows_iterator, 2_000)):
        print(f"start loop #{idx} with {len(chunk)} ids...")
...
def get_offers_with_ean_inside_title(
    ...
    step = 10_000 if max_id > 100_000 else max((min_id - max_id) // 10, 2)

    for upper_bound in reversed(range(min_id, max_id + step + 1, step)):
        query_params = {"upper_bound": upper_bound, "lower_bound": upper_bound - step}
...
def update_offers_names(offer_rows: Collection[OfferRow]) -> None:
    ...
    for offer in base_query:
        offer.name = offer.name.replace(offer_ean_mapping[offer.id], "").replace("  ", " ").strip()
        offer.ean = offer_ean_mapping[offer.id].strip()
...
if __name__ == "__main__":
    ...
    handle_offers_with_ean_inside_title(min_id_arg, max_id_arg)
