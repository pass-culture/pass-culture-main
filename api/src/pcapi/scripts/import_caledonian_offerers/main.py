...
CSV_COLUMN_MAPPING = {
    "ridet": "RIDET",
    "venue_name": "Nom de la structure",
    "number": "numéro",
...
def parse_csv_data(file: TextIO) -> list[dict]:
    data_rows = []

    reader = csv.DictReader(file)

    for row in reader:
        ridet = row[CSV_COLUMN_MAPPING["ridet"]].strip()
        venue_name = row[CSV_COLUMN_MAPPING["venue_name"]].strip()

        if not (ridet and venue_name):
            continue
        ...
        for field, csv_column in CSV_COLUMN_MAPPING.items():
            ...
            if field in MANDATORY_FIELDS and not value:
                logger.error(f"Données incomplètes pour la ligne {row}. Il manque le champ {field}")
        ...
        extracted_data["address"] = f"{extracted_data['number']} {extracted_data['street']}"
...
def main(not_dry: bool) -> None:
    ...
    file_path = f"{namespace_dir}/caledonian-offerers.csv"

    if not os.path.exists(file_path):
        logger.error(f"Fichier non trouvé: {file_path}")
        return

    logger.info(f"Début de l'import depuis {file_path}")
    logger.info(f"Mode dry run: {not not_dry}")

    with open(file_path, "r", encoding="utf-8") as f:
        data_rows = parse_csv_data(f)
    logger.info(f"Nombre d'entrées trouvées: {len(data_rows)}")
