...
def get_invoices_lines(year: int) -> typing.Iterable:
    ...
    for line in collective_data:
        yield line


def main(year: int) -> None:
    logger.info("Export invoices for year %s", year)
    ...
    filename = f"invoices{year}_{local_now:%Y%m%d}_{local_now:%H%m}.csv"
    ...
    for line in get_invoices_lines(year):
        row = _invoice_row_formatter(line)
        with open(path, "a", encoding="utf-8") as fp:
            writer = csv.writer(fp, quoting=csv.QUOTE_NONNUMERIC)
            writer.writerow(row)

    logger.info("Export finished, compressing to zipfile…")
    ...
    path.unlink()
    logger.info("Writing to zipfile finished")
