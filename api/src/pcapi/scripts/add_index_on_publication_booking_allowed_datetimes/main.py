...
def add_indexes() -> None:
    ...
    db.session.execute(
        text(
            """ CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS "ix_offer_bookingAllowedDatetime" ON offer ("bookingAllowedDatetime") WHERE "bookingAllowedDatetime" IS NOT NULL ; """
        )
    )
    db.session.execute(
        text(
            """ CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS "ix_offer_publicationDatetime" ON offer ("publicationDatetime") WHERE "publicationDatetime" IS NOT NULL ; """
        )
    )
