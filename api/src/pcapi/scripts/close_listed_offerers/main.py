...
def get_offerer_closed_email_data(offerer: offerers_models.Offerer) -> mails_models.TransactionalEmailData:
    ...
    return mails_models.TransactionalEmailData(
        template=mails_models.TemplatePro(id_prod=0000, id_not_prod=000),
        params={
...
def main(offerer_ids: list[int]) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--offerer-ids", type=str, default=0, help="Offerer ids, separated by a comma")
    args = parser.parse_args()

    offerer_ids = [int(offerer_id) for offerer_id in set(args.offerer_ids.split(","))]
    main(offerer_ids)
