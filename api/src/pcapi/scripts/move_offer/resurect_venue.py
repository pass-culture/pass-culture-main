...
def _move_price_categories(
    ...
    for price_category_label in existing_price_category_labels:
        ...
        price_category_ids = (
            (
                ...
                .join(
                    offer_models.PriceCategory.offer,
                    offer_models.PriceCategory.priceCategoryLabel,
                    offer_models.PriceCategoryLabel.venue,
                )
                .filter(
    ...
    price_category_label_ids = (
        db.session.query(offer_models.PriceCategoryLabel)
        .join(
            offer_models.PriceCategoryLabel.priceCategories,
            offer_models.PriceCategory.offer,
        )
        .filter(
            offer_models.PriceCategory.offerId.in_(objects_to_move.offers),
...
def resurrect_venue(dry_run: bool, origin_id: int, destination_id: int, objects_to_move: ObjectsToMove) -> None:
    ...
    origin_venue = db.session.query(offerers_models.Venue).filter(offerers_models.Venue.id == origin_id).one_or_none()
    destination_venue = (
    ...
    individual_finance_incident_ids = (
        finance_models.FinanceIncident.query.join(
