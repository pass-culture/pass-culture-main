...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="mais qui va lire ça ? on se le demande...")
    parser.add_argument("--dry-run", action=argparse.BooleanOptionalAction, default=True)
    args = parser.parse_args()

    try:
        db.session.execute("SET SESSION statement_timeout = '1200s'")  # 20 minutes
        _move_all_venue_offers(dry_run=args.dry_run, origin=None, destination=None)
