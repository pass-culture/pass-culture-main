...
def main(provider_id: int, venue_id: int | None = None) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--venue-id", type=int, required=False)
    parser.add_argument("--provider-id", type=int, required=True)
    args = parser.parse_args()

    with transaction_manager.atomic():
        main(provider_id=args.provider_id, venue_id=args.venue_id)
