...
gh workflow run on_dispatch_pcapi_console_job.yaml \
  ...
  -f RESOURCES="512Mi/.5" \
  -f BRANCH_NAME=PC-40636-extract-all-finance-data-2026 \
  -f NAMESPACE=extract_data_finance \
  -f SCRIPT_ARGUMENTS="";
    ...
    query = (
        db.session.query(
            *select_fields,
            ...
            sa.literal_column("'INCIDENT INDIV'").label(BOOKING_TYPE_LABEL),
    ...
    query = (
        db.session.query(
            *select_fields,
            ...
            educational_models.CollectiveOffer.formats[1].label(OFFER_SUBCATEGORY_ID_LABEL),
    ...
    query = (
        db.session.query(
            *select_fields,
            ...
            sa.literal_column("'INDIV'").label(BOOKING_TYPE_LABEL),
            offers_models.Offer.name.label(OFFER_NAME_LABEL),
            offers_models.Offer.subcategoryId.label(OFFER_SUBCATEGORY_ID_LABEL),
            bookings_models.Booking.userId.label(BENEFICIARY_ID_LABEL),
            sa.func.coalesce(offers_models.Stock.beginningDatetime, finance_models.Pricing.valueDate).label(
        ...
        with open(output_file, "w", encoding="utf-8") as csvfile:
            ...
            for row in get_extract_data(year, month):
                ...
                for field in fields:
                    ...
                    value = row._mapping[field_label]
