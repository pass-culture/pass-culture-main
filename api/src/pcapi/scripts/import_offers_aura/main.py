...
def _process_csv_row(csv_row: dict[str, str], row_number: int, template: models.CollectiveOfferTemplate) -> None:
    ...
    booking_limit = datetime.datetime.strptime(csv_row[BOOKING_LIMIT_FIELD], "%d/%m/%Y")
    ...
    try:
        teacher = (
            db.session.query(models.EducationalRedactor).filter(models.EducationalRedactor.email == teacher_email).one()
        )
    except sa_exc.NoResultFound:
        logger.warning("No teacher found with this email for row %s, setting None", row_number)
        teacher = None
