...
def add_column_and_constraint_for_collective_booking_table() -> None:
    ...
    db.session.execute(
        text(
            """ 
            ALTER TABLE collective_booking ADD COLUMN cancellationAuthorId BIGINT;
            ALTER TABLE collective_booking ADD CONSTRAINT collective_booking_cancellation_author_fk FOREIGN KEY (cancellationAuthorId) REFERENCES "user"(id) NOT VALID;
            """
        )
    )
...
if __name__ == "__main__":
    add_column_and_constraint_for_collective_booking_table()
