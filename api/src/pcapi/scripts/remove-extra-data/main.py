...
def remove_offer_extraData_if_offer_linked_to_product(
    ...
    batch_size: int,
    not_dry: bool = False,
    db_session_timeout: int = 600,
) -> None:
    db.session.execute(sa.text(f"SET SESSION statement_timeout = '{db_session_timeout}s'"))
    max_id = _get_max_id()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--db-session-timeout", type=int, default=600, help="set database session timeout")
