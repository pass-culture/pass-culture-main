            ...
            try:
                ...
                if bank_account:
                    ...
                    if bank_account.iban != application_details.iban:
                        logger.error("[DS] Mismatched IBAN for application %s", application_details.application_id)
                        ok = False
                    if bank_account.bic != application_details.bic:
                        logger.error("[DS] Mismatched BIC for application %s", application_details.application_id)
                        ok = False
                    if bank_account.label != application_details.label:
                        logger.error("[DS] Mismatched label for application %s", application_details.application_id)
                        ok = False
                    if not ok:
                        writer.writerow(
                            [
                    ...
                    logger.error("Application %s not found in database", application_details.application_id)
                    writer.writerow(
                        [
                            application_details.application_id,
                            application_details.status.value,
