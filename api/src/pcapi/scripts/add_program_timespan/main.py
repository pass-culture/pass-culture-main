...
def get_meg_program() -> models.EducationalInstitutionProgram:
    """Get the MEG program from the database."""
    return (
...
def get_institution(institution_id: str) -> Optional[models.EducationalInstitution]:
    """Get an educational institution by its ID."""
    return (
...
def get_program_association(
    institution: models.EducationalInstitution, program: models.EducationalInstitutionProgram
) -> Optional[models.EducationalInstitutionProgramAssociation]:
    """Get the program association for an institution."""
    return (
...
def update_institutions_timespan(
    ...
    for institution_id in institution_ids:
        ...
        if not institution:
            logger.warning("Institution not found for institution %s", institution_id)
            continue

        association = get_program_association(institution, program)
        if not association:
            logger.warning("Association not found for institution %s", institution_id)
            continue
