...
def main(not_dry: bool) -> None:
    logger.info("Finding collective offer templates at venueId 31305 with inactive status and price '308,01'")
    templates_to_update = (
        db.session.query(educational_models.CollectiveOfferTemplate)
        .filter(
            educational_models.CollectiveOfferTemplate.venueId == 911,
    ...
    for template in templates_to_update:
        old_price = template.priceDetail
        template.priceDetail = old_price.replace("308,01", "310,47")
        logger.info(f"Template id={template.id}: priceDetail changed from '{old_price}' to '{template.priceDetail}'")
