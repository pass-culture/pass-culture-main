...
def get_inconsistent_venue_addresses() -> list[offerers_models.Venue]:
    ...
    return (
        ...
        .filter(
            ...
            offerers_models.Venue.id.not_in((6884, 57748)),
            # and some outside France
            offerers_models.Venue.id.not_in((43097, 43100, 43102, 43104)),
        )
    )
...
def create_venue_address_as_manual(existing_venues: list[offerers_models.Venue]) -> None:
    ...
    for venue in existing_venues:
        try:
            ...
            print("Error with venue #%s : %s" % (venue.id, str(e)))
            failed_venues.append(venue.id)
    print("FAILED %s (%s)" % (len(failed_venues), failed_venues))
