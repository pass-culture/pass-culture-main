...
gh workflow run on_dispatch_pcapi_console_job.yaml \
  ...
  -f RESOURCES="512Mi/.5" \
  -f BRANCH_NAME=ogeber/pc-39748-soft-delete-venues-without-offers \
  -f NAMESPACE=soft_delete_venues_without_offers \
  -f SCRIPT_ARGUMENTS="";
...
def _venue_can_be_deleted(venue: Venue) -> bool:
    ...
    if db.session.query(Venue).filter(Venue.managingOffererId == venue.managingOffererId).count() == 1:
...
@atomic()
def main(author: users_models.User, commit: bool) -> None:
    if not commit:
    ...
    for venue in venues_without_offers.all():
        ...
        if vp := venue_providers.get(venue.id, None):
            ...
            delete_venue_provider(vp, author, send_email=False)
        ...
        db.session.add(
            history_models.ActionHistory(
                ...
                comment="Venue without Offer Soft Deleted (PC-39748)",
            )
        )
        db.session.flush()
...
if __name__ == "__main__":
    ...
    main(author=author, commit=args.commit)

    logger.info("Finished" if args.commit else "Finished dry run")
