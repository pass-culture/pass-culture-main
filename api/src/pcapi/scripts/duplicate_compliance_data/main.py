...
MIN_OFFER_WITH_COMPLIANCE_SCORE = 0
...
def execute_query(i: int, batch_size: int) -> None:
    ...
    while nb_retry > 0:
        try:
            ...
            logger.info(
                "Erreur de type %s sur les lignes offererAddressId entre %s et %s", type(e).__name__, i, i + batch_size
            )
            db.session.rollback()
            nb_retry -= 1
            if nb_retry == 0:
...
if __name__ == "__main__":
    ...
    if args.starting_id < MIN_OFFER_WITH_COMPLIANCE_SCORE:
        raise ValueError('"start" must be greater than 76_375_533')
