...
https://github.com/pass-culture/pass-culture-main/blob/PC-37631-script-generate-BA-for-NC/api/src/pcapi/scripts/generate_bank_accounts_nc/main.py
...
def link_venue_to_bank_account(
    ...
    created_log = history_models.ActionHistory(
        ...
        comment="PC-37631 - Compte bancaire créé manuellement pour la Nouvelle-Calédonie",
    )
...
def create_or_update_bank_account(
    ...
    bank_account.bic = bank_account_data["bic"]
...
def generate_bank_account(bank_account_data: dict) -> None:
    venue = db.session.query(offerers_models.Venue).filter(offerers_models.Venue.siret == bank_account_data["siret"])
    offerer = venue.managingOfferer

    if not is_iban_valid(bank_account_data["iban"]):
        logger.info(f"Invalid IBAN {bank_account_data['iban']} for siret {bank_account_data['siret']}")
        return

    if not is_bic_valid(bank_account_data["bic"]):
        logger.info(f"Invalid BIC {bank_account_data['bic']} for siret {bank_account_data['siret']}")
        return
...
def main(filename: str, not_dry: bool) -> None:
    ...
    for row in rows:
        # TODO je n'ai pas l'état final du csv, donc bcp de suppositions
        bank_account_data = {
            "iban": row["IBAN"].replace(" ", ""),
            "bic": row["BIC"],
            "ds_id": int(row["N° dossier"]),
            "label": row["Intitulé du compte bancaire"],
            "siret": siren_utils.ridet_to_siret(row["RIDET"]),
        }
