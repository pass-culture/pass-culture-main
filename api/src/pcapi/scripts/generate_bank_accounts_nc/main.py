...
def create_or_update_bank_account(
    ...
    bank_account = (
        ...
        .filter_by(finance_models.BankAccount.iban == bank_account_data["iban"])
...
def generate_bank_account(bank_account_data: dict) -> None:
    ...
    if not is_iban_valid(bank_account_data["iban"]) or is_bic_valid(bank_account_data["bic"]):
        return
...
def main(filename: str, not_dry: bool) -> None:
    ...
    for row in rows:
        ...
        bank_account_data = {
            ...
            "bic": row["BIC"],
            "ds_id": int(row["JESAISPAS"]),
            "label": row["label"],
            "siret": f"NC{row['RIDET']}XX",
        }
