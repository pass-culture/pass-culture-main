...
This script creates a Venue for any Offerer created a long time ago which is still validated/active and does not have a
Venue. These offerers have not created any offer in the past years but now some of them come back and can't login
because of frontend which does not deal with offerers without venue.
...
def main(is_dry_run: bool) -> None:
    ...
    logger.info("Found %d validated offerers without venue", len(offerers))
    ...
    for offerer in offerers:
        try:
            ...
            logger.exception("Failed to create venue, skip. %s", exc)
        time.sleep(5)  # Sirene API rate limit: 30/min - 2 API calls per venue
