...
def main(apply: bool, locations_to_exclude: list[int]) -> None:
    ...
    for location in offer_locations_to_check:
        try:
            ...
            find_incorrect_offers(location, apply=apply, offer_model=offers_models.Offer)
            counter += 1
            if counter % 100 == 0:
                logger.info("Checked %s offer locations with error risk", counter)
    if FAILED_CHECKS:
...
if __name__ == "__main__":
    ...
    main(apply=args.apply, locations_to_exclude=args.exclude_locations)
