            ...
            while db.session.query(subquery_for_batch).first() and batch_counter < UPDATED_OFFERS_MAX_LIMIT:
                try:
                    with atomic():
                        ...
                        db.session.flush()
                        logger.info(
                            "updated %s offers for location %s and %s in total",
                    ...
                    logger.info(
                        "batch update failed for old location %s - Trying to update offers one by one",
                        old_location_id,
                    )
                    try:
                        with atomic():
                            ...
                            while updated_offer.rowcount and counter < BATCH_SIZE:
                                ...
                                if counter % 100 == 0:
                                    logger.info(
                                        "updated %s offers for location %s one by one, currently %s done",
                                        counter,
                                        old_location_id,
                                        batch_counter,
                                    )
                        ...
                        logger.info(
                            "Failed to update offers for old location %s, logging old location",
                            old_location_id,
                        )
...
def main_func(apply: bool = False, locations_to_exclude: list[int] = []) -> None:
    ...
    logger.info(f"Found {len(locations_doubles)} locations to check for merging")
...
if __name__ == "__main__":
    ...
    main_func(apply=args.apply, locations_to_exclude=args.exclude_locations)
