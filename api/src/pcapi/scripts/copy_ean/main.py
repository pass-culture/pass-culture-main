...
def _get_max_id() -> int:
    result = db.session.execute(sa.text("SELECT MAX(id) FROM product")).fetchone()
...
def copy_ean(
    ...
    max_id = _get_max_id()
    logger.info("Max ID for the 'product' table: %d", max_id)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Duplicate product ean between jsonData to ean column")
    parser.add_argument("--starting-id", type=int, default=0, help="starting product id")
    parser.add_argument("--ending-id", type=int, default=0, help="ending product id")
    ...
    copy_ean(args.starting_id, args.ending_id, args.batch_size, args.not_dry, args.wait_autovacuum)
