...
def extract_venues_info_to_csv(query: BaseQuery) -> None:
    venues_query_for_csv = query.with_entities(
        offerers_models.Venue.id, offerers_models.Venue.bookingEmail, offerers_models.Venue.hasOffers
    )
...
def get_venues_to_update_query() -> BaseQuery:
    offerer_with_one_physical_venue_subquery = (
        ...
        .join(offerers_models.Venue, offerers_models.Venue.managingOffererId == offerers_models.Offerer.id)
