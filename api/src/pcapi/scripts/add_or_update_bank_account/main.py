...
def _get_bank_account_from_venue_with_siret_and_same_pricing_point_as_origin_venue(
    offerer_id: int, origine_venue_pricing_point_id: int, iban: str | None
) -> BankAccount | None:
    query = (
        ...
        .join(VenuePricingPointLink, VenuePricingPointLink.pricingPointId == origine_venue_pricing_point_id)
        .filter(
    ...
    if iban:
        query.filter(BankAccount.iban == iban)
    if query.count() == 1:
        return query.one()
    ...
    for venue in venue_list:
        try:
            with atomic():
                ...
                assert venue.current_pricing_point_id
