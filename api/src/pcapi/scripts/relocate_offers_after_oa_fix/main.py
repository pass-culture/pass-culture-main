...
def relocate_offers(items: list[dict]) -> list:
    ...
    for item in items:
        ...
        try:
            ...
            (
                offers_models.Offer.query.filter(offers_models.Offer.venueId == venue_id)
                .filter(offers_models.Offer.offererAddressId == incorrect_oa_id)
                .update({offers_models.Offer.offererAddressId: correct_oa_id}, synchronize_session=False)
            )
