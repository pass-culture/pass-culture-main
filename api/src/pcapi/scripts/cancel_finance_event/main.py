...
gh workflow run on_dispatch_pcapi_console_job.yaml \
  ...
  -f RESOURCES="512Mi/.5" \
  -f BRANCH_NAME=pc-41395-cancel-uncancelled-finance-event \
...
def cancel_finance_event(event: finance_models.FinanceEvent, is_dry_run: bool) -> None:
...
if __name__ == "__main__":
    ...
    finance_event = db.session.query(finance_models.FinanceEvent).filter_by(id=50862106).one()
    cancel_finance_event(finance_event, not args.not_dry)
