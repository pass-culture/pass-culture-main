...
gh workflow run on_dispatch_pcapi_console_job.yaml \
  ...
  -f RESOURCES="512Mi/.5" \
  -f BRANCH_NAME=PC-42528-move-offers-from-offerer-to-another \
  -f NAMESPACE=move_offers_to_offerer \
  -f SCRIPT_ARGUMENTS="";
...
def main(old_venue_id: int, new_venue_id: int) -> None:
    ...
    for offer in offers:
        ...
        assert offer.offererAddress is not None  # helps mypy
        offer.offererAddressId = get_or_create_offer_location(
            new_venue.managingOffererId, offer.offererAddress.addressId, new_venue_id, label=offer.offererAddress.label
        ).id
    ...
    history_api.add_action(
        ...
        author=current_user,
        venue=old_venue,
        comment=f"PC-39757 - Offres et réservations en cours déplacées vers la venue {new_venue_id}",
    )
    history_api.add_action(
        history_models.ActionType.COMMENT,
        author=current_user,
        venue=new_venue,
        comment=f"PC-39757 - Offres et réservations en cours importées depuis la venue {old_venue_id}",
    )
