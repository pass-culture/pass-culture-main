...
def sandbox(name: tuple[str], clean: str, clean_bucket: str, index_all_offers: str, step_to_skip: tuple[str]) -> None:
    if settings.CAN_RUN_SANDBOX:
        ...
        save_sandbox(name, with_clean, with_clean_bucket, with_index_all_offers, step_to_skip)
    else:
