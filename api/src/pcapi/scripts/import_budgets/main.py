...
def _update_program(program: educational_models.EducationalInstitutionProgram, path: str, uai_header: str) -> None:
    with open(path, "r", encoding="utf-8") as csv_file:
        csv_rows = csv.DictReader(csv_file, delimiter=";")
        uais = {row[uai_header] for row in csv_rows}

    institutions = educational_models.EducationalInstitution.query.options(
        joinedload(educational_models.EducationalInstitution.programs)
    )
    ...
    for uai in uais:
        ...
        if program.id not in {prog.id for prog in institution.programs}:
            logger.info("Linking UAI %s to program %s", uai, program.name)
...
if __name__ == "__main__":
    ...
    # TODO: move this logic in import_deposit_institution_csv
    if args.educational_program_name is not None:
        educational_program = educational_models.EducationalInstitutionProgram.query.filter_by(
            name=args.educational_program_name
        ).one()
        _update_program(program=educational_program, path=file_path, uai_header="UAI")
