...
gh workflow run on_dispatch_pcapi_console_job.yaml \
  ...
  -f RESOURCES="512Mi/.5" \
  -f BRANCH_NAME=BSR-drop-offer-offer-report-fk \
  -f NAMESPACE=drop-offer-fk \
  -f SCRIPT_ARGUMENTS="";
...
if __name__ == "__main__":
    ...
    db.session.execute(
        sa.text(""" ALTER TABLE "offer_report" DROP CONSTRAINT IF EXISTS "offer_report_offerId_fkey"; """)
    )
