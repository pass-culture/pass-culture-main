...
def recreate_booking(
    ...
    if remaining_credit < amount:
        ...
        recredit = recredit_beneficiary(beneficiary, recredit_amount, original_booking.token)
        domains_credit = users_api.get_domains_credit(beneficiary)
        assert domains_credit  # helps mypy
        remaining_credit = domains_credit.all.remaining
    else:
    ...
    if original_is_canceled:
        ...
        if recredit:
            comment += (
                f" Ce jeune a été temporairement recrédité de {format_price(recredit.amount, None)}"
    ...
    if recredit:
        delete_recredit(recredit)

    update_external_user(booking.user)


def recredit_beneficiary(
    beneficiary: users_models.User, amount: decimal.Decimal, token: str
    ...
    recredit = finance_models.Recredit(
        ...
        comment=(
            "PC-43548 - Recrédit temporaire suite au retrait d'une réservation dans une autre structure afin "
    ...
    logger.info(
        "Temporary recredit", extra={"user_id": beneficiary.id, "amount": float(amount), "deposit_balance": balance}
    )

    return recredit


def delete_recredit(recredit: finance_models.Recredit) -> None:
    deposit = recredit.deposit
    deposit.amount -= recredit.amount
    db.session.delete(recredit)
    db.session.flush()

    balance = db.session.query(sa.func.get_deposit_balance(deposit.id, False)).scalar()
    logger.info("Deleted temporary recredit", extra={"user_id": deposit.userId, "deposit_balance": balance})
