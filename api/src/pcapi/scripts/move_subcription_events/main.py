...
gh workflow run on_dispatch_pcapi_console_job.yaml \
  ...
  -f NAMESPACE=move_subcription_events \
  -f SCRIPT_ARGUMENTS="";
...
def move_subscription_and_credit_destination_account(
    ...
    if was_beneficiary_credited:
        logger.info("Beneficiary %s was credited", to_user.id)
    else:
        logger.warning("Beneficiary %s was not credited", to_user.id)
...
def read_user_ids_from_csv(file_name: str) -> list[tuple[int, int]]:
    ...
    with open(file_name, encoding="utf-8") as csv_file:
        ...
        for row in reader:
            ...
            to_user_id = int(row["from_user_id"])
