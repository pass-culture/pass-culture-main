...
class BeneficiaryImportStatus(PcObject, Model):
    ...
    status: sa_orm.Mapped[ImportStatus] = sa.orm.mapped_column(
        sa.Enum(ImportStatus, create_constraint=False), nullable=False
    )

    date: sa_orm.Mapped[datetime] = sa.orm.mapped_column(
        sa.DateTime, nullable=False, default=datetime.utcnow, server_default=sa.func.now()
    )

    detail = sa.orm.mapped_column(sa.String(255), nullable=True)

    beneficiaryImportId: sa_orm.Mapped[int] = sa.orm.mapped_column(
    ...
    authorId = sa.orm.mapped_column(sa.BigInteger, sa.ForeignKey("user.id"), nullable=True)
