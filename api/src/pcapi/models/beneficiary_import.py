...
class BeneficiaryImport(PcObject, Model):
    ...
    applicationId = sa.orm.mapped_column(sa.BigInteger, nullable=True)

    beneficiaryId = sa.orm.mapped_column(sa.BigInteger, sa.ForeignKey("user.id"), index=True, nullable=True)

    sourceId = sa.orm.mapped_column(sa.Integer, nullable=True)

    source: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.String(255), nullable=False)

    thirdPartyId = sa.orm.mapped_column(sa.TEXT, nullable=True, index=True)

    eligibilityType: sa_orm.Mapped[EligibilityType] = sa.orm.mapped_column(
