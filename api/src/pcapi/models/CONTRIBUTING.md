...
- Cas 1 : Le cas nominal
  ...
  - Exemple:
    ```python
    quantity = sa.orm.mapped_column(sa.Integer, nullable=True) # quantity est de type int | None
    ```
...
- Cas 2 : le cas des colonnes non nullables
  ...
  - Exemple :
    ```python
    subcategoryId: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.Text, nullable=False) # subcategoryId est de type str
    ```
