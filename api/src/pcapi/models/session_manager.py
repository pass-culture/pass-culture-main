...
class EnginesContainer:
    ...
    and `clean_engines`
    All public method of thuis class are thread safe
    """
    ...
    def checkout_engine(self, database_url: str, **engine_options: str | int | None) -> sa.engine.Engine:
...
class DbClass:
    ...
    def init_app(self, app) -> None:
        app.extensions[EXTENSION_NAME] = EnginesContainer()
    ...
    def clean_engines(self) -> None:
        number = random.randint(0, settings.SQLALCHEMY_ENGINE_CLEAN_PROBABILITY)
        if number == 1:
            engines_container = self._get_engines_container()
