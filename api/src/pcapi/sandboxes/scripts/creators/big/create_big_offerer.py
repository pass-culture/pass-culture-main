...
def create_big_offerer() -> None:
    ...
    user_offerer = UserOffererFactory(user__email="gros@example.com", offerer__name="Gros offerer")
    # offerers have always a virtual venue so we have to create one to match reality
    VirtualVenueFactory(name="Lieu virtuel du gros offerer", managingOfferer=user_offerer.offerer)
    ...
    for i in range(0, 51):
        ...
        for j in range(0, 100):
            ...
            stock = EventStockFactory(
                offer=offer_event,
                quantity=10,
                beginningDatetime=datetime.datetime.now().replace(second=0, microsecond=0)
