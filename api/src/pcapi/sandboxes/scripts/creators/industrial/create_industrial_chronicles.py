...
def create_industrial_chronicles() -> None:
    ...
    for user, product, i in zip(itertools.cycle(users), itertools.cycle(products), range(30)):
        ...
        offers_factories.ChronicleFactory(
            age=(15 + (i % 5)),
            city=user.city,
            content="Chronique sur le produit {product.name} écrite par l'utilisateur {user.name} ({user.id}).",
    ...
    for user, i in zip(itertools.cycle(users), range(5)):
        ...
        offers_factories.ChronicleFactory(
            age=(15 + (i % 5)),
            city=user.city,
            content=f"Chronique sans produit mais écrite par l'utilisateur {user.name} ({user.id}).",
            eanChoiceId=hashlib.sha256(ean.encode()).hexdigest()[:20],
            ean=ean,
            email=user.email,
            firstName=user.firstName,
    ...
    offers_factories.ChronicleFactory(
        ...
        content="""
Une chronique avec un tres long contenu en Français mais comme je n'ai pas d'idées voici à la place un extrait quelconque du Discours de la servitude volontaire d'Étienne de La Boetie:
