...
def create_e2e_app_users() -> dict[str, User]:
    beneficiaries = create_e2e_app_beneficiaries()
    underage_beneficiaries = create_e2e_app_underage_beneficiaries()
    other_users = create_e2e_app_other_users()
    general_public_users = create_e2e_app_general_public_users()
...
def create_e2e_app_beneficiaries() -> dict[str, User]:
    logger.info("create_e2e_app_beneficiaries")
...
def create_e2e_app_underage_beneficiaries() -> dict[str, User]:
    logger.info("create_e2e_app_underage_beneficiaries")
...
def create_e2e_app_other_users() -> dict[str, User]:
    logger.info("create_e2e_app_other_users")
    ...
    for index, (departement_code, tag) in enumerate(variants, start=0):
        ...
        email = f"signupJourney{departement_code}.{tag}@example.com"
...
def create_e2e_app_general_public_users() -> dict[str, User]:
    logger.info("create_e2e_app_general_public_users")
    ...
    for index, (age,) in enumerate(variants, start=100):
        ...
        email = f"signupJourney.{age}@example.com"
        departement_code = 39
