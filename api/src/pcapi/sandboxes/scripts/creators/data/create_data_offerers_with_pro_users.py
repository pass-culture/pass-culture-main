...
OFFERER_LOCATIONS_DATA= [
    ...
    Location(address="Rue Francois Rude DATA", city="Drancy", latitude=48.926432, longitude=2.432279, postalCode="93700"),
    Location(address="Rue Pollet DATA", city="Aulnay-sous-Bois", latitude=48.945379, longitude=2.502902, postalCode="93600"),
    Location(
        address="Rue de Pimodan DATA", city="Aulnay-sous-Bois", latitude=48.926299, longitude=2.490079, postalCode="93600"
    ),
    Location(
        address="Rue de Pologne DATA", city="Aulnay-sous-Bois", latitude=48.940826, longitude=2.479869, postalCode="93600"
    ),
...
def create_industrial_offerers_with_pro_users() -> tuple[dict[str, Offerer], dict[str, User]]:
    ...
    for location_index, location in enumerate(OFFERER_LOCATIONS_DATA):
        ...
        for _i in range(ATTACHED_PRO_USERS_COUNT):
            ...
            last_name = MOCK_LAST_NAMES[user_index % len(MOCK_LAST_NAMES)]
            email = get_email(first_name, last_name, domain)
            user_name = "{} {} {}".format(first_name, last_name,"DATA")
            user_validation_token = None
            pro = users_factories.ProFactory(
                departementCode=departement_code,
                email=email,
    ...
    objects_to_save = (
        list(offerers_by_name.values()) + list(users_by_name.values())
    )
