...
def create_pro_user_with_collective_offer_templates() -> dict:
    ...
    offerPublished = educational_factories.create_collective_offer_template_by_status(
        CollectiveOfferDisplayedStatus.PUBLISHED, venue=venue1, name="Offre vitrine publiée", formats=[EacFormat.ATELIER_DE_PRATIQUE]
    )
    offerDraft = educational_factories.create_collective_offer_template_by_status(
        CollectiveOfferDisplayedStatus.DRAFT, venue=venue1, name="Offre vitrine brouillon", formats=[EacFormat.REPRESENTATION]
    )
    offerArchived = educational_factories.create_collective_offer_template_by_status(
        CollectiveOfferDisplayedStatus.ARCHIVED, venue=venue2, name="Offre vitrine archivée en établissement scolaire", formats=[EacFormat.PROJECTION_AUDIOVISUELLE], locationType=educational_models.CollectiveLocationType.SCHOOL
    )
    offerUnderReview = educational_factories.create_collective_offer_template_by_status(
        CollectiveOfferDisplayedStatus.UNDER_REVIEW, venue=venue2, name="Offre vitrine en instruction", formats=[EacFormat.REPRESENTATION]
    )
    offerRejected = educational_factories.create_collective_offer_template_by_status(
        CollectiveOfferDisplayedStatus.REJECTED, venue=venue2, name="Offre vitrine non conforme", formats=[EacFormat.REPRESENTATION]
    )
