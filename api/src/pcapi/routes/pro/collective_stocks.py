...
@blueprint.pro_private_schema.validate(
    resp=spectree.Response(
        HTTP_201=collective_stock_serialize.CollectiveStockResponseModel,
        HTTP_403=None,
        HTTP_400=None,
        HTTP_404=None,
    ),
...
def create_collective_stock(
    json: collective_stock_serialize.CollectiveStockCreationBodyModel,
) -> tuple[collective_stock_serialize.CollectiveStockResponseModel, int]:
    try:
        offerer = offerers_repository.get_by_collective_offer_id(json.offerId)
    except offerers_exceptions.CannotFindOffererForOfferId:
        raise ResourceNotFoundError({"offerer": ["Aucune structure trouvée à partir de cette offre"]})
    check_user_has_access_to_offerer(current_user, offerer.id)

    try:
        collective_stock = educational_api_stock.create_collective_stock(json)
    ...
    return collective_stock_serialize.CollectiveStockResponseModel.model_validate(collective_stock), 201
...
@blueprint.pro_private_schema.validate(
    resp=spectree.Response(
        HTTP_200=collective_stock_serialize.CollectiveStockResponseModel,
        HTTP_403=None,
        HTTP_400=None,
        HTTP_401=None,
        HTTP_404=None,
...
def edit_collective_stock(
    collective_stock_id: int, json: collective_stock_serialize.CollectiveStockEditionBodyModel
) -> tuple[collective_stock_serialize.CollectiveStockResponseModel, int]:
    ...
    try:
        educational_api_stock.edit_collective_stock(
            stock=collective_stock, stock_data=json.model_dump(exclude_unset=True, by_alias=False)
        )
    ...
    return collective_stock_serialize.CollectiveStockResponseModel.model_validate(collective_stock), 200
