...
def post_offer(body: offers_serialize.PostOfferBodyModel) -> offers_serialize.GetIndividualOfferResponseModel:
    ...
    try:
        with repository.transaction():
            ...
            fields.pop("venueId")
            fields["extraData"] = offers_serialize.deserialize_extra_data(fields["extraData"])
            offer_body = offers_schemas.CreateOffer(**fields)
            offer = offers_api.create_offer(offer_body, venue, is_from_private_api=True)
...
def patch_offer(
    ...
    try:
        with repository.transaction():
            ...
            updates["extraData"] = offers_serialize.deserialize_extra_data(updates.get("extraData", offer.extraData))
