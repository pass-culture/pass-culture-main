...
def update_event_opening_hours(
    ...
    try:
        validation.check_event_opening_hours_can_be_updated(offer, opening_hours, body)
    except exceptions.EventOpeningHoursError as error:
        extra = {"offer": offer_id, "event": event_opening_hours_id, "body": "pending"}
        with suppress(Exception):
            extra["body"] = body.dict()

        logger.warning("Failed to update event opening hours", extra=extra)
