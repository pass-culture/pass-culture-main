...
@spectree_serialize(
    api=blueprint.api,
    response_model=venue_serialization.GetVenuesByNameResponseModel,
    on_error_statuses=[404, 422],
    tags=("get venues",),
)
@adage_api_key_required
def get_venues_from_name(venues_name: str) -> venue_serialization.GetVenuesByNameResponseModel:
    venues = api.get_venues_by_name(venues_name)
    if len(venues) == 0:
        raise ApiErrors({"code": "VENUES_NOT_FOUND"}, status_code=404)

    return venue_serialization.GetVenuesByNameResponseModel(
