...
from pcapi.routes.provider.services.authentication import current_api_key
...
def check_location_belongs_to_offer_venue(
...
def save_image(image_body: serialization.ImageBody, offer: offers_models.Offer) -> None:
    try:
        image_as_bytes = provider_utils.get_bytes_from_base64_string(image_body.file)
    except provider_utils.InvalidBase64Exception:
...
def extract_offerer_address_from_location(
    ...
    if location.type == "address":
        address = provider_utils.get_address_or_raise_404(location.address_id)
        # "merge" the location with the one from the venue if both match
        if address.id == venue.offererAddress.addressId and location.address_label == venue.publicName:
