...
def create_individual_booking_incident() -> utils.BackofficeResponse:
    ...
    if not validation.check_incident_bookings(bookings):
        return redirect(
            request.referrer or url_for("backoffice_v3_web.individual_bookings.list_individual_bookings"), 303
        )
    ...
    return redirect(request.referrer or url_for("backoffice_v3_web.individual_bookings.list_individual_bookings"), 303)
...
def create_collective_booking_incident(collective_booking_id: int) -> utils.BackofficeResponse:
    ...
    if not validation.check_incident_collective_booking(collective_booking) or not validation.check_total_amount(
        ...
        return redirect(
            request.referrer or url_for("backoffice_v3_web.collective_bookings.list_collective_bookings"), 303
        )
    ...
    return redirect(request.referrer or url_for("backoffice_v3_web.collective_bookings.list_collective_bookings"), 303)
...
def get_finance_incident_validation_form(finance_incident_id: int) -> utils.BackofficeResponse:
    ...
    return render_template(
        ...
        dst=url_for(
            "backoffice_v3_web.finance_incident.validate_finance_incident", finance_incident_id=finance_incident_id
        ),
...
def validate_finance_incident(finance_incident_id: int) -> utils.BackofficeResponse:
    ...
    if finance_incident.status != finance_models.IncidentStatus.CREATED:
        ...
        return redirect(
            request.referrer
            or url_for("backoffice_v3_web.finance_incident.get_incident", finance_incident_id=finance_incident.id),
            303,
        )
