...
from pcapi.routes.public.individual_offers.v1 import bookings_serialization as serialization
from pcapi.routes.public.united import utils
from pcapi.validation.routes.users_authentifications import current_api_key


INDIVIDUAL_BOOKINGS_TAG = "Individual bookings"


@utils.public_api_route("/individual/bookings", method="GET", tags=[INDIVIDUAL_BOOKINGS_TAG])
def get_bookings_by_offer(
    query: serialization.GetFilteredBookingsRequest,
) -> serialization.GetFilteredBookingsResponse:
    """List individual offers"""
...
@utils.public_api_route("/individual/bookings/token/<string:token>", method="GET", tags=[INDIVIDUAL_BOOKINGS_TAG])
def get_booking_by_token(token: str) -> serialization.GetBookingResponse:
    """Find a booking by token

    The token is a unique string that serves as a proof of booking. It
    is generated for each booking and send to the each user.
    """
...
@utils.public_api_route("/individual/bookings/token/use/<string:token>", method="PATCH", tags=[INDIVIDUAL_BOOKINGS_TAG])
def validate_booking_by_token(token: str) -> None:
    """Validate a booking

    Find a booking using its token and validate it.
    """
    booking = _get_booking_by_token(token)
    if booking is None:
        raise api_errors.ApiErrors(errors={"global": "This countermark cannot be found"}, status_code=404)
