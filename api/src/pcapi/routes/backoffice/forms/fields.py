    ...
    def pre_validate(self, form: wtforms.Form) -> None:
        if self.data:
            if self.data.mimetype not in ("image/jpeg", "image/png"):
                raise ValidationError("Format invalid, seuls les images au format PNG ou JPEG sont acceptés")
            MAX_IMAGE_SIZE = 1_000_000
            self.data.seek(0, os.SEEK_END)
            file_size = self.data.tell()
            self.data.seek(0)
            if file_size > MAX_IMAGE_SIZE:
                raise ValidationError(f"Image trop grande, max: {MAX_IMAGE_SIZE / 1_000}Ko")
        super().pre_validate(form)
