...
def get_user_id_for_connect_as_base_query() -> BaseQuery:
    return (
        ...
        .filter(
            users_models.User.isActive == True,
...
def _get_user_from_user_id(user_id: int) -> users_models.User:
    if not FeatureToggle.WIP_CONNECT_AS.is_active():
        raise MessageException("L'utilisation du « connect as » requiert l'activation de la feature : WIP_CONNECT_AS")
    user = users_models.User.query.filter(users_models.User.id == user_id).one_or_none()
    if not user:
        raise NotFound()

    if not user.isActive:
        raise MessageException("L'utilisation du « connect as » n'est pas disponible pour les comptes inactifs")

    if user.has_admin_role:
        raise MessageException("L'utilisation du « connect as » n'est pas disponible pour les comptes admin")

    if user.has_anonymized_role:
        raise MessageException("L'utilisation du « connect as » n'est pas disponible pour les comptes anonymisés")

    if not (user.has_non_attached_pro_role or user.has_pro_role):
        raise MessageException("L'utilisation du « connect as » n'est disponible que pour les comptes pro")
    return user.id


def _get_user_from_venue_id(venue_id: int) -> users_models.User:
    ...
    if not user_id:
        raise MessageException("Aucun utilisateur approprié n'a été trouvé pour se connecter à ce lieu")
    return user_id


def _get_user_from_offerer_id(offerer_id: int) -> users_models.User:
    if not FeatureToggle.WIP_CONNECT_AS_EXTENDED.is_active():
        raise MessageException(
            "L'utilisation de la version étendue de « connect as » requiert l'activation de la feature : WIP_CONNECT_AS_EXTENDED"
        )
    query = get_user_id_for_connect_as_base_query()
    user_id = query.filter(offerers_models.UserOfferer.offererId == offerer_id).limit(1).scalar()

    if not user_id:
        raise MessageException("Aucun utilisateur approprié n'a été trouvé pour se connecter à cette structure")
    return user_id


def _get_user_from_offer_id(offer_id: int) -> users_models.User:
    if not FeatureToggle.WIP_CONNECT_AS_EXTENDED.is_active():
        raise MessageException(
            "L'utilisation de la version étendue de « connect as » requiert l'activation de la feature : WIP_CONNECT_AS_EXTENDED"
        )
    ...
    if not user_id:
        raise MessageException("Aucun utilisateur approprié n'a été trouvé pour se connecter à cette offre")
    return user_id
...
def connect_as() -> utils.BackofficeResponse:
    ...
    try:
        match form.object_type.data:
            case "offer":
                user_id = _get_user_from_offer_id(form.object_id.data)
            case "offerer":
                user_id = _get_user_from_offerer_id(form.object_id.data)
            case "venue":
                user_id = _get_user_from_venue_id(form.object_id.data)
            case "user":
                user_id = _get_user_from_user_id(form.object_id.data)
            case _:
                raise MessageException(f"{form.object_type.data} non supporté pour le connect as")
    except MessageException as exp:
        flash(exp.message, "warning")
        mark_transaction_as_invalid()
        return redirect(request.referrer or url_for("backoffice_web.home"), code=303)
