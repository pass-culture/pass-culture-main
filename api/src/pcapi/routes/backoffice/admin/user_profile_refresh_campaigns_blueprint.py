...
def create_campaign() -> utils.BackofficeResponse:
    ...
    if new_campaign_is_active:
        db.session.query(users_models.UserProfileRefreshCampaign).filter(
            users_models.UserProfileRefreshCampaign.isActive == True,
...
def edit_campaign(campaign_id: int) -> utils.BackofficeResponse:
    ...
    if (campaign.isActive != form.is_active.data) or (campaign.campaignDate != form.campaign_date.data):
        ...
        if campaign.isActive:
            db.session.query(users_models.UserProfileRefreshCampaign).filter(
                users_models.UserProfileRefreshCampaign.isActive == True,
                users_models.UserProfileRefreshCampaign.id != campaign_id,
            ).update({"isActive": False})
