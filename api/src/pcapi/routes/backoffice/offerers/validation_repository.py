...
def _build_address_filter(condition: sa.ColumnElement[bool]) -> sa.Exists:
    return (
        ...
        .where(offerers_models.OffererAddress.venueId == offerers_models.Venue.id)
        .where(offerers_models.OffererAddress.type == offerers_models.LocationType.VENUE_LOCATION)
    ...
    if q:
        ...
        if string_utils.is_numeric(sanitized_q):
            ...
            if num_digits == 12:
                ...
                raise ApiErrors(
                    {"q": ["Le nombre de chiffres ne correspond pas à un SIREN, code postal, département ou ID DN CB"]},
                    status_code=400,
                )
