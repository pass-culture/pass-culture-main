...
from pcapi.routes.backoffice.filters import format_offer_category
...
@list_artists_blueprint.route("/<string:artist_id>/associate-product", methods=["GET", "POST"])
@utils.permission_required(perm_models.Permissions.MANAGE_OFFERS)
def associate_product(artist_id: str) -> utils.BackofficeResponse:
    artist = db.session.query(artist_models.Artist).filter_by(id=artist_id).one()
    search_form = forms.AssociateProductSearchForm(request.form)

    context = {
        "div_id": f"associate-product-modal-{artist.id}",
        "dst": url_for(".associate_product", artist_id=artist_id),
        "title": "Associer un produit",
        "form": search_form,
        "information": Markup(
            "Rechercher un produit via son identifiant pour l'associer à <b>{artist_name}</b>."
        ).format(artist_name=artist.name),
    ...
    if request.method == "POST" and search_form.validate():
        ...
        if found_product:
            ...
            html_template = """
            <div class="alert alert-light mt-2 d-flex gap-3 align-items-start">

                {% if found_product.images.recto %}
                    <img src="{{ found_product.images.recto }}" class="img-thumbnail" style="max-height: 120px;" alt="Image du produit">
                {% endif %}

                <div>
                    <h5 class="mb-1">{{ found_product.name }}</h5>
                    <p class="mb-1 text-muted">ID : {{ found_product.id }}</p>
                    <p class="mb-1">
                        <strong>Catégorie :</strong> {{ format_offer_category(found_product.subcategoryId) }}<br>
                        <strong>Sous-catégorie :</strong> {{ format_offer_subcategory(found_product.subcategoryId) }}
                    </p>
                    <p class="mb-0">
                        {% if found_product.extraData.author %}
                            <strong>Auteur :</strong> {{ found_product.extraData.author }}<br>
                        {% endif %}
                        {% if found_product.extraData.editeur %}
                            <strong>Éditeur :</strong> {{ found_product.extraData.editeur }}<br>
                        {% endif %}
                        {% if found_product.extraData.performer %}
                            <strong>Interprète :</strong> {{ found_product.extraData.performer }}<br>
                        {% endif %}
                        {% if found_product.durationMinutes %}
                            <strong>Durée :</strong> {{ found_product.durationMinutes }} minutes<br>
                        {% endif %}
                        {% if found_product.extraData.diffusionVersion %}
                            <strong>Langue :</strong> {{ found_product.extraData.diffusionVersion }}
                        {% endif %}
                    </p>
                </div>
            </div>
            ...
            information_html = template.render(
                found_product=found_product,
                format_offer_category=format_offer_category,
                format_offer_subcategory=format_offer_subcategory,
            )

            context.update(
                {
                    "dst": url_for(".confirm_association", artist_id=artist_id),
            ...
            context["alert"] = "Aucun produit trouvé pour cet identifiant."

    return render_template("components/turbo/modal_form.html", **context)
...
def confirm_association(artist_id: str) -> utils.BackofficeResponse:
    ...
    response = make_response(render_template("artists/close_turbo_frame_modal.html"))
...
def post_split_artist(artist_id: str) -> utils.BackofficeResponse:
    ...
    if not form.validate():
        ...
        return render_template("components/turbo/modal_form.html", **context), 422
