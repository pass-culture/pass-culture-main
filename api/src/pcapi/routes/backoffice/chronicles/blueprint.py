...
def attach_offer(chronicle_id: int) -> utils.BackofficeResponse:
    ...
    if not offer:
        mark_transaction_as_invalid()
        flash("Aucune offre n'a été trouvée pour cet identifiant", "warning")
        return redirect(
    ...
    flash(
        Markup(
            "L'offre <b>{offer_name}</b> a été rattaché à toutes les chroniques sur la même œuvre que celle-ci"
...
def detach_offer(chronicle_id: int, offer_id: int) -> utils.BackofficeResponse:
    ...
    if deleted:
        flash("L'offre a bien été détaché de toutes les chroniques sur la même œuvre que celle-ci", "success")
    else:
        mark_transaction_as_invalid()
        flash("L'offre n'existe pas ou n'était pas attaché à la chronique", "warning")
