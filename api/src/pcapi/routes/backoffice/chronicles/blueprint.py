...
def list_chronicles() -> utils.BackofficeResponse:
    ...
    q_filters = []
    if form.q.data and form.q.data.isdigit():
        query = query.join(chronicles_models.Chronicle.products)
        product_identifier = string_utils.format_ean_or_visa(form.q.data)
        q_filters.append(
            sa.or_(
...
def attach_product(chronicle_id: int) -> utils.BackofficeResponse:
    ...
    if selected_chronicle.productIdentifierType == chronicles_models.ChronicleProductIdentifierType.EAN:
        products = (
            db.session.query(offers_models.Product)
            .filter(offers_models.Product.ean == form.product_identifier.data)
            .all()
        )
    elif selected_chronicle.productIdentifierType == chronicles_models.ChronicleProductIdentifierType.ALLOCINE_ID:
        products = (
            db.session.query(offers_models.Product)
