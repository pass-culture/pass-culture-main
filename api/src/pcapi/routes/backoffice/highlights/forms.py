...
class CreateHighlightForm(FlaskForm):
    ...
    availability_timespan = fields.PCDateRangeField(
        "Dates de disponibilité des demandes de participation",
        validators=(wtforms.validators.InputRequired("Les dates de demande sont obligatoires"),),
    )
    ...
    def validate(self, extra_validators: dict | None = None) -> bool:
        ...
        if availability_timespan[1] > highlight_timespan[1]:
            flash(
                "La disponibilité des demandes de participation ne peut pas se terminer après la fin du temps fort",
                "warning",
            )
            return False
        return True
    ...
    def validate_availability_timespan(self, dates: fields.PCDateRangeField) -> fields.PCDateRangeField:
        if dates.data[1].date() < datetime.date.today():
            raise wtforms.validators.ValidationError(
                "La date de fin de demande de participation ne peut pas être dans le passé",
            )
        return dates
