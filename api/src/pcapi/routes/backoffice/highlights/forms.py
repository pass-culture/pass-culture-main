...
class CreateHighlightForm(FlaskForm):
    ...
    communication_date = fields.PCDateField(
        ...
        validators=(wtforms.validators.InputRequired("La date de valorisation est obligatoire"),),
    )
    ...
    def validate(self, extra_validators: dict | None = None) -> bool:
        ...
        if communication_date > highlight_datespan[1].date():
            flash(
                "La date de valorisation ne peut pas être après la fin de l'évènement",
                "warning",
            )
            return False
    ...
    def validate_communication_date(self, communication_date: fields.PCDateField) -> fields.PCDateField:
        if communication_date.data < datetime.date.today():
            raise wtforms.validators.ValidationError("La date de valorisation ne peut pas être dans le passé")
