...
def _render_responses_rows(special_event_id: int, responses_ids: list[int]) -> utils.BackofficeResponse:
    if not responses_ids:
        return render_template("operations/details_rows.html", items=[])
...
def _get_special_event_responses(
    ...
    response_rows_query = response_rows_query.outerjoin(operations_models.SpecialEventResponse.user)
