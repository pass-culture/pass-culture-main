    ...
    if is_advanced_pro_support:
        try:
            venue_choices = offers_api.check_can_move_offer(
                offer, only_event=FeatureToggle.ONLY_MOVE_EVENT_OFFERS.is_active()
            )
...
def edit_offer_venue(offer_id: int) -> utils.BackofficeResponse:
    ...
    try:
        ...
        offers_api.move_offer(
            offer, destination_venue, notify_beneficiary=form.notify_beneficiary.data, only_event=True
        )
