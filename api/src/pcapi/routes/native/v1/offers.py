...
@spectree_serialize(
    response_model=serializers.VenueMovieCalendarResponse, api=blueprint.api, on_error_statuses=[400, 404]
)
def get_movie_screenings_by_venue(
    ...
    offers = repository.get_nearby_bookable_screenings_from_venue(
        venue_id,
        query.from_datetime,
        query.to_datetime,
    )

    calendar = serializers.make_venue_movie_calendar_from_offers(offers, query.from_datetime, query.to_datetime)
    return serializers.VenueMovieCalendarResponse(calendar=calendar)
