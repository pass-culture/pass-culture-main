...
class ListCollectiveBookingsResponseModel(BaseModel):
    collective_bookings_recap: list[CollectiveBookingResponseModel]
    page: int
    pages: int
    total: int
...
def build_status_history(
    ...
    if booking_status == CollectiveBookingStatus.USED:
        serialized_booking_status_history.append(
            _serialize_collective_booking_status_info(CollectiveBookingRecapStatus.validated, date_used)
        )

    if booking_status == CollectiveBookingStatus.CANCELLED:
        serialized_booking_status_history.append(
            _serialize_collective_booking_status_info(CollectiveBookingRecapStatus.cancelled, cancellation_date)
        )
    if booking_status == CollectiveBookingStatus.REIMBURSED:
        serialized_booking_status_history.append(
...
def serialize_collective_booking_stock(collective_booking: AbstractKeyedTuple) -> dict:
    return {
        "offer_name": collective_booking.offerName,
        "offer_identifier": humanize(collective_booking.offerId),
        "event_beginning_datetime": collective_booking.stockBeginningDatetime,
        "offer_isbn": None,
        "offer_is_educational": True,
    }


def serialize_collective_booking_redactor(collective_booking: AbstractKeyedTuple) -> dict:
    return {
...
def serialize_collective_booking(collective_booking: AbstractKeyedTuple) -> dict:
    return {
...
def serialize_collective_bookings_page(
    page: int, per_page_limit: int, total_collective_bookings: int, collective_bookings: list[AbstractKeyedTuple]
) -> dict:
    return {
        "bookings_recap": [serialize_collective_booking(booking) for booking in collective_bookings],
        "page": page,
