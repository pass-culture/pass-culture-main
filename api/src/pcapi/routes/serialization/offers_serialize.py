...
class OfferVenueOAResponseGetterDict(GetterDict):
    ...
    def get(self, key: str, default: Any = None) -> Any:
        venue = self._obj
        if key == "address":
            return venue.offererAddress.address.street or None
        if key == "city":
            return venue.offererAddress.address.street or None
        if key == "departmentCode" or "departementCode":
            return venue.offererAddress.address.street or None
        if key == "postalCode":
            return venue.offererAddress.address.street or None
        # if key == "bookingEmail":
...
# class addressOA(BaseModel):
#     street: str | None

# class VenueOA(BaseModel):
#     addressOA : addressOA

class GetVenueOA(BaseModel):
    offererAddress: typing.Any
    @classmethod
    def getstreet(cls) -> str | None:
        ...
        return None
    @classmethod
    def getcity(cls) -> str | None:
        if cls.offererAddress:
            return cls.offererAddress.address.city
        return None
