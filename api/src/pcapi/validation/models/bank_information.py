...
def validate(bank_information: BankInformation, api_errors: ApiErrors) -> ApiErrors:
    if bank_information.status == BankInformationStatus.ACCEPTED:
        ...
        if bank_information.iban is not None:
            api_errors.add_error("iban", f"L’IBAN doit être vide pour le statut {bank_information.status.name}")  # type: ignore [attr-defined]
        if bank_information.bic is not None:
            api_errors.add_error("bic", f"Le BIC doit être vide pour le statut {bank_information.status.name}")  # type: ignore [attr-defined]
