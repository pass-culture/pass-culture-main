...
def upgrade() -> None:
    ...
    op.execute(
        """
            ...
            WHERE offer."subcategoryId" IN ('SPECTACLE_REPRESENTATION','SPECTACLE_ENREGISTRE','LIVESTREAM_EVENEMENT','FESTIVAL_SPECTACLE','ABO_SPECTACLE','SPECTACLE_VENTE_DISTANCE')
            AND (("jsonData" ->> 'showSubType'::text)) IS NOT NULL;
        """
    )
    op.execute(
        f"""
