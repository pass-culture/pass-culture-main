...
class AlgoliaBackend(base.SearchBackend):
    ...
    def _pop_ids_from_queue(
        ...
        # for example. To work around that, we move the items to
        # process to a specific queue. This queue is deleted once all
        # items are processed. A separate cron job looks for these
        # (specially-named) queues and adds back their items to the
        # initial queue (see `manage_processing_queues`).
        try:
...
def manage_processing_queues() -> None:
    ...
    for queue in QUEUES:
        ...
        _cursor, processing_queues = redis_client.scan(0, f"{queue}:processing:*")
            ...
            with redis_client.pipeline(transaction=True) as pipeline:
                try:
                    ...
                    for _i in range(count):
                        # Google Cloud has Redis 5.0. When it's migrated to 6.2 or
                        # newer, we can use `lmove` or `smove` instead.
                        pipeline.rpoplpush(processing_queue, queue)
                        pipeline.execute()
                    ...
                    logger.info(
                        "Found old processing queue, moved items to main queue",
                        extra={"queue": queue, "processing_queue": processing_queue},
                    )
