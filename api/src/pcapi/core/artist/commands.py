...
def import_all_artists_data(override_existing_data: bool = False) -> None:
    import_all_artists(override_existing_data=override_existing_data)
    import_all_artist_product_links(override_existing_data=override_existing_data)
    import_all_artist_aliases(override_existing_data=override_existing_data)
...
def bulk_update_database(inserted_data: list, updated_data: list, override_existing_data: bool = False) -> None:
    with transaction():
        db.session.bulk_save_objects(inserted_data)
    if updated_data and override_existing_data:
        with transaction():
            db.session.bulk_save_objects(updated_data)

    name_of_class = type(inserted_data[0]).__name__

    logger.info("Successfully imported %s %s", len(inserted_data), name_of_class)
    logger.info("%s %s %s", "Modified" if override_existing_data else "Skipped", len(updated_data), name_of_class)


def import_all_artists(override_existing_data: bool = False) -> None:
    ...
    for raw_artist in get_all_artists():
        ...
        if existing_artist and not override_existing_data:
            continue
        ...
        new_artist = artist_models.Artist(
            ...
            image_author=raw_artist.image_author,
            image_license=raw_artist.image_license,
            image_license_url=raw_artist.image_license_url,
        )

        if existing_artist:
            modified_artists.append(new_artist)
        else:
            imported_artists.append(new_artist)

    bulk_update_database(
        inserted_data=imported_artists, updated_data=modified_artists, override_existing_data=override_existing_data
    )


def import_all_artist_product_links(override_existing_data: bool = False) -> None:
    logger.info("Importing artist product links from BigQuery")
    imported_product_links: list[artist_models.ArtistProductLink] = []
    modified_product_links: list[artist_models.ArtistProductLink] = []
    ...
    for raw_product_link in get_all_artist_product_links():
        ...
        if existing_product_link and not override_existing_data:
            continue
        new_product_link = artist_models.ArtistProductLink(
            artist_id=raw_product_link.artist_id,
            product_id=raw_product_link.product_id,
            artist_type=raw_product_link.artist_type,
        )
        if existing_product_link:
            modified_product_links.append(new_product_link)
        else:
            imported_product_links.append(new_product_link)

    bulk_update_database(
        inserted_data=imported_product_links,
        updated_data=modified_product_links,
        override_existing_data=override_existing_data,
    )


def import_all_artist_aliases(override_existing_data: bool = False) -> None:
    logger.info("Importing artist aliases from BigQuery")
    imported_aliases: list[artist_models.ArtistAlias] = []
    modified_aliases: list[artist_models.ArtistAlias] = []
