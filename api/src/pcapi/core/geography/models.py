...
class IrisFrance(PcObject, Model):
    ...
    code = sa.orm.mapped_column(sa.String(9), nullable=False, unique=True)
    shape: sa_orm.Mapped[str | Geometry] = sa.orm.mapped_column(
...
class Address(PcObject, Model):
    ...
    banId: sa_orm.Mapped[str | None] = sa.orm.mapped_column(sa.Text(), nullable=True)
    inseeCode: sa_orm.Mapped[str | None] = sa.orm.mapped_column(sa.Text(), nullable=True)
    street: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.Text(), nullable=False)
    postalCode: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.Text(), nullable=False)
    city: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.Text(), nullable=False)
    latitude: sa_orm.Mapped[Decimal] = sa.orm.mapped_column(sa.Numeric(8, 5), nullable=False)
    longitude: sa_orm.Mapped[Decimal] = sa.orm.mapped_column(sa.Numeric(8, 5), nullable=False)
    departmentCode: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.Text(), nullable=False, index=True)
    ...
    isManualEdition: sa_orm.Mapped[bool] = sa.orm.mapped_column(
