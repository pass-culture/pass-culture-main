...
def get_educonnect_failure_subscription_message(
    ...
    if educonnect_fraud_check.resultContent is not None:
        educonnect_content = typing.cast(educonnect_schemas.EduconnectContent, educonnect_fraud_check.source_data())
    ...
    if (
        subscription_models.FraudReasonCode.AGE_NOT_VALID in reason_codes
        or subscription_models.FraudReasonCode.NOT_ELIGIBLE in reason_codes
    ) and birth_date:
    ...
    elif subscription_models.FraudReasonCode.DUPLICATE_USER in reason_codes:
        user_message = subscription_messages.build_duplicate_error_message(
            educonnect_fraud_check.user, subscription_models.FraudReasonCode.DUPLICATE_USER, educonnect_content
        )

    elif subscription_models.FraudReasonCode.DUPLICATE_INE in reason_codes:
        user_message = subscription_messages.build_duplicate_error_message(
            educonnect_fraud_check.user, subscription_models.FraudReasonCode.DUPLICATE_INE, educonnect_content
        )
