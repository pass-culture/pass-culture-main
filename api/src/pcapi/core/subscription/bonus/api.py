...
def apply_for_quotient_familial_bonus(quotient_familial_fraud_check: subscription_models.BeneficiaryFraudCheck) -> None:
    ...
    if status == subscription_models.FraudCheckStatus.OK and given_recredit:
        logger.info("Recredited user %s with bonus credit", user.id)
        transactional_mails.send_bonification_granted_email(user)
    elif (
        status == subscription_models.FraudCheckStatus.OK
        and not given_recredit
        and deposit_api.can_receive_bonus_credit(user)
    ):
        logger.error(
            "Failed to recredit user with bonus credit despite %s Quotient Familial fraud check",
            status,
        ...
        transactional_mails.send_bonification_declined_email(user)
