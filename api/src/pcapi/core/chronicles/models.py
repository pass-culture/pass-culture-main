...
class Chronicle(PcObject, Base, Model, DeactivableMixin):
    ...
    def productIdentifierName(self) -> str:
        if self.productIdentifierType == ChronicleProductIdentifierType.EAN:
            return "EAN"
        elif self.productIdentifierType == ChronicleProductIdentifierType.ALLOCINE_ID:
            return "ID Allociné"
        else:
            return "Visa"

    def get_product_identifier(self, product: "Product") -> str | None:
        if self.productIdentifierType == ChronicleProductIdentifierType.EAN:
            return product.ean
        elif self.productIdentifierType == ChronicleProductIdentifierType.ALLOCINE_ID:
