...
def import_book_club_chronicles() -> None:
    ...
    for form in typeform.get_responses_generator(
        lambda: _get_last_chronicle_date(models.ChronicleClubType.BOOK_CLUB), form_id
    ):
...
def import_cine_club_chronicles() -> None:
    ...
    for form in typeform.get_responses_generator(
        lambda: _get_last_chronicle_date(models.ChronicleClubType.CINE_CLUB), form_id
    ):
...
def _extract_cine_club_movie_id(answer: typeform.TypeformAnswer) -> str | None:
    if not answer.choice_id:
        return None
    if answer.text:
...
def save_chronicle(
    ...
    logger.info(
        f"Import chronicle for {club_name}: starting",
        extra={
    ...
    if chronicle_type_id == models.ChronicleProductIdentifierType.EAN:
        ...
        product_identifier = _extract_cine_club_movie_id(answer_dict.get(product_identifier_field, EMPTY_ANSWER))
        if product_identifier:
            products = (
    ...
    try:
        if all((content, product_identifier, form.email)):
            chronicle = models.Chronicle(
                age=age,
                city=(
                    answer_dict.get(club_constants.CITY_ID.value, EMPTY_ANSWER).text
            ...
            db.session.flush()
            logger.info(
                f"Import chronicle for {club_name}: success",
                extra={
                    "response_id": form.response_id,
                },
            )
            ...
            logger.info(
                f"Import chronicle for {club_name}: ignored",
                extra={
                    "response_id": form.response_id,
