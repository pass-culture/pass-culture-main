...
class BeneficiaryFraudCheck(PcObject, Model):
    ...
    reason = sa.orm.mapped_column(sa.Text, nullable=True)
    reasonCodes: sa_orm.Mapped[list[FraudReasonCode]] = sa.orm.mapped_column(
...
class OrphanDmsApplication(PcObject, Model):
    ...
    application_id: sa_orm.Mapped[int] = sa.orm.mapped_column(
        sa.BigInteger, primary_key=True
    )  # refers to DMS application "number"
    dateCreated = sa.orm.mapped_column(
    ...
    email = sa.orm.mapped_column(sa.Text, nullable=True, index=True)
    latest_modification_datetime = sa.orm.mapped_column(
        sa.DateTime, nullable=True
    )  # This field copies the value provided in the DMS application
    process_id = sa.orm.mapped_column(sa.BigInteger)


class BeneficiaryFraudReview(PcObject, Model):
    __tablename__ = "beneficiary_fraud_review"
    authorId: sa_orm.Mapped[int] = sa.orm.mapped_column(
        sa.BigInteger, sa.ForeignKey("user.id"), index=True, nullable=False
    )
    author: sa_orm.Mapped[users_models.User] = sa_orm.relationship(
        "User", foreign_keys=[authorId], backref="adminFraudReviews"
    )
    dateReviewed: sa_orm.Mapped[datetime.datetime] = sa.orm.mapped_column(
        sa.DateTime, nullable=False, server_default=sa.func.now()
    )
    eligibilityType: sa_orm.Mapped[users_models.EligibilityType | None] = sa.orm.mapped_column(
        sa.Enum(users_models.EligibilityType, create_constraint=False), nullable=True
    )
    reason = sa.orm.mapped_column(sa.Text)
    review = sa.orm.mapped_column(sa.Enum(FraudReviewStatus, create_constraint=False))
    userId: sa_orm.Mapped[int] = sa.orm.mapped_column(
        sa.BigInteger, sa.ForeignKey("user.id"), index=True, nullable=False
    )
...
class BlacklistedDomainName(PcObject, Model):
    ...
    domain: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.Text, nullable=False, unique=True)
    dateCreated: sa_orm.Mapped[datetime.datetime] = sa.orm.mapped_column(
...
class ProductWhitelist(PcObject, Model):
    ...
    ean: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.String(length=13), nullable=False, unique=True, index=True)
    dateCreated: sa_orm.Mapped[datetime.datetime] = sa.orm.mapped_column(
        sa.DateTime, nullable=False, default=datetime.datetime.utcnow, server_default=sa.func.now()
    )
    comment: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.Text, nullable=False)
    authorId: sa_orm.Mapped[int] = sa.orm.mapped_column(sa.BigInteger, sa.ForeignKey("user.id"), nullable=False)
