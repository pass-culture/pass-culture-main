...
class SuspendedUserActionHistoryFactory(ActionHistoryFactory):
    ...
    user = factory.SubFactory(
        ...
        isActive=False,
        dateCreated=datetime.datetime.now(datetime.timezone.utc) - relativedelta(days=2),
    )
    actionType = models.ActionType.USER_SUSPENDED
    actionDate = factory.LazyFunction(lambda: datetime.datetime.now(datetime.timezone.utc) - relativedelta(days=1))
