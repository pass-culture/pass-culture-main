...
# following line triggers a semgrep error although it should not since
# the template is rendered using Flask's render_template (which is the
# recommended fix...).
# nosemgrep
...
def to_html(schemas: typing.Collection[types.ModelSchema], dst: Path | None = None) -> str:
    ...
    with open(dst, mode="w") as f:
        content = flask.render_template(template, schemas=schemas)
        f.write(content)
