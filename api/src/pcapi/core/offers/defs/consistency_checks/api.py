...
from pcapi.core.offers.defs.diff import api as diff_api
...
def build_diff_status(model: models.Base, subcategory: subcategories.Subcategory) -> diff_types.SubcategoryDiffStatus:
    """Build fields difference between model and subcategory with details"""
...
def build_properties_diff_status() -> dict[str, diff_types.PropertiesDiff]:
    res = {}
    for subcategory in subcategories.ALL_SUBCATEGORIES:
        model = utils.MODELS[subcategory.id]
        res[subcategory.id] = diff_api.build_properties_diff(subcategory, model)
    return res
