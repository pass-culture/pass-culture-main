...
@blueprint.cli.command("check_all_properties_are_equal")
def check_all_properties_are_equal() -> None:
    """Check that all properties (is_digital/is_event) are equal"""
    status = api.build_properties_diff_status()

    errors_count = 0
    for subcategory_id, diff in status.items():
        print(f"{subcategory_id} -> {diff.kind}")
