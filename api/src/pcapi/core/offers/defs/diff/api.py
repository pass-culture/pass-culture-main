...
def build_model_and_subcategory_fields_diff_status(
    model: type[base.Base], subcategory: subcategories.Subcategory
) -> types.DiffStatus:
...
def build_typology_diff(subcategory: subcategories.Subcategory, model: type[base.Base]) -> types.TypologyDiff:
    ...
    """
    if subcategory.is_selectable and base.cannot_be_created(model):
        return "should_be_selectable"
    elif not subcategory.is_selectable and not base.cannot_be_created(model):
        return "should_not_be_selectable"

    if subcategory.is_digital_deposit and not base.is_digital(model):
        return "should_be_digital"
    elif not subcategory.is_digital_deposit and base.is_digital(model):
    ...
    if subcategory.is_event and not base.is_activity(model):
        return "should_be_activity"
    elif not subcategory.is_event and base.is_activity(model):
