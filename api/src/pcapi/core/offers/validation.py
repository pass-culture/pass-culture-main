...
def check_event_opening_hours_can_be_updated(
    ...
    * the new end date can't less than 48 hours from now (same as above)
    ...
    if opening_hours.isSoftDeleted:
        raise exceptions.EventOpeningHoursIsSoftDeleted()

    if current_end and current_end <= now:
        raise exceptions.EventOpeningHoursEndPassed()

    if new_start:
        assert new_start

        if new_start <= two_days_from_now:
            raise exceptions.EventOpeningHoursStartsTooSoon()
        ...
        if not new_end and new_start_date_after_current_end:
            raise exceptions.EventOpeningHoursNewStartAfterEnd()

    if new_end and new_end <= two_days_from_now:
        raise exceptions.EventOpeningHoursEndsTooSoon()
