...
def check_offer_extra_data(
    ...
    try:
        ...
        _check_isbn_or_ean_does_not_exists(extra_data, venue)
...
def _check_isbn_or_ean_does_not_exists(extra_data: models.OfferExtraData | None, venue: offerers_models.Venue) -> None:
    if not extra_data:
        return
    ...
    if not ean and not isbn:
        return
    if repository.find_active_offer_by_ean_or_isbn(ean, isbn, venue):
        if isbn:
