...
def update_collective_offer(
    ...
    updated_fields = _update_collective_offer(
        offer=offer_to_update, new_values=new_values, address_body=body.address, user=user
    )
...
def update_collective_offer_template(
    ...
    _update_collective_offer(offer=offer_to_update, new_values=new_values, address_body=body.address, user=user)
...
def _update_collective_offer(
    ...
    new_values: dict,
    address_body: offerers_schemas.AddressBodyModel | None,
    ...
    edit_oa = ("locationType" in new_values) or ("locationComment" in new_values) or ("address" in new_values)
    if edit_offer_venue and edit_oa:
    ...
    offerer_address_from_body = _get_offerer_address_from_address_body(address_body=address_body, venue=offer.venue)
    if offerer_address_from_body is not None:
        new_values["offererAddress"] = offerer_address_from_body
        new_values.pop("address", None)

    # receive OA fields -> also write to offerVenue to keep the field up to date
    if edit_oa:
        new_offer_venue, new_intervention_area = educational_api_offer.get_offer_venue_intervention_area(
            location_type=new_values.get("locationType"),
            location_comment=new_values.get("locationComment"),
            offerer_address=offerer_address_from_body,
            venue_id=offer.venueId,
        )

        if new_offer_venue is not None:
            new_values["offerVenue"] = new_offer_venue

        if new_intervention_area is not None:
            new_values["interventionArea"] = new_intervention_area
