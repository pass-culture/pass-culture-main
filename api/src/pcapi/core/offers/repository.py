...
def _get_offer_home_score(offer: models.Offer, date_limit: datetime.datetime) -> tuple[int, datetime.datetime | None]:
    ...
    if offer.isEvent:
        ...
        active_future_stocks = (
            stock for stock in offer.activeStocks if (stock.beginningDatetime and stock.beginningDatetime > date_limit)
        )
...
def get_offers_for_homepage(*, venue_id: int, offers_limit: int) -> list[models.Offer]:
    ...
    status_values = (
        ...
        offer_mixin.OfferStatus.SCHEDULED,
    ...
    sorted_offers = sorted(query, key=lambda offer: _get_offer_home_score(offer, date_limit=now))
    offers = sorted_offers[:offers_limit]
