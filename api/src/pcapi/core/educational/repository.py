...
def get_collective_offer_by_id_query(offer_id: int) -> sa_orm.Query:
    return (
        ...
        .options(*_get_collective_offer_address_joinedload(with_is_linked_to_venue=True))
    )
...
def get_collective_offer_by_id_for_adage(offer_id: int) -> educational_models.CollectiveOffer:
    query = (
        ...
        .options(
            ...
            *_get_collective_offer_address_joinedload(with_is_linked_to_venue=True),
        )
    )
...
def _get_collective_offer_template_address_joinedload() -> tuple[sa_orm.Load, ...]:
...
def _get_collective_offer_address_joinedload(with_is_linked_to_venue: bool = False) -> tuple[sa_orm.Load, ...]:
    """
    Use this when querying CollectiveOffer and you need to load its address
    When with_is_linked_to_venue is True, also load the isLinkedToVenue expression (this is not needed if you only need the Address fields)
    """

    result: tuple[sa_orm.Load, ...] = (
        sa_orm.joinedload(educational_models.CollectiveOffer.offererAddress).joinedload(
            offerers_models.OffererAddress.address
        ),
    )

    if with_is_linked_to_venue:
        result = (
            *result,
