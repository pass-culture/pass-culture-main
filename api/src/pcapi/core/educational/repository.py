        ...
        .options(
            ...
            sa_orm.joinedload(models.CollectiveOffer.venue, innerjoin=True).options(
                sa_orm.load_only(
                    ...
                    typing.cast(sa_orm.QueryableAttribute[str], offerers_models.Venue.street),
...
) -> tuple[list[sa.Row[tuple[str, int, str, str, str, str, str]]], int]:
...
def field_to_venue_timezone(field: sa_orm.InstrumentedAttribute) -> sa.Function[sa.Date]:
    # TODO(OA) - use Venue.offererAddress.address.timezone when the virtual venues are migrated
    return sa.func.timezone(Venue.timezone, sa.func.timezone("UTC", field))
...
def _get_collective_offer_template_address_joinedload_with_expression() -> tuple[sa_orm.interfaces.LoaderOption, ...]:
    """
