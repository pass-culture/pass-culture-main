...
class HasImageMixin:
    ...
    imageId = sa.orm.mapped_column(sa.Text, nullable=True)
    imageCrop: sa_orm.Mapped[dict | None] = sa.orm.mapped_column(
        sa_mutable.MutableDict.as_mutable(postgresql.json.JSONB), nullable=True
    )
    imageCredit = sa.orm.mapped_column(sa.Text, nullable=True)
    # Whether or not we also stored the original image in the storage bucket.
    imageHasOriginal = sa.orm.mapped_column(sa.Boolean, nullable=True)
...
class ValidationRuleCollectiveOfferLink(PcObject, models.Model):
    ...
    ruleId: sa_orm.Mapped[int] = sa.orm.mapped_column(
        sa.BigInteger, sa.ForeignKey("offer_validation_rule.id", ondelete="CASCADE"), nullable=False
    )
    collectiveOfferId: sa_orm.Mapped[int] = sa.orm.mapped_column(
...
class ValidationRuleCollectiveOfferTemplateLink(PcObject, models.Model):
    ...
    collectiveOfferTemplateId: sa_orm.Mapped[int] = sa.orm.mapped_column(
...
class CollectiveOfferTemplateDomain(models.Model):
    ...
    educationalDomainId: sa_orm.Mapped[int] = sa.orm.mapped_column(
...
class CollectiveOffer(
    ...
    isActive: sa_orm.Mapped[bool] = sa.orm.mapped_column(
    ...
    bookingEmails: sa_orm.Mapped[list[str]] = sa.orm.mapped_column(
        sa_mutable.MutableList.as_mutable(postgresql.ARRAY(sa.String)),
        nullable=False,
        server_default="{}",
    )

    description: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.Text, nullable=False, server_default="", default="")

    durationMinutes = sa.orm.mapped_column(sa.Integer, nullable=True)

    dateCreated: sa_orm.Mapped[datetime.datetime] = sa.orm.mapped_column(
        sa.DateTime, nullable=False, default=datetime.datetime.utcnow
    )

    dateArchived: sa_orm.Mapped[datetime.datetime | None] = sa.orm.mapped_column(sa.DateTime, nullable=True)

    dateUpdated: sa_orm.Mapped[datetime.datetime] = sa.orm.mapped_column(
        sa.DateTime, nullable=True, default=datetime.datetime.utcnow, onupdate=datetime.datetime.utcnow
    )

    students: sa_orm.Mapped[list[StudentLevels]] = sa.orm.mapped_column(
    ...
    contactEmail: sa_orm.Mapped[str | None] = sa.orm.mapped_column(sa.String(120), nullable=True)

    contactPhone: sa_orm.Mapped[str | None] = sa.orm.mapped_column(sa.Text, nullable=True)
    ...
    # place (filled when 3.)
    offerVenue: sa_orm.Mapped[OfferVenueDictStr] = sa.orm.mapped_column(
        sa_mutable.MutableDict.as_mutable(postgresql.json.JSONB), nullable=False
    )

    interventionArea: sa_orm.Mapped[list[str]] = sa.orm.mapped_column(
    ...
    institutionId = sa.orm.mapped_column(
    ...
    templateId: sa_orm.Mapped[int | None] = sa.orm.mapped_column(
    ...
    teacherId: sa_orm.Mapped[int | None] = sa.orm.mapped_column(
        sa.BigInteger,
    ...
    providerId: sa_orm.Mapped[int | None] = sa.orm.mapped_column(
        sa.BigInteger,
    ...
    formats: sa_orm.Mapped[list[EacFormat]] = sa.orm.mapped_column(
        postgresql.ARRAY(sa.Enum(EacFormat, create_constraint=False, native_enum=False)), nullable=False
    )

    rejectionReason: sa_orm.Mapped[CollectiveOfferRejectionReason | None] = sa.orm.mapped_column(
    ...
    locationType: sa_orm.Mapped[CollectiveLocationType | None] = sa.orm.mapped_column(
    ...
    locationComment: sa_orm.Mapped[str | None] = sa.orm.mapped_column(sa.Text(), nullable=True)
    ...
    nationalProgramId: sa_orm.Mapped[int | None] = sa.orm.mapped_column(
        sa.BigInteger,
...
class CollectiveOfferTemplateEducationalRedactor(PcObject, models.Model):
    ...
    educationalRedactorId: sa_orm.Mapped[int] = sa.orm.mapped_column(
...
class CollectiveOfferTemplate(
    ...
    priceDetail = sa.orm.mapped_column(sa.Text, nullable=True)
    ...
    dateRange: sa_orm.Mapped[DateTimeRange] = sa.orm.mapped_column(postgresql.TSRANGE, nullable=True)
    ...
    contactEmail: sa_orm.Mapped[str | None] = sa.orm.mapped_column(sa.String(120), nullable=True)
    contactPhone: sa_orm.Mapped[str | None] = sa.orm.mapped_column(sa.Text, nullable=True)
    contactUrl: sa_orm.Mapped[str | None] = sa.orm.mapped_column(sa.Text, nullable=True)
    contactForm: sa_orm.Mapped[OfferContactFormEnum | None] = sa.orm.mapped_column(
...
class CollectiveStock(PcObject, models.Model):
    ...
    dateModified: sa_orm.Mapped[datetime.datetime] = sa.orm.mapped_column(
        sa.DateTime, nullable=False, default=datetime.datetime.utcnow, onupdate=datetime.datetime.utcnow
    )

    startDatetime: sa_orm.Mapped[datetime.datetime] = sa.orm.mapped_column(sa.DateTime, nullable=False)
    endDatetime: sa_orm.Mapped[datetime.datetime] = sa.orm.mapped_column(sa.DateTime, nullable=False)
    ...
    bookingLimitDatetime: sa_orm.Mapped[datetime.datetime] = sa.orm.mapped_column(sa.DateTime, nullable=False)

    numberOfTickets: sa_orm.Mapped[int] = sa.orm.mapped_column(sa.Integer, nullable=False)
...
class EducationalInstitution(PcObject, models.Model):
    ...
    institutionId: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.String(30), nullable=False, unique=True, index=True)

    institutionType: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.String(80), nullable=False)
    ...
    postalCode: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.String(10), nullable=False)

    email = sa.orm.mapped_column(sa.Text(), nullable=True)

    phoneNumber: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.String(30), nullable=False)
    ...
    ruralLevel: sa_orm.Mapped[InstitutionRuralLevel] = sa.orm.mapped_column(
...
class EducationalYear(PcObject, models.Model):
    ...
    adageId: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.String(30), unique=True, nullable=False)

    beginningDate: sa_orm.Mapped[datetime.datetime] = sa.orm.mapped_column(sa.DateTime, nullable=False)

    expirationDate: sa_orm.Mapped[datetime.datetime] = sa.orm.mapped_column(sa.DateTime, nullable=False)
...
class EducationalDeposit(PcObject, models.Model):
    ...
    educationalInstitutionId: sa_orm.Mapped[int] = sa.orm.mapped_column(
    ...
    educationalYearId: sa_orm.Mapped[str] = sa.orm.mapped_column(
    ...
    creditRatio: sa_orm.Mapped[decimal.Decimal | None] = sa.orm.mapped_column(
        sa.Numeric(10, 3),
        sa.CheckConstraint(
    ...
    isFinal: sa_orm.Mapped[bool] = sa.orm.mapped_column(sa.Boolean, nullable=False, default=True)

    ministry = sa.orm.mapped_column(
        sa.Enum(Ministry),
        nullable=True,
    )
...
class EducationalRedactor(PcObject, models.Model):
    ...
    email: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.String(120), nullable=False, unique=True, index=True)

    firstName = sa.orm.mapped_column(sa.String(128), nullable=True)

    lastName = sa.orm.mapped_column(sa.String(128), nullable=True)

    civility = sa.orm.mapped_column(sa.String(20), nullable=True)
...
class CollectiveBooking(PcObject, models.Model):
    ...
    dateUsed = sa.orm.mapped_column(sa.DateTime, nullable=True, index=True)

    collectiveStockId: sa_orm.Mapped[int] = sa.orm.mapped_column(
    ...
    cancellationDate = sa.orm.mapped_column(sa.DateTime, nullable=True)
    ...
    cancellationLimitDate: sa_orm.Mapped[datetime.datetime] = sa.orm.mapped_column(sa.DateTime, nullable=False)
    ...
    status: sa_orm.Mapped[CollectiveBookingStatus] = sa.orm.mapped_column(
    ...
    confirmationDate = sa.orm.mapped_column(sa.DateTime, nullable=True)
    confirmationLimitDate: sa_orm.Mapped[datetime.datetime] = sa.orm.mapped_column(sa.DateTime, nullable=False)
...
class DomainToNationalProgram(PcObject, models.Model):
    ...
    domainId: sa_orm.Mapped[int] = sa.orm.mapped_column(
        sa.BigInteger, sa.ForeignKey("educational_domain.id", ondelete="CASCADE"), index=True, nullable=False
    )
    nationalProgramId: sa_orm.Mapped[int] = sa.orm.mapped_column(
...
class CollectiveDmsApplication(PcObject, models.Model):
    ...
    state: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.String(30), nullable=False)
    procedure: sa_orm.Mapped[int] = sa.orm.mapped_column(sa.BigInteger, nullable=False)
    application: sa_orm.Mapped[int] = sa.orm.mapped_column(sa.BigInteger, nullable=False, index=True)
    siret: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.String(14), nullable=False, index=True)
    lastChangeDate: sa_orm.Mapped[datetime.datetime] = sa.orm.mapped_column(sa.DateTime, nullable=False)
    depositDate = sa.orm.mapped_column(sa.DateTime, nullable=False)
    expirationDate = sa.orm.mapped_column(sa.DateTime, nullable=True)
    buildDate = sa.orm.mapped_column(sa.DateTime, nullable=True)
    instructionDate = sa.orm.mapped_column(sa.DateTime, nullable=True)
    processingDate = sa.orm.mapped_column(sa.DateTime, nullable=True)
    userDeletionDate = sa.orm.mapped_column(sa.DateTime, nullable=True)
...
class CollectiveOfferRequest(PcObject, models.Model):
    ...
    _phoneNumber: sa_orm.Mapped[str | None] = sa.orm.mapped_column(sa.String(30), nullable=True, name="phoneNumber")

    requestedDate: sa_orm.Mapped[datetime.date | None] = sa.orm.mapped_column(sa.Date, nullable=True)

    totalStudents: sa_orm.Mapped[int | None] = sa.orm.mapped_column(sa.Integer, nullable=True)

    totalTeachers: sa_orm.Mapped[int | None] = sa.orm.mapped_column(sa.Integer, nullable=True)

    comment: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.Text, nullable=False)

    dateCreated: sa_orm.Mapped[datetime.date] = sa.orm.mapped_column(
...
class NationalProgram(PcObject, models.Model):
    ...
    name: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.Text, unique=True, nullable=True)
    dateCreated: sa_orm.Mapped[datetime.datetime] = sa.orm.mapped_column(
        sa.DateTime, nullable=False, default=datetime.datetime.utcnow
    )
    domains: sa_orm.Mapped[list["EducationalDomain"]] = sa_orm.relationship(
...
class EducationalInstitutionProgramAssociation(models.Model):
    ...
    institutionId: sa_orm.Mapped[int] = sa.orm.mapped_column(
        sa.BigInteger,
    ...
    programId: sa_orm.Mapped[int] = sa.orm.mapped_column(
        sa.BigInteger,
    ...
    timespan: sa_orm.Mapped[DateTimeRange] = sa.orm.mapped_column(
...
class EducationalInstitutionProgram(PcObject, models.Model):
    ...
    # technical name
    name: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.Text, nullable=False, unique=True)
    ...
    description: sa_orm.Mapped[str | None] = sa.orm.mapped_column(sa.Text, nullable=True)


class CollectivePlaylist(PcObject, models.Model):
    __tablename__ = "collective_playlist"
    type: sa_orm.Mapped[str] = sa.orm.mapped_column(db_utils.MagicEnum(PlaylistType), nullable=False)
    distanceInKm: sa_orm.Mapped[float] = sa.orm.mapped_column(sa.Float, nullable=True)
...
class AdageVenueAddress(PcObject, models.Model):
    __tablename__ = "adage_venue_address"
    adageId: sa_orm.Mapped[str | None] = sa.orm.mapped_column(sa.Text, nullable=True, unique=True)
    adageInscriptionDate: sa_orm.Mapped[datetime.datetime | None] = sa.orm.mapped_column(sa.DateTime, nullable=True)
