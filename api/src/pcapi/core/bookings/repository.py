...
def _create_export_query(offer_id: int, event_beginning_date: date) -> BaseQuery:
    ...
    timezone_column: sa.sql.functions.Function = func.coalesce(Address.timezone, VenueAddress.timezone, Venue.timezone)
...
def _get_filtered_bookings_query(
    ...
    *,
    ...
    bookings_query = (
        ...
        .join(Booking.venue, isouter=True)  # why outer ??
        .outerjoin(Offer.offererAddress)
        .outerjoin(OffererAddress.address)
    ...
    timezone_column: sa.orm.Mapped[typing.Any] | sa.sql.functions.Function = func.coalesce(
        Address.timezone, VenueAddress.timezone, Venue.timezone
    )
