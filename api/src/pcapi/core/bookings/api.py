...
def _get_booking_options() -> sa_orm.Query:
    return (
...
def get_booking_by_id(user: users_models.User, booking_id: int) -> models.Booking:
    """
    Get a booking by id for a user, with all the data needed for the bookings page
    including the offer and venue data.
    """
    query = db.session.query(models.Booking).filter_by(userId=user.id, id=booking_id).options(_get_booking_options())
    return query
