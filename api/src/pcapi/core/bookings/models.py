...
class ExternalBooking(PcObject, Model):
    ...
    bookingId: sa_orm.Mapped[int] = sa.orm.mapped_column(
    ...
    barcode: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.String, nullable=False)

    seat = sa.orm.mapped_column(sa.String)

    additional_information: sa_orm.Mapped[dict | None] = sa.orm.mapped_column(postgresql.JSONB)
...
class Booking(PcObject, Model):
    ...
    dateUsed: sa_orm.Mapped[datetime | None] = sa.orm.mapped_column(sa.DateTime, nullable=True, index=True)

    stockId: sa_orm.Mapped[int] = sa.orm.mapped_column(
    ...
    quantity: sa_orm.Mapped[int] = sa.orm.mapped_column(sa.Integer, nullable=False, default=1)

    token: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.String(6), unique=True, nullable=False)
    ...
    amount: sa_orm.Mapped[Decimal] = sa.orm.mapped_column(sa.Numeric(10, 2), nullable=False)

    priceCategoryLabel: sa_orm.Mapped[str | None] = sa.orm.mapped_column(sa.Text, nullable=True)

    cancellationDate: sa_orm.Mapped[datetime | None] = sa.orm.mapped_column(sa.DateTime, nullable=True)

    displayAsEnded = sa.orm.mapped_column(sa.Boolean, nullable=True)

    cancellationLimitDate = sa.orm.mapped_column(sa.DateTime, nullable=True)

    cancellationReason = sa.orm.mapped_column(
        "cancellationReason",
        sa.Enum(
    ...
    cancellationUserId: sa_orm.Mapped[int | None] = sa.orm.mapped_column(
    ...
    status: sa_orm.Mapped[BookingStatus] = sa.orm.mapped_column(
        sa.Enum(BookingStatus), nullable=False, default=BookingStatus.CONFIRMED
    )
    sa.Index("ix_booking_status", status)

    validationAuthorType: sa_orm.Mapped[BookingValidationAuthorType] = sa.orm.mapped_column(
        sa.Enum(BookingValidationAuthorType), nullable=True
    )

    reimbursementDate = sa.orm.mapped_column(sa.DateTime, nullable=True)

    depositId = sa.orm.mapped_column(sa.BigInteger, sa.ForeignKey("deposit.id"), index=True, nullable=True)
    ...
    usedRecreditType: sa_orm.Mapped[BookingRecreditType] = sa.orm.mapped_column(
...
class FraudulentBookingTag(PcObject, Model):
    ...
    authorId = sa.orm.mapped_column(sa.BigInteger, sa.ForeignKey("user.id"), index=True, nullable=False)
