...
class Achievement(PcObject, Model):
    ...
    bookingId: sa_orm.Mapped[int] = sa.orm.mapped_column(sa.BigInteger, sa.ForeignKey("booking.id"), nullable=False)
    booking: sa_orm.Mapped["Booking"] = sa_orm.relationship("Booking", foreign_keys=[bookingId], backref="achievements")

    name: sa_orm.Mapped[AchievementEnum] = sa.orm.mapped_column(db_utils.MagicEnum(AchievementEnum), nullable=False)
    unlockedDate: sa_orm.Mapped[datetime] = sa.orm.mapped_column(
    ...
    seenDate: sa_orm.Mapped[datetime] = sa.orm.mapped_column(sa.DateTime, nullable=True)
