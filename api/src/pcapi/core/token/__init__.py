...
class AsymetricToken(AbstractToken):
    ...
    @classmethod
    def load_without_checking(cls, encoded_token: str, *args: typing.Any, **kwargs: typing.Any) -> "Token":
        print("JWT_PUBLIC_KEY", settings.JWT_PUBLIC_KEY)
        try:
            payload = utils.decode_jwt_token_rs256(
                encoded_token, public_key=settings.JWT_PUBLIC_KEY
        ...
        data = payload.get("data", {})
        return cls(type_, uuid, encoded_token, data)
    ...
    def create(
        ...
        encoded_token = utils.encode_jwt_payload_rs256(payload, private_key=settings.JWT_PRIVATE_KEY)
        if ttl is None or ttl > timedelta(0):
