...
def check_venue_edition(modifications: dict[str, typing.Any], venue: models.Venue) -> None:
    ...
    from . import api as offerers_api

    modifications_disability_compliance = [
        modifications.get("audioDisabilityCompliant"),
        modifications.get("mentalDisabilityCompliant", offerers_api.UNCHANGED),
        modifications.get("motorDisabilityCompliant", offerers_api.UNCHANGED),
        modifications.get("visualDisabilityCompliant", offerers_api.UNCHANGED),
    ]
    ...
    if (
        ...
        and offerers_api.UNCHANGED not in modifications_disability_compliance
    ):
        raise ApiErrors(errors={"global": ["L'accessibilité du lieu doit être définie."]})
