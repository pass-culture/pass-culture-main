...
class Venue(PcObject, Model, HasThumbMixin, AccessibilityMixin, SoftDeletableMixin):
    ...
    name: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.String(140), nullable=False)

    siret = sa.orm.mapped_column(sa.String(14), nullable=True, unique=True)

    departementCode = sa.orm.mapped_column(sa.String(3), nullable=True, index=True)

    latitude: sa_orm.Mapped[decimal.Decimal | None] = sa.orm.mapped_column(sa.Numeric(8, 5), nullable=True)

    longitude: sa_orm.Mapped[decimal.Decimal | None] = sa.orm.mapped_column(sa.Numeric(8, 5), nullable=True)

    venueProviders: sa_orm.Mapped[list["providers_models.VenueProvider"]] = sa_orm.relationship(
        "VenueProvider", back_populates="venue"
    )

    managingOffererId: sa_orm.Mapped[int] = sa.orm.mapped_column(
    ...
    bookingEmail = sa.orm.mapped_column(sa.String(120), nullable=True)
    sa.Index("idx_venue_bookingEmail", bookingEmail)

    _address = sa.orm.mapped_column("address", sa.String(200), nullable=True)

    _street = sa.orm.mapped_column("street", sa.Text(), nullable=True)

    postalCode = sa.orm.mapped_column(sa.String(6), nullable=True)

    city = sa.orm.mapped_column(sa.String(50), nullable=True)
    ...
    banId = sa.orm.mapped_column(sa.Text(), nullable=True)

    timezone: sa_orm.Mapped[str] = sa.orm.mapped_column(
        sa.String(50), nullable=False, default=METROPOLE_TIMEZONE, server_default=METROPOLE_TIMEZONE
    )

    publicName = sa.orm.mapped_column(sa.String(255), nullable=True)

    isVirtual: sa_orm.Mapped[bool] = sa.orm.mapped_column(
        sa.Boolean,
        nullable=False,
        default=False,
    ...
    isPermanent: sa_orm.Mapped[bool] = sa.orm.mapped_column(sa.Boolean, nullable=False, default=False)

    isOpenToPublic = sa.orm.mapped_column(sa.Boolean, nullable=False, default=False)

    comment = sa.orm.mapped_column(
        sa.TEXT,
        sa.CheckConstraint(
    ...
    venueTypeCode: sa_orm.Mapped[VenueTypeCode] = sa.orm.mapped_column(
        sa.Enum(VenueTypeCode, create_constraint=False), nullable=False, default=VenueTypeCode.OTHER
    )

    venueLabelId = sa.orm.mapped_column(sa.Integer, sa.ForeignKey("venue_label.id"), nullable=True)

    venueLabel: sa_orm.Mapped["VenueLabel"] = sa_orm.relationship("VenueLabel", foreign_keys=[venueLabelId])

    dateCreated: sa_orm.Mapped[datetime] = sa.orm.mapped_column(sa.DateTime, nullable=False, default=datetime.utcnow)

    withdrawalDetails = sa.orm.mapped_column(sa.Text, nullable=True)

    description = sa.orm.mapped_column(sa.Text, nullable=True)
    ...
    _bannerUrl = sa.orm.mapped_column(sa.Text, nullable=True, name="bannerUrl")
    ...
    _bannerMeta: sa_orm.Mapped[dict | None] = sa.orm.mapped_column(
        MutableDict.as_mutable(JSONB), nullable=True, name="bannerMeta"
    )

    adageId = sa.orm.mapped_column(sa.Text, nullable=True)
    adageInscriptionDate = sa.orm.mapped_column(sa.DateTime, nullable=True)
    ...
    dmsToken: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.Text, nullable=False, unique=True)

    venueEducationalStatusId = sa.orm.mapped_column(
    ...
    collectiveDescription = sa.orm.mapped_column(sa.Text, nullable=True)
    collectiveStudents: sa_orm.Mapped[list[educational_models.StudentLevels] | None] = sa.orm.mapped_column(
        MutableList.as_mutable(sa.dialects.postgresql.ARRAY(sa.Enum(educational_models.StudentLevels))),
        nullable=True,
        server_default="{}",
    )
    collectiveWebsite = sa.orm.mapped_column(sa.Text, nullable=True)
    ...
    collectiveInterventionArea: sa_orm.Mapped[list[str] | None] = sa.orm.mapped_column(
        MutableList.as_mutable(sa.dialects.postgresql.json.JSONB), nullable=True
    )
    collectiveNetwork: sa_orm.Mapped[list[str] | None] = sa.orm.mapped_column(
        MutableList.as_mutable(sa.dialects.postgresql.json.JSONB), nullable=True
    )
    collectiveAccessInformation = sa.orm.mapped_column(sa.Text, nullable=True)
    collectivePhone = sa.orm.mapped_column(sa.Text, nullable=True)
    collectiveEmail = sa.orm.mapped_column(sa.Text, nullable=True)
...
class GooglePlacesInfo(PcObject, Model):
    ...
    venueId: sa_orm.Mapped[int] = sa.orm.mapped_column(
    ...
    placeId: sa_orm.Mapped[str | None] = sa.orm.mapped_column(sa.Text, nullable=True, unique=False)
    bannerUrl: sa_orm.Mapped[str | None] = sa.orm.mapped_column(sa.Text, nullable=True, name="bannerUrl")
    bannerMeta: sa_orm.Mapped[dict | None] = sa.orm.mapped_column(MutableDict.as_mutable(JSONB), nullable=True)
    updateDate: sa_orm.Mapped[datetime] = sa.orm.mapped_column(
...
class OpeningHours(PcObject, Model):
    ...
    offerId: sa_orm.Mapped[int] = sa.orm.mapped_column(
...
class VenueLabel(PcObject, Model):
    ...
    id: sa_orm.Mapped[int] = sa.orm.mapped_column(sa.Integer, primary_key=True, autoincrement=True)
    label: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.String(100), nullable=False)
...
class VenueContact(PcObject, Model):
    ...
    email = sa.orm.mapped_column(sa.String(256), nullable=True)

    website = sa.orm.mapped_column(sa.String(256), nullable=True)

    phone_number = sa.orm.mapped_column(sa.String(64), nullable=True)

    social_medias: sa_orm.Mapped[dict] = sa.orm.mapped_column(
...
class VenuePricingPointLink(Model):
    ...
    pricingPointId: sa_orm.Mapped[int] = sa.orm.mapped_column(
        sa.BigInteger, sa.ForeignKey("venue.id"), index=True, nullable=False
    )
    ...
    # is set to the Epoch.
    timespan: sa_orm.Mapped[psycopg2.extras.DateTimeRange] = sa.orm.mapped_column(sa_psql.TSRANGE, nullable=False)
...
class VenueBankAccountLink(PcObject, Model):
    ...
    bankAccountId: sa_orm.Mapped[int] = sa.orm.mapped_column(
...
class VenueEducationalStatus(Model):
    ...
    id: sa_orm.Mapped[int] = sa.orm.mapped_column(sa.BigInteger, primary_key=True, autoincrement=False, nullable=False)
    name: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.String(256), nullable=False)
...
class VenueRegistration(PcObject, Model):
    ...
    id: sa_orm.Mapped[int] = sa.orm.mapped_column("id", sa.BigInteger, sa.Identity(), primary_key=True)
    ...
    target: sa_orm.Mapped[Target] = sa.orm.mapped_column(db_utils.MagicEnum(Target), nullable=False)

    webPresence: sa_orm.Mapped[str | None] = sa.orm.mapped_column(sa.Text, nullable=True)


class OffererTagMapping(PcObject, Model):
    __tablename__ = "offerer_tag_mapping"

    offererId: sa_orm.Mapped[int] = sa.orm.mapped_column(
        sa.BigInteger, sa.ForeignKey("offerer.id", ondelete="CASCADE"), index=True, nullable=False
    )
    tagId: sa_orm.Mapped[int] = sa.orm.mapped_column(
...
class Offerer(
    ...
    siren: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.String(9), nullable=False, unique=True)

    dateValidated = sa.orm.mapped_column(sa.DateTime, nullable=True, default=None)
    ...
    allowedOnAdage: sa_orm.Mapped[bool] = sa.orm.mapped_column(
...
class UserOfferer(PcObject, Model, ValidationStatusMixin):
    ...
    userId: sa_orm.Mapped[int] = sa.orm.mapped_column(sa.BigInteger, sa.ForeignKey("user.id"), primary_key=True)
    ...
    dateCreated: sa_orm.Mapped[datetime] = sa.orm.mapped_column(sa.DateTime, nullable=True, default=datetime.utcnow)
...
class ApiKey(PcObject, Model):
    ...
    providerId: sa_orm.Mapped[int] = sa.orm.mapped_column(
    ...
    dateCreated: sa_orm.Mapped[datetime] = sa.orm.mapped_column(
        sa.DateTime, nullable=False, default=datetime.utcnow, server_default=sa.func.now()
    )
    prefix = sa.orm.mapped_column(sa.Text, nullable=True, unique=True)
    secret: sa_orm.Mapped[bytes] = sa.orm.mapped_column(LargeBinary, nullable=True)
...
class OffererTagCategoryMapping(PcObject, Model):
    ...
    categoryId: sa_orm.Mapped[int] = sa.orm.mapped_column(
...
class OffererTag(PcObject, Model):
    ...
    label: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.String(140), nullable=True)
    description: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.Text, nullable=True)
...
class OffererInvitation(PcObject, Model):
    ...
    email: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.Text, nullable=False)
    dateCreated: sa_orm.Mapped[datetime] = sa.orm.mapped_column(sa.DateTime, nullable=False, default=datetime.utcnow)
    ...
    status: sa_orm.Mapped[InvitationStatus] = sa.orm.mapped_column(db_utils.MagicEnum(InvitationStatus), nullable=False)
...
class IndividualOffererSubscription(PcObject, Model):
    ...
    isEmailSent: sa_orm.Mapped[bool] = sa.orm.mapped_column(
        sa.Boolean, nullable=False, server_default=sa.sql.expression.false(), default=False
    )
    dateEmailSent: sa_orm.Mapped[date | None] = sa.orm.mapped_column(sa.Date, nullable=True)
    dateReminderEmailSent: sa_orm.Mapped[date | None] = sa.orm.mapped_column(sa.Date, nullable=True)

    isCriminalRecordReceived: sa_orm.Mapped[bool] = sa.orm.mapped_column(
        sa.Boolean, nullable=False, server_default=sa.sql.expression.false(), default=False
    )
    dateCriminalRecordReceived: sa_orm.Mapped[date | None] = sa.orm.mapped_column(sa.Date, nullable=True)

    isCertificateReceived: sa_orm.Mapped[bool] = sa.orm.mapped_column(
        sa.Boolean, nullable=False, server_default=sa.sql.expression.false(), default=False
    )
    certificateDetails: sa_orm.Mapped[str | None] = sa.orm.mapped_column(sa.Text, nullable=True)
    isExperienceReceived: sa_orm.Mapped[bool] = sa.orm.mapped_column(
        sa.Boolean, nullable=False, server_default=sa.sql.expression.false(), default=False
    )
    experienceDetails: sa_orm.Mapped[str | None] = sa.orm.mapped_column(sa.Text, nullable=True)
    has1yrExperience: sa_orm.Mapped[bool] = sa.orm.mapped_column(
        sa.Boolean, nullable=False, server_default=sa.sql.expression.false(), default=False
    )
    has5yrExperience: sa_orm.Mapped[bool] = sa.orm.mapped_column(
        sa.Boolean, nullable=False, server_default=sa.sql.expression.false(), default=False
    )
    isCertificateValid: sa_orm.Mapped[bool] = sa.orm.mapped_column(
        sa.Boolean, nullable=False, server_default=sa.sql.expression.false(), default=False
    )
...
class OffererStats(PcObject, Model):
    ...
    syncDate: sa_orm.Mapped[datetime] = sa.orm.mapped_column(sa.DateTime, nullable=False)
    table: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.String(120), nullable=False)
    jsonData: sa_orm.Mapped[dict] = sa.orm.mapped_column(  # serialized from `OffererStatsData`
        "jsonData",
...
class OffererAddress(PcObject, Model):
    ...
    label: sa_orm.Mapped[str | None] = sa.orm.mapped_column(sa.Text(), nullable=True)
    addressId: sa_orm.Mapped[int] = sa.orm.mapped_column(
...
class OffererConfidenceRule(PcObject, Model):
    ...
    offererId = sa.orm.mapped_column(
    ...
    confidenceLevel: sa_orm.Mapped[OffererConfidenceLevel] = sa.orm.mapped_column(
...
class NonPaymentNotice(PcObject, Model):
    ...
    amount: sa_orm.Mapped[decimal.Decimal] = sa.orm.mapped_column(sa.Numeric(10, 2), nullable=False)
    batchId = sa.orm.mapped_column(sa.BigInteger, sa.ForeignKey("cashflow_batch.id"), nullable=True, index=True)
    batch: sa_orm.Mapped["finance_models.CashflowBatch"] = sa_orm.relationship("CashflowBatch", foreign_keys=[batchId])
    dateReceived = sa.orm.mapped_column(sa.Date, nullable=False, server_default=sa.func.current_date())
    dateCreated = sa.orm.mapped_column(sa.DateTime, nullable=False, default=datetime.utcnow)
    emitterName = sa.orm.mapped_column(sa.Text(), nullable=False)
    emitterEmail = sa.orm.mapped_column(sa.Text(), nullable=False)
    reference: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.Text(), nullable=False)
    noticeType: sa_orm.Mapped[NoticeType] = sa.orm.mapped_column(db_utils.MagicEnum(NoticeType), nullable=False)
    status: sa_orm.Mapped[NoticeStatus] = sa.orm.mapped_column(
        db_utils.MagicEnum(NoticeStatus),
        nullable=False,
        server_default=NoticeStatus.CREATED.value,
        default=NoticeStatus.CREATED,
    )
    motivation: sa_orm.Mapped[NoticeStatusMotivation] = sa.orm.mapped_column(
