...
def get_venues_stats(venue_ids: tuple[int]) -> dict[str, int | decimal.Decimal | None]:
    ...
    try:
        if individual_bookings := clickhouse_queries.CountIndividualBookingQuery().execute({"venue_ids": venue_ids}):
            stats["individual_bookings"] = individual_bookings[0].quantity
        if collective_booking := clickhouse_queries.CountCollectiveBookingQuery().execute({"venue_ids": venue_ids}):
            stats["collective_booking"] = collective_booking[0].quantity
        if offers_data := clickhouse_queries.CountOffersQuery().execute({"venue_ids": venue_ids}):
            stats |= offers_data[0].dict()
        revenues = clickhouse_queries.AggregatedTotalRevenueQuery().execute({"venue_ids": venue_ids})
