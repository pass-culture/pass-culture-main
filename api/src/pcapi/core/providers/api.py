...
from pcapi.celery_tasks.batch_updates_statuses_tasks import UpdateVenueOffersActiveStatusPayload
from pcapi.celery_tasks.batch_updates_statuses_tasks import update_venue_collective_offers_active_status_task
from pcapi.celery_tasks.batch_updates_statuses_tasks import update_venue_offers_active_status_task
...
def delete_venue_provider(
    ...
    if FeatureToggle.WIP_ASYNCHRONOUS_CELERY_BATCH_UPDATE_STATUSES.is_active():
        payload = UpdateVenueOffersActiveStatusPayload(
            is_active=False,
            venue_id=venue_provider.venueId,
            provider_id=venue_provider.providerId,
        )
        on_commit(functools.partial(update_venue_offers_active_status_task.delay, payload.model_dump()))
    else:
        on_commit(
            functools.partial(
...
def disable_offers_linked_to_provider(provider_id: int, current_user: typing.Any) -> None:
    ...
    for venue_provider in venue_providers:
        ...
        if FeatureToggle.WIP_ASYNCHRONOUS_CELERY_BATCH_UPDATE_STATUSES.is_active():
            on_commit(functools.partial(update_venue_collective_offers_active_status_task.delay, payload.model_dump()))
        else:
            on_commit(
                functools.partial(
