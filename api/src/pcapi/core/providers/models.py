...
class Provider(PcObject, Model, DeactivableMixin):
    ...
    name: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.String(90), index=True, nullable=False)

    localClass = sa.orm.mapped_column(sa.String(60), nullable=True, unique=True)

    # bool meant for hiding custom provider (for instance, FNAC provider) in the pro Interface (if set to False)
    enabledForPro: sa_orm.Mapped[bool] = sa.orm.mapped_column(
        sa.Boolean, nullable=False, default=False, server_default=sa_sql.expression.false()
    )

    enableParallelSynchronization: sa_orm.Mapped[bool] = sa.orm.mapped_column(
        sa.Boolean, nullable=False, default=False, server_default=sa_sql.expression.false()
    )
...
class VenueProviderExternalUrls(PcObject, Model, DeactivableMixin):
    ...
    venueProviderId: sa_orm.Mapped[int] = sa.orm.mapped_column(
...
class VenueProvider(PcObject, Model, DeactivableMixin):
    ...
    venueId: sa_orm.Mapped[int] = sa.orm.mapped_column(sa.BigInteger, sa.ForeignKey("venue.id"), nullable=False)
    ...
    venueIdAtOfferProvider: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.String(70), nullable=True)
    ...
    dateCreated: sa_orm.Mapped[datetime.datetime] = sa.orm.mapped_column(
        sa.DateTime, nullable=False, server_default=sa.func.now()
    )

    lastSyncDate = sa.orm.mapped_column(sa.DateTime, nullable=True)
...
class CinemaProviderPivot(PcObject, Model):
    ...
    venueId = sa.orm.mapped_column(sa.BigInteger, sa.ForeignKey("venue.id"), index=False, nullable=False, unique=True)

    venue: sa_orm.Mapped["Venue | None"] = sa_orm.relationship(
        Venue, foreign_keys=[venueId], backref="cinemaProviderPivot", uselist=False
    )

    providerId: sa_orm.Mapped[int] = sa.orm.mapped_column(sa.BigInteger, sa.ForeignKey("provider.id"), nullable=False)

    provider: sa_orm.Mapped["Provider"] = sa_orm.relationship(
        "Provider", foreign_keys=[providerId], backref="cinemaProviderPivots"
    )

    idAtProvider: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.Text, nullable=False)
...
class CDSCinemaDetails(PcObject, Model):
    ...
    cinemaProviderPivotId = sa.orm.mapped_column(
    ...
    cinemaApiToken: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.Text, nullable=False)

    accountId: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.Text, nullable=False)
...
class AllocineVenueProvider(VenueProvider):
    ...
    id: sa_orm.Mapped[int] = sa.orm.mapped_column(sa.BigInteger, sa.ForeignKey("venue_provider.id"), primary_key=True)
    isDuo: sa_orm.Mapped[bool] = sa.orm.mapped_column(
        sa.Boolean, default=True, server_default=sa.true(), nullable=False
    )
    quantity = sa.orm.mapped_column(sa.Integer, nullable=True)
    internalId: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.Text, nullable=False, unique=True)
    price: sa_orm.Mapped[decimal.Decimal] = sa.orm.mapped_column(
...
class AllocinePivot(PcObject, Model):
    ...
    theaterId: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.String(20), nullable=False, unique=True)
...
class LocalProviderEvent(PcObject, Model):
    ...
    date: sa_orm.Mapped[datetime.datetime] = sa.orm.mapped_column(
        sa.DateTime, nullable=False, default=datetime.datetime.utcnow
    )
    type: sa_orm.Mapped[LocalProviderEventType] = sa.orm.mapped_column(sa.Enum(LocalProviderEventType), nullable=False)
    payload = sa.orm.mapped_column(sa.String(50), nullable=True)
...
class BoostCinemaDetails(PcObject, Model):
    ...
    cinemaUrl: sa_orm.Mapped[str] = sa.orm.mapped_column(
        sa.Text, nullable=False
    )  # including http:// or https:// and trailing /
    token: sa_orm.Mapped[str | None] = sa.orm.mapped_column(sa.Text, nullable=True)
    tokenExpirationDate: sa_orm.Mapped[datetime.datetime | None] = sa.orm.mapped_column(sa.DateTime, nullable=True)
...
class CGRCinemaDetails(PcObject, Model):
    ...
    cinemaUrl: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.Text, nullable=False)
    numCinema: sa_orm.Mapped[int] = sa.orm.mapped_column(sa.Integer, nullable=True)
    password: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.Text, nullable=False)
...
class EMSCinemaDetails(PcObject, Model):
    ...
    lastVersion: sa_orm.Mapped[int] = sa.orm.mapped_column(sa.BigInteger, default=0, nullable=False)
