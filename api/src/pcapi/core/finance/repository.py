...
def _get_sent_pricings_for_collective_bookings(
    ...
    columns: tuple[sa.sql.elements.Label, ...] = (
        ...
        # TODO bdalbianco 06/06/2025 CLEAN_OA enlever le coalesce pour ne garder que la venue apres la regul
        # bulle check if collective a vraiment venue comme ça
        # sa_func.coalesce(
        #     geography_models.Address.street,
        #     # educational_models.collectiveOffer.offererAddress.Address.street
        #     # educational_models.collectiveOffer.offerVenue.
        #     # geography_models.Venue.offererAddress.address.street,
        #     # offerers_models.Venue.OffererAddress.Address.street,
        ...
        #     offerers_models.Offerer.departementCode,
        # ).label("venue_departement_code"),
...
def _get_sent_pricings_for_individual_bookings(
    ...
    # TODO bdalbianco 06/06/2025 CLEAN_OA enlever le coalesce pour ne garder que venue/offer apres la regul
    sub = sa.select(
    ...
    # columns.extend(
    #     [
    #         sa_func.coalesce(sub_venue.c.street, offerers_models.Offerer.street).label("address_street"),
    #         sa_func.coalesce(sub_venue.c.postalCode, offerers_models.Offerer.postalCode).label("address_postal_code"),
    #         sa_func.coalesce(sub_venue.c.city, offerers_models.Offerer.city).label("address_city"),
    #     ]
    # #
...
def _get_collective_booking_reimbursement_data(query: BaseQuery) -> list[tuple]:
    return (
        ...
        .with_entities(
            ...
            # sa.case(
            #     (geography_models.Address.postalCode.is_not(None), geography_models.Address.street),
            #     else_=offerers_models.Offerer.street,
            # ).label("venue_address"),
            # sa.case(
            #     (geography_models.Address.postalCode.is_not(None), geography_models.Address.postalCode),
            #     else_=offerers_models.Offerer.postalCode,
            # ).label("venue_postal_code"),
            # sa.case(
            #     (geography_models.Address.postalCode.is_not(None), geography_models.Address.city),
