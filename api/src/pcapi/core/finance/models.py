...
class Deposit(PcObject, Model):
    ...
    source: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.String(300), nullable=False)

    dateCreated: sa_orm.Mapped[datetime.datetime] = sa.orm.mapped_column(
        sa.DateTime, nullable=False, server_default=sa.func.now()
    )

    dateUpdated = sa.orm.mapped_column(sa.DateTime, nullable=True, onupdate=sa.func.now())

    expirationDate = sa.orm.mapped_column(sa.DateTime, nullable=True)

    version: sa_orm.Mapped[int] = sa.orm.mapped_column(sa.SmallInteger, nullable=False)

    type: sa_orm.Mapped[DepositType] = sa.orm.mapped_column(
...
class Recredit(PcObject, Model):
    ...
    depositId: sa_orm.Mapped[int] = sa.orm.mapped_column(sa.BigInteger, sa.ForeignKey("deposit.id"), nullable=False)
    ...
    recreditType: sa_orm.Mapped[RecreditType] = sa.orm.mapped_column(
...
class BankAccount(PcObject, Model, DeactivableMixin):
    ...
    iban: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.String(27), nullable=False)
    bic: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.String(11), nullable=False)
    dsApplicationId: sa_orm.Mapped[int | None] = sa.orm.mapped_column(sa.BigInteger, nullable=True, unique=True)
    status: sa_orm.Mapped[BankAccountApplicationStatus] = sa.orm.mapped_column(
        db_utils.MagicEnum(BankAccountApplicationStatus), nullable=False
    )
    dateCreated: sa_orm.Mapped[datetime.datetime] = sa.orm.mapped_column(
        sa.DateTime, nullable=False, server_default=sa.func.now()
    )
    dateLastStatusUpdate: sa_orm.Mapped[datetime.datetime] = sa.orm.mapped_column(sa.DateTime, nullable=True)
    ...
    lastCegidSyncDate = sa.orm.mapped_column(sa.DateTime, nullable=True)
...
class FinanceEvent(PcObject, Model):
    __tablename__ = "finance_event"
    creationDate: sa_orm.Mapped[datetime.datetime] = sa.orm.mapped_column(
        sa.DateTime, nullable=False, server_default=sa.func.now()
    )
    ...
    # avoid a JOIN.
    valueDate: sa_orm.Mapped[datetime.datetime] = sa.orm.mapped_column(sa.DateTime, nullable=False)
    # The date that is used to price events in a determined, stable order.
    pricingOrderingDate: sa_orm.Mapped[datetime.datetime | None] = sa.orm.mapped_column(sa.DateTime, nullable=True)

    status: sa_orm.Mapped[FinanceEventStatus] = sa.orm.mapped_column(
        db_utils.MagicEnum(FinanceEventStatus), index=True, nullable=False
    )
    motive: sa_orm.Mapped[FinanceEventMotive] = sa.orm.mapped_column(
        db_utils.MagicEnum(FinanceEventMotive), nullable=False
    )

    bookingId = sa.orm.mapped_column(sa.BigInteger, sa.ForeignKey("booking.id"), index=True, nullable=True)
    ...
    collectiveBookingId = sa.orm.mapped_column(
    ...
    bookingFinanceIncidentId = sa.orm.mapped_column(
    ...
    venueId = sa.orm.mapped_column(sa.BigInteger, sa.ForeignKey("venue.id"), index=True, nullable=True)
    ...
    pricingPointId = sa.orm.mapped_column(sa.BigInteger, sa.ForeignKey("venue.id"), index=True, nullable=True)
...
class CashflowPricing(Model):
    ...
    cashflowId: sa_orm.Mapped[int] = sa.orm.mapped_column(
        sa.BigInteger, sa.ForeignKey("cashflow.id"), index=True, primary_key=True
    )
    pricingId: sa_orm.Mapped[int] = sa.orm.mapped_column(
        sa.BigInteger, sa.ForeignKey("pricing.id"), index=True, primary_key=True
    )


class Pricing(PcObject, Model):
    __tablename__ = "pricing"
    status: sa_orm.Mapped[PricingStatus] = sa.orm.mapped_column(
    ...
    eventId = sa.orm.mapped_column(sa.BigInteger, sa.ForeignKey("finance_event.id"), index=True, nullable=False)
    event: sa_orm.Mapped[FinanceEvent] = sa_orm.relationship("FinanceEvent", foreign_keys=[eventId], backref="pricings")

    venueId = sa.orm.mapped_column(sa.BigInteger, sa.ForeignKey("venue.id"), index=True, nullable=False)
    venue: sa_orm.Mapped["offerers_models.Venue"] = sa_orm.relationship("Venue", foreign_keys=[venueId])

    pricingPointId = sa.orm.mapped_column(sa.BigInteger, sa.ForeignKey("venue.id"), index=True, nullable=False)
    ...
    # again and again.
    amount: sa_orm.Mapped[int] = sa.orm.mapped_column(sa.Integer, nullable=False)
    # See constraints below about the relationship between rate,
    # standardRule and customRuleId.
    standardRule: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.Text, nullable=False)
    customRuleId = sa.orm.mapped_column(
    ...
    revenue: sa_orm.Mapped[int] = sa.orm.mapped_column(sa.Integer, nullable=False)
...
class PricingLine(PcObject, Model):
    ...
    pricingId = sa.orm.mapped_column(sa.BigInteger, sa.ForeignKey("pricing.id"), index=True, nullable=True)
    pricing: sa_orm.Mapped[Pricing] = sa_orm.relationship("Pricing", foreign_keys=[pricingId], back_populates="lines")

    # See the note about `amount` at the beginning of this module.
    amount: sa_orm.Mapped[int] = sa.orm.mapped_column(sa.Integer, nullable=False)

    category: sa_orm.Mapped[PricingLineCategory] = sa.orm.mapped_column(
        db_utils.MagicEnum(PricingLineCategory), nullable=False
    )


class PricingLog(PcObject, Model):
    ...
    timestamp: sa_orm.Mapped[datetime.datetime] = sa.orm.mapped_column(
        sa.DateTime, nullable=False, server_default=sa.func.now()
    )
    statusBefore: sa_orm.Mapped[PricingStatus] = sa.orm.mapped_column(db_utils.MagicEnum(PricingStatus), nullable=False)
    statusAfter: sa_orm.Mapped[PricingStatus] = sa.orm.mapped_column(db_utils.MagicEnum(PricingStatus), nullable=False)
    reason: sa_orm.Mapped[PricingLogReason] = sa.orm.mapped_column(db_utils.MagicEnum(PricingLogReason), nullable=False)
...
class CustomReimbursementRule(PcObject, ReimbursementRule, Model):
    ...
    offerId = sa.orm.mapped_column(sa.BigInteger, sa.ForeignKey("offer.id"), nullable=True)
    ...
    venueId = sa.orm.mapped_column(sa.BigInteger, sa.ForeignKey("venue.id"), nullable=True)
    ...
    offererId = sa.orm.mapped_column(sa.BigInteger, sa.ForeignKey("offerer.id"), nullable=True)
    ...
    # offerer.
    subcategories: sa_orm.Mapped[list[str]] = sa.orm.mapped_column(
        sa_psql.ARRAY(sa.Text()), server_default="{}", nullable=True
    )

    # The amount of the reimbursement, or NULL if `rate` is set.
    amount: sa_orm.Mapped[int] = sa.orm.mapped_column(sa.Integer, nullable=True)
    # rate is between 0 and 1 (included), or NULL if `amount` is set.
    rate: sa_orm.Mapped[decimal.Decimal] = sa.orm.mapped_column(sa.Numeric(5, 4), nullable=True)
    ...
    timespan: sa_orm.Mapped[psycopg2.extras.DateTimeRange] = sa.orm.mapped_column(sa_psql.TSRANGE, nullable=True)
...
class InvoiceCashflow(Model):
    ...
    invoiceId: sa_orm.Mapped[int] = sa.orm.mapped_column(
...
class Cashflow(PcObject, Model):
    ...
    status: sa_orm.Mapped[CashflowStatus] = sa.orm.mapped_column(
    ...
    batchId: sa_orm.Mapped[int] = sa.orm.mapped_column(
...
class CashflowLog(PcObject, Model):
    ...
    statusBefore: sa_orm.Mapped[CashflowStatus] = sa.orm.mapped_column(
        db_utils.MagicEnum(CashflowStatus), nullable=False
    )
    statusAfter: sa_orm.Mapped[CashflowStatus] = sa.orm.mapped_column(
        db_utils.MagicEnum(CashflowStatus), nullable=False
    )
    details: sa_orm.Mapped[dict | None] = sa.orm.mapped_column(
        sa_mutable.MutableDict.as_mutable(sa_psql.JSONB), nullable=True, default={}, server_default="{}"
    )


class CashflowBatch(PcObject, Model):
    ...
    cutoff: sa_orm.Mapped[datetime.datetime] = sa.orm.mapped_column(sa.DateTime, nullable=False, unique=True)
    label: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.Text, nullable=False, unique=True)


class InvoiceLine(PcObject, Model):
    __tablename__ = "invoice_line"
    invoiceId: sa_orm.Mapped[int] = sa.orm.mapped_column(
        sa.BigInteger, sa.ForeignKey("invoice.id"), index=True, nullable=False
    )
    invoice: sa_orm.Mapped["Invoice"] = sa_orm.relationship("Invoice", foreign_keys=[invoiceId], back_populates="lines")
    label: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.Text, nullable=False)
    # a group is a dict of label and position, as defined in ..conf.InvoiceLineGroup
    group: sa_orm.Mapped[dict] = sa.orm.mapped_column(sa_psql.JSONB, nullable=False)
    contributionAmount: sa_orm.Mapped[int] = sa.orm.mapped_column(sa.Integer, nullable=False)
    reimbursedAmount: sa_orm.Mapped[int] = sa.orm.mapped_column(sa.Integer, nullable=False)
    rate: sa_orm.Mapped[decimal.Decimal] = sa.orm.mapped_column(
...
class Invoice(PcObject, Model):
    ...
    reference: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.Text, nullable=False, unique=True)
    bankAccountId = sa.orm.mapped_column(sa.BigInteger, sa.ForeignKey("bank_account.id"), index=True, nullable=True)
    bankAccount: sa_orm.Mapped[BankAccount] = sa_orm.relationship("BankAccount", foreign_keys=[bankAccountId])
    # See the note about `amount` at the beginning of this module.
    amount: sa_orm.Mapped[int] = sa.orm.mapped_column(sa.Integer, nullable=False)
    token: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.Text, unique=True, nullable=False)
    ...
    status: sa_orm.Mapped[InvoiceStatus] = sa.orm.mapped_column(db_utils.MagicEnum(InvoiceStatus), nullable=False)
...
class Payment(PcObject, Model):
    ...
    amount: sa_orm.Mapped[decimal.Decimal] = sa.orm.mapped_column(sa.Numeric(10, 2), nullable=False)
    reimbursementRule = sa.orm.mapped_column(sa.String(200))
    reimbursementRate = sa.orm.mapped_column(sa.Numeric(10, 2))
    customReimbursementRuleId = sa.orm.mapped_column(
        sa.BigInteger,
    ...
    recipientName: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.String(140), nullable=False)
    recipientSiren: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.String(9), nullable=False)
    iban = sa.orm.mapped_column(sa.String(27), nullable=True)
    bic = sa.orm.mapped_column(
        sa.String(11),
        sa.CheckConstraint(
    ...
    comment = sa.orm.mapped_column(sa.Text, nullable=True)
    author: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.String(27), nullable=False)
    transactionEndToEndId: sa_orm.Mapped[UUID] = sa.orm.mapped_column(sa_psql.UUID(as_uuid=True), nullable=True)
    transactionLabel = sa.orm.mapped_column(sa.String(140), nullable=True)
    paymentMessageId = sa.orm.mapped_column(sa.BigInteger, sa.ForeignKey("payment_message.id"), nullable=True)
    ...
    batchDate = sa.orm.mapped_column(sa.DateTime, nullable=True, index=True)
...
class PaymentStatus(PcObject, Model):
    ...
    paymentId: sa_orm.Mapped[int] = sa.orm.mapped_column(
    ...
    status: sa_orm.Mapped[TransactionStatus] = sa.orm.mapped_column(sa.Enum(TransactionStatus), nullable=False)
    detail = sa.orm.mapped_column(sa.VARCHAR(), nullable=True)
...
class PaymentMessage(PcObject, Model):
    ...
    name: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.String(50), unique=True, nullable=False)
    checksum: sa_orm.Mapped[bytes] = sa.orm.mapped_column(sa.LargeBinary(32), unique=True, nullable=False)
...
class FinanceIncident(PcObject, Model):
    ...
    kind: sa_orm.Mapped[IncidentType] = sa.orm.mapped_column(
        sa.Enum(IncidentType, native_enum=False, create_contraint=False),
        nullable=False,
    )
    status: sa_orm.Mapped[IncidentStatus] = sa.orm.mapped_column(
    ...
    venueId = sa.orm.mapped_column(sa.BigInteger, sa.ForeignKey("venue.id"), nullable=False)
    venue: sa_orm.Mapped["offerers_models.Venue"] = sa_orm.relationship(
        "Venue", foreign_keys=[venueId], backref="finance_incidents"
    )

    details: sa_orm.Mapped[dict] = sa.orm.mapped_column(
        sa_mutable.MutableDict.as_mutable(sa_psql.JSONB), nullable=False, default={}, server_default="{}"
    )

    forceDebitNote: sa_orm.Mapped[bool] = sa.orm.mapped_column(
        sa.Boolean, nullable=False, server_default="false", default=False
    )

    zendeskId: sa_orm.Mapped[int | None] = sa.orm.mapped_column(sa.BigInteger, nullable=True, index=True)

    origin: sa_orm.Mapped[FinanceIncidentRequestOrigin] = sa.orm.mapped_column(
        db_utils.MagicEnum(FinanceIncidentRequestOrigin),
        nullable=False,
        index=True,
    )

    comment: sa_orm.Mapped[str | None] = sa.orm.mapped_column(sa.Text, nullable=True)
...
class BookingFinanceIncident(PcObject, Model):
    ...
    incidentId = sa.orm.mapped_column(sa.BigInteger, sa.ForeignKey("finance_incident.id"), index=True, nullable=False)
    ...
    beneficiaryId: sa_orm.Mapped[int] = sa.orm.mapped_column(
    ...
    newTotalAmount: sa_orm.Mapped[int] = sa.orm.mapped_column(sa.Integer, nullable=False)  # in cents
