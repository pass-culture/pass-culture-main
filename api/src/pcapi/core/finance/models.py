...
class Settlement(PcObject, Model):
    ...
    settlementDate: sa_orm.Mapped[datetime.datetime] = sa_orm.mapped_column(sa.DateTime, nullable=False)
    dateImported: sa_orm.Mapped[datetime.datetime] = sa_orm.mapped_column(
        sa.DateTime, nullable=False, server_default=sa.func.now()
    )
    dateRejected: sa_orm.Mapped[datetime.datetime] = sa_orm.mapped_column(sa.DateTime, nullable=True)

    cegidSettlementId: sa_orm.Mapped[str] = sa_orm.mapped_column(sa.Text, nullable=False)
    bankAccountId = sa_orm.mapped_column(sa.BigInteger, sa.ForeignKey("bank_account.id"), index=True)
    bankAccount: sa_orm.Mapped[BankAccount] = sa_orm.relationship("BankAccount", foreign_keys=[bankAccountId])
    # See the note about `amount` at the beginning of this module.
    amount: sa_orm.Mapped[int] = sa_orm.mapped_column(sa.Integer, nullable=False)
