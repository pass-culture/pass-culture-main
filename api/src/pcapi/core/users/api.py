...
def notify_pro_users_before_anonymization() -> None:
    ...
    """
    if not FeatureToggle.WIP_ENABLE_PRO_NOTIFICATION_BEFORE_ANONYMIZATION.is_active():
        return

    almost_three_years_ago = datetime.date.today() - relativedelta(years=3, days=-30)

    users = (
        _get_anonymize_pro_query(
            sa.or_(
        ...
        .options(
            sa_orm.contains_eager(models.User.UserOfferers)
            .load_only(offerers_models.UserOfferer.id)
            .contains_eager(offerers_models.UserOfferer.offerer)
    ...
    for user in users:
        offerer = user.UserOfferers[0].offerer if user.UserOfferers else None
        transactional_mails.send_pre_anonymization_email_to_pro(user, offerer)
