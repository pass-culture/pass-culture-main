...
def get_user_is_eligible_for_bonification(user: models.User) -> bool:
    return get_user_bonification_status(user) not in (
        subscription_models.BonificationStatus.TOO_MANY_RETRIES,
        subscription_models.BonificationStatus.STARTED,
        subscription_models.BonificationStatus.GRANTED,
    )


def get_user_bonification_status(user: models.User) -> subscription_models.BonificationStatus:
    qf_bonus_credit_fraud_checks = sorted(
        [e for e in user.beneficiaryFraudChecks if e.type == subscription_models.FraudCheckType.QF_BONUS_CREDIT],
        key=lambda f: f.dateCreated,
        reverse=True,
    )
    ...
    if bonus_fraud_check_status in (
        ...
        return subscription_models.BonificationStatus.STARTED

    if bonus_fraud_check_status == subscription_models.FraudCheckStatus.OK:
        return subscription_models.BonificationStatus.GRANTED
    ...
    if not is_18_years_old or not has_v3_credit:
        return subscription_models.BonificationStatus.NOT_ELIGIBLE

    if is_18_years_old and has_v3_credit and has_never_completely_tried:
        return subscription_models.BonificationStatus.ELIGIBLE
    ...
    if bonus_fraud_check_status == subscription_models.FraudCheckStatus.KO:
        ...
        if len(qf_bonus_credit_fraud_checks) >= constants.MAX_QF_BONUS_RETRIES:
            return subscription_models.BonificationStatus.TOO_MANY_RETRIES

        if subscription_models.FraudReasonCode.NOT_IN_TAX_HOUSEHOLD in reason_codes:
            return subscription_models.BonificationStatus.NOT_IN_TAX_HOUSEHOLD

        if subscription_models.FraudReasonCode.QUOTIENT_FAMILIAL_TOO_HIGH in reason_codes:
            return subscription_models.BonificationStatus.QUOTIENT_FAMILIAL_TOO_HIGH

        if subscription_models.FraudReasonCode.CUSTODIAN_NOT_FOUND in reason_codes:
            return subscription_models.BonificationStatus.CUSTODIAN_NOT_FOUND

    return subscription_models.BonificationStatus.KO
