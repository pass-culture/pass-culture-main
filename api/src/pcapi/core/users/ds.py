...
def _reject_user_request_with_duplicates(user_request: users_models.UserAccountUpdateRequest, data: dict) -> None:
    ...
    if data.get("newEmail"):
        if data["oldEmail"] == data["newEmail"]:
            motivation = "Dossier rejeté car l'email est en doublon"
        ...
        if user_request.user and user_request.user.phoneNumber == data["newPhoneNumber"]:
            motivation = "Dossier rejeté car le numéro de téléphone est sur ton compte"
        ...
        motivation = "Dossier rejeté car les nom et prénom sont en doublon"
