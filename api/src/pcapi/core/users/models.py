...
class User(PcObject, Base, Model, DeactivableMixin):
    ...
    def received_pass_18_v3(self) -> bool:
        ...
        if not self.received_pass_17_18:
            return False
        recredits: list[Recredit] = sum([deposit.recredits for deposit in self.deposits], [])
        return any(recredit.recreditType == RecreditType.RECREDIT_18 for recredit in recredits)
