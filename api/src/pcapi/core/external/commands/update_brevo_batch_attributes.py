...
from pcapi.core.external.batch.api import UserUpdateData
from pcapi.core.external.batch.api import update_users_attributes
        ...
        .filter(
            ...
            sa.or_(
                sa.and_(
                    ...
                    # sa.not_(User.has_admin_role), #TODO bulle replace by backoffice_profile?
                ),
                User.is_beneficiary,
            ),
        )
...
def _format_batch_users(users_attributes: list[CollectedAttributes]) -> list[UserUpdateData]:
    ...
    for user_attributes in users_attributes:
        attributes = batch_attributes.format_user_attributes(user_attributes.attributes)
