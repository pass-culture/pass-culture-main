...
def send_confirmation_email_change_email(user: User, confirmation_link: str, cancellation_link: str) -> None:
...
def send_email_update_cancellation_email(user: User) -> None:
...
def send_validation_email_change_email(user: User, new_email: str, confirmation_link: str) -> None:
    data = get_validation_email_change_data(user.firstName, confirmation_link)
    mails.send(recipients=[new_email], data=data)
...
def send_email_change_information_email(user: User) -> None:
    data = get_email_change_information_data(user.firstName)
    mails.send(recipients=[user.email], data=data)


def send_pro_confirmation_email_change_email(new_email: str, confirmation_link: str) -> None:
...
def send_pro_information_email_change_email(user: User, new_email: str) -> None:
