...
class SpecialEvent(PcObject, Model):
    ...
    externalId: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.Text(), index=True, unique=True, nullable=False)
    title: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.Text(), nullable=False)
    endImportDate: sa_orm.Mapped[date] = sa.orm.mapped_column(sa.Date, index=True, nullable=False)
    eventDate: sa_orm.Mapped[date] = sa.orm.mapped_column(
...
class SpecialEventQuestion(PcObject, Model):
    ...
    eventId: sa_orm.Mapped[int] = sa.orm.mapped_column(
...
class SpecialEventResponse(PcObject, Model):
    ...
    eventId: sa_orm.Mapped[int] = sa.orm.mapped_column(sa.BigInteger, sa.ForeignKey("special_event.id"), nullable=False)
    event: sa_orm.Mapped[SpecialEvent] = sa_orm.relationship(
        "SpecialEvent", foreign_keys=[eventId], backref=sa_orm.backref("responses")
    )
    externalId: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.Text(), index=True, unique=True, nullable=False)
    dateSubmitted: sa_orm.Mapped[datetime] = sa.orm.mapped_column(sa.DateTime, nullable=False)
    phoneNumber: sa_orm.Mapped[str | None] = sa.orm.mapped_column(sa.Text(), nullable=True)
    email: sa_orm.Mapped[str | None] = sa.orm.mapped_column(sa.Text(), nullable=True)
    status: sa_orm.Mapped[SpecialEventResponseStatus] = sa.orm.mapped_column(
...
class SpecialEventAnswer(PcObject, Model):
    ...
    responseId: sa_orm.Mapped[int] = sa.orm.mapped_column(
    ...
    questionId: sa_orm.Mapped[int] = sa.orm.mapped_column(
        sa.BigInteger, sa.ForeignKey("special_event_question.id"), index=True, nullable=False
    )
    text: sa_orm.Mapped[str] = sa.orm.mapped_column(sa.Text(), nullable=False)
