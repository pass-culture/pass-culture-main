...
def create_flask_app() -> Flask:
    ...
    def log_request_details(response: flask.wrappers.Response) -> flask.wrappers.Response:
        ...
        try:
            ...
            logger.warning(
                "g.public_api_log_request_details_extra was not available in log_request_details", exc_info=True
            )
        except Exception:
            logger.warning(
                "g.public_api_log_request_details_extra does not seem to contain a valid dict", exc_info=True
            )
...
# DO NOT USE THIS APP
# its only purpose is for flask run/shell/...
# You should use the dedicated app instead (pcapi.app:app or pcapi.backoffice:app)
