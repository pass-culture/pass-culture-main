...
class GetUserHistoryTest:
    ...
    def test_get_user_subscription_history_dms(self, client):
        ...
        assert len(data["UNDERAGE"]["idCheckHistory"]) == 3

        assert data["UNDERAGE"]["idCheckHistory"][0]["type"] == "profile_completion"
        assert data["UNDERAGE"]["idCheckHistory"][0]["dateCreated"]
        assert data["UNDERAGE"]["idCheckHistory"][0]["status"] == "ok"

        assert data["UNDERAGE"]["idCheckHistory"][1]["type"] == "dms"
        assert data["UNDERAGE"]["idCheckHistory"][1]["dateCreated"]
        assert data["UNDERAGE"]["idCheckHistory"][1]["thirdPartyId"] == "24680"
        assert data["UNDERAGE"]["idCheckHistory"][1]["status"] == "ok"
        assert data["UNDERAGE"]["idCheckHistory"][1]["reason"] is None
        assert data["UNDERAGE"]["idCheckHistory"][1]["sourceId"] == "13579"

        assert data["UNDERAGE"]["idCheckHistory"][2]["type"] == "honor_statement"
        assert data["UNDERAGE"]["idCheckHistory"][2]["dateCreated"]
        assert data["UNDERAGE"]["idCheckHistory"][2]["status"] == "ok"
        assert data["UNDERAGE"]["idCheckHistory"][2]["reason"] is None
        assert not data["UNDERAGE"]["idCheckHistory"][2]["reasonCodes"]
        assert not data["UNDERAGE"]["idCheckHistory"][2]["technicalDetails"]
