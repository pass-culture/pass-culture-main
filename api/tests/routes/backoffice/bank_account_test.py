...
class DownloadReimbursementDetailsTest(PostEndpointHelper):
    ...
    def test_download_reimbursement_details(self, is_use_offer_address_ff_active, authenticated_client):
        ...
        if is_use_offer_address_ff_active:
            with override_features(WIP_ENABLE_OFFER_ADDRESS=True):
        ...
        rows = list(reader)
        if is_use_offer_address_ff_active:
            assert rows[0]["Adresse de l'offre"] == booking.stock.offer.offererAddress.address.fullAddress
        else:
            assert rows[0]["Adresse du lieu"] == venue.offererAddress.address.fullAddress
