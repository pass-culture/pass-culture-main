...
class GetVenueTest(GetEndpointHelper):
    ...
    def test_get_venue_with_provider(self, authenticated_client):
        ...
        with assert_num_queries(self.expected_num_queries + 1):  # venue.venue_provider.isFromCinemaProvider
            response = authenticated_client.get(url_for(self.endpoint, venue_id=venue_id))
            assert response.status_code == 200
...
class UpdateVenueTest(PostEndpointHelper):
    ...
    def test_update_venue_becomes_permanent_should_not_call_match_acceslibre_task(
        ...
        assert response.status_code == 303
        match_acceslibre_task.assert_not_called()
...
class AddCinemaSessionsSynchronizeTaskPostTest(PostEndpointHelper):
    ...
    def test_add_cinema_sessions_synchronize_task(
        ...
        synchronize_cinema_sessions_task_mock.assert_called_with({"venue_provider_id": venue_provider.id})
