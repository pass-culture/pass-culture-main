...
class DeleteGdprUserExtractTest(PostEndpointHelper):
    ...
    def test_delete_gdpr_user_extract_id_gdpr_dont_exist(self, authenticated_client):
        response = self.post_to_endpoint(authenticated_client, gdpr_id=42)

        expected_url = url_for("backoffice_web.gdpr_extract.list_gdpr_user_data_extract", _external=True)
        assert response.location == expected_url

        response = authenticated_client.get(response.location)
        assert "Une erreur est survenue lors de la tentative de suppression de l'extrait." in html_parser.extract_alert(
            response.data
        )
