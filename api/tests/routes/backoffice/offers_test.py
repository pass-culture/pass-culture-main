...
class EditOfferTest(PostEndpointHelper):
    ...
    def test_update_offer_tags_from_htmx(self, mock_reindex_offer_ids, legit_user, authenticated_client, criteria):
        ...
        assert cells[next(i)] == (  # Actions
            "Voir le détail de l’offre Valider l'offre Mettre en attente Rejeter l'offre Mettre l'offre en pause Taguer / Pondérer"
        )
...
class RejectOfferTest(PostEndpointHelper):
    ...
    def test_reject_offer_with_htmx(self, legit_user, authenticated_client):
        ...
        assert cells[next(i)] == ""  # Checkbox
        assert cells[next(i)] == (  # Actions
            "Voir le détail de l’offre Valider l'offre Mettre en attente Rejeter l'offre Publier l'offre Taguer / Pondérer"
        )
        assert cells[next(i)] == ""  # Image
...
class BatchOfferPendingTest(PostEndpointHelper):
    ...
    def test_batch_pending_offers(self, legit_user, authenticated_client):
        ...
        for offer in offers:
            ...
            assert offer.lastValidationAuthor == None
