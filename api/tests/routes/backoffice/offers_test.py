...
class ListOffersTest(GetEndpointHelper):
    ...
    @pytest.mark.parametrize(
        "filtered_status,expected_offer_index,expected_status",
        [
            (cultural_outreach_models.CulturalOutreachStatus.PENDING, 0, "En attente"),
            (cultural_outreach_models.CulturalOutreachStatus.QUALIFIED, 1, "Qualifiée"),
            (cultural_outreach_models.CulturalOutreachStatus.DISQUALIFIED, 2, "Disqualifiée"),
        ],
    )
    def test_list_offers_with_cultural_outreach(
        self, authenticated_client, filtered_status, expected_offer_index, expected_status
    ):
        offers = [
            cultural_outreach_factories.ClaimedCulturalOutreachFactory(
                status=cultural_outreach_models.CulturalOutreachStatus.PENDING
            ).offer,
            cultural_outreach_factories.ClaimedCulturalOutreachFactory(
        ...
        query_args = {
            "search-0-search_field": "CULTURAL_OUTREACH",
            "search-0-operator": "IN",
            "search-0-cultural_outreach": filtered_status.name,
        }
        with assert_num_queries(self.expected_num_queries_with_results):
            response = authenticated_client.get(url_for(self.endpoint, **query_args))
            assert response.status_code == 200
        ...
        rows = html_parser.extract_table_rows(response.data)
        assert len(rows) == 1
        assert rows[0]["ID"] == str(offers[expected_offer_index].id)
...
class QualifyCulturalOutreachTest(PostEndpointHelper):
    ...
    def test_qualify_claimed_cultural_outreach(self, legit_user, authenticated_client):
        ...
        assert offer.culturalOutreach.claimedDatetime is not None
    ...
    def test_qualify_unclaimed_cultural_outreach(self, legit_user, authenticated_client):
        ...
        assert offer.culturalOutreach.status is cultural_outreach_models.CulturalOutreachStatus.QUALIFIED
        assert offer.culturalOutreach.claimedDatetime is None
...
class BatchQualifyCulturalOutreachTest(PostEndpointHelper):
    ...
    def test_batch_qualify_cultural_outreach(self, legit_user, authenticated_client):
        ...
        expected_queries += 1  # insert cultural outreach
        ...
        assert unclaimed_offer.culturalOutreach.claimedDatetime is None
