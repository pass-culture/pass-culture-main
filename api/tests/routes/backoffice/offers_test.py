...
class ListOffersTest(GetEndpointHelper):
    ...
    def test_list_offers_advanced_search_by_price_multiple_stock(self, authenticated_client):
        ...
        offers_factories.StockFactory(
            ...
            bookingLimitDatetime=datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(days=1),
        )
...
class GetOfferDetailsTest(GetEndpointHelper):
    ...
    def test_get_offer_details_with_one_expired_stock(self, legit_user, authenticated_client):
        ...
        expired_stock = offers_factories.EventStockFactory(
            offer=offer,
            beginningDatetime=datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(hours=1),
            price=6.66,
        )
    ...
    def test_get_offer_details_with_two_expired_stocks(self, legit_user, authenticated_client):
        ...
        expired_stock_1 = offers_factories.EventStockFactory(
            ...
            quantity=100,
            dnBookedQuantity=70,
            beginningDatetime=datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(hours=2),
        )
...
class EditOfferVenueTest(PostEndpointHelper):
    ...
    def _test_move_event(
        ...
        offers_factories.EventStockFactory(
            beginningDatetime=datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(days=7)
        )
    ...
    def test_cant_move_when_event_is_in_the_past(self, authenticated_client, venues_in_same_offerer):
        ...
        offers_factories.EventStockFactory.create_batch(
            2, offer=offer, beginningDatetime=datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(days=1)
        )
        offers_factories.EventStockFactory(
            offer=offer, beginningDatetime=datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(days=1)
        )
...
class EditOfferStockTest(PostEndpointHelper):
    ...
    def test_offer_stock_edit_used_booking(self, authenticated_client):
        ...
        later_booking = bookings_factories.UsedBookingFactory(
            ...
            stock__beginningDatetime=datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(hours=2),
        )
