    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                "co",
                {
                    f"Temps costaud - {datetime.datetime.strftime((datetime.datetime.utcnow() - datetime.timedelta(days=10)).date(), '%d/%m/%Y')}"
                    f" - {datetime.datetime.strftime((datetime.datetime.utcnow() + datetime.timedelta(days=10)).date(), '%d/%m/%Y')}",
                },
                3,
            ),
            (
                "fort",
                {
                    f"Temps Fôrt - {datetime.datetime.strftime((datetime.datetime.utcnow() - datetime.timedelta(days=10)).date(), '%d/%m/%Y')}"
                    f" - {datetime.datetime.strftime((datetime.datetime.utcnow() + datetime.timedelta(days=10)).date(), '%d/%m/%Y')}",
                },
                3,
            ),
