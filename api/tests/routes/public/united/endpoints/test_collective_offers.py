...
class CreateCollectiveOfferUnauthorizedTest(helpers.PublicApiUnauthorizedBaseTest):
    controller = create_collective_offer
...
class CreateCollectiveOfferNotFoundTest(helpers.PublicApiNotFoundBaseTest):
    controller = create_collective_offer
    ...
    def test_unknown_domains(self, api_client, payload):
        payload["domains"] = [-1]
        assert "domains" in self.assert_is_not_known(api_client, payload)
...
def assert_no_collective_offer_created(test_instance, api_client, payload, side_effect=None):
    mock_path = "pcapi.core.offerers.api.can_venue_create_educational_offer"
