...
class CollectiveOffersGetVenuesTest:
    def test_list_venues(self, client):
        ...
        response = client.with_explicit_token(offerers_factories.DEFAULT_CLEAR_API_KEY).get(
            url_for("public_api.v2_prefixed_public_api.list_venues")
        )
    ...
    def test_list_venues_anonymous_returns_401(self, client):
        ...
        # When
        response = client.get(url_for("public_api.v2_prefixed_public_api.list_venues"))
...
class GetOfferersVenuesTest:
    def test_get_offerers_venues(self, client):
        ...
        response = client.with_explicit_token(offerers_factories.DEFAULT_CLEAR_API_KEY).get(
            url_for("public_api.v2_prefixed_public_api.get_offerer_venues")
        )
    ...
    def test_filter_offerers_venues_by_siren(self, client):
        ...
        response = client.with_explicit_token(offerers_factories.DEFAULT_CLEAR_API_KEY).get(
            url_for("public_api.v2_prefixed_public_api.get_offerer_venues", siren=siren)
        )
    ...
    def test_filter_offerers_venues_by_siren_error(self, client):
        response = client.with_explicit_token(offerers_factories.DEFAULT_CLEAR_API_KEY).get(
            url_for("public_api.v2_prefixed_public_api.get_offerer_venues", siren="not a siren")
        )
        assert response.status_code == 400

    def test_unauthenticated_client(self, client):
        response = client.get(url_for("public_api.v2_prefixed_public_api.get_offerer_venues", siren="123456789"))
        assert response.status_code == 401
