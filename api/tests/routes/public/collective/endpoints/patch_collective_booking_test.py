...
class CancelCollectiveBookingTest(PublicAPIVenueEndpointHelper):
    ...
    def test_should_raise_404_because_has_no_access_to_venue(self):
        ...
        response = self.make_request(plain_api_key, {"booking_id": booking.id})
    ...
    def test_allowed_action(self, status):
        plain_api_key, venue_provider = self.setup_active_venue_provider()
    ...
    def test_cancel_unallowed_action(self, status):
        plain_api_key, venue_provider = self.setup_active_venue_provider()
        offer = educational_factories.create_collective_offer_by_status(status, venue=venue_provider.venue)
        [booking] = offer.collectiveStock.collectiveBookings
    ...
    @pytest.mark.parametrize(
        "factory,expected_json",
        [
            (
                educational_factories.CancelledWithBookingCollectiveOfferFactory,
                {"booking": "Impossible d'annuler cette réservation car le statut de l'offre est CANCELLED"},
            ),
            (
                educational_factories.EndedCollectiveOfferFactory,
                {"booking": "Impossible d'annuler cette réservation car le statut de l'offre est ENDED"},
            ),
            (
                educational_factories.ArchivedReimbursedCollectiveOfferFactory,
                {"booking": "Impossible d'annuler cette réservation car le statut de l'offre est ARCHIVED"},
            ),
        ],
    )
    def test_should_raise_403(self, factory, expected_json):
        plain_api_key, venue_provider = self.setup_active_venue_provider()
        offer = factory(venue=venue_provider.venue)
        [booking] = offer.collectiveStock.collectiveBookings
