...
class Returns200Test(PatchEventEndpointHelper):
    ...
    def test_should_update_an_event_created_on_the_pro_interface(self):
        plain_api_key, venue_provider = self.setup_active_venue_provider()
...
class Returns400Test(PatchEventEndpointHelper):
    ...
    def test_should_raise_400_because_extra_data_of_an_event_linked_to_a_product_cannot_be_changed(self):
        ...
        # a value that differs from the product's, so `extraData` lands in `updates` and the
        # check runs — resending the product's own values is a 200 (see `Returns200Test`)
        response = self.make_request(
            plain_api_key,
            {"event_id": event.id},
