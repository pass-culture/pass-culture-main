...
class GetProductByEanTest:
    ...
    def test_400_when_missing_venue_id(self, client):
        ...
        with testing.assert_num_queries(1):
            response = client.get(f"/public/offers/v1/products/ean?eans={product_offer.extraData['ean']}")
    ...
    def test_no_404_when_ean_not_found(self, client):
        ...
        client = client.with_explicit_token(offerers_factories.DEFAULT_CLEAR_API_KEY)
        with testing.assert_num_queries(5):
            response = client.get(f"/public/offers/v1/products/ean?eans=1234567890123&venueId={venue.id}")
    ...
    def test_400_when_eans_list_is_empty(self, client):
        ...
        client = client.with_explicit_token(offerers_factories.DEFAULT_CLEAR_API_KEY)
        with testing.assert_num_queries(1):
            response = client.get(f"/public/offers/v1/products/ean?venueId={venue.id}")
