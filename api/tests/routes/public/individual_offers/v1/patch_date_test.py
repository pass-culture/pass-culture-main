...
class PatchEventStockTest(PublicAPIVenueEndpointHelper):
    ...
    def test_update_all_fields_on_date_with_price(self, mocked_async_index_offer_ids, client, caplog):
        ...
        log_message = "Stock updated with beginning datetime updated"
        log = next(record for record in caplog.records if record.message == log_message)

        assert log.extra["api_key_id"]
        assert not log.extra["user_id"]

        changes = log.extra["changes"]

        assert changes["priceCategory"]["old"] != changes["priceCategory"]["new"]
        assert changes["priceCategory"]["new"].id == price_category.id

        assert changes["idAtProviders"]["old"] != changes["idAtProviders"]["new"]
        assert changes["idAtProviders"]["new"] == "hey you !"
