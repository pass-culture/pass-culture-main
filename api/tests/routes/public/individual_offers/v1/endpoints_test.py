...
class PostProductTest:
    ...
    def test_create_multiple_products(self, client):
        venue, _ = create_offerer_provider_linked_to_venue()
        product = offers_factories.ProductFactory(subcategoryId="LIVRE_PAPIER", extraData={"ean": "0123456789123"})
        ...
        assert not created_offers[2].isDuo
        assert created_offers[0].extraData == {"ean": "1234567891234", "musicType": "820", "musicSubType": "829"}
        assert created_offers[1].extraData == {"ean": "0123456789123", "author": "Victor Hugo"}
        assert created_offers[2].extraData == {"ean": "1234567890987", "musicType": "900", "musicSubType": "910"}
    ...
    def test_event_category_not_accepted(self, client):
        ...
        assert response.status_code == 400
        assert "productOffers.0.categoryRelatedFields" in response.json
        assert offers_models.Offer.query.first() is None
    ...
    def test_only_physical_music_is_allowed(self, client):
        ...
        assert response.status_code == 400
        assert "productOffers.0.categoryRelatedFields" in response.json
        assert "SUPPORT_PHYSIQUE_MUSIQUE" in response.json["productOffers.0.categoryRelatedFields"][0]
        assert offers_models.Offer.query.first() is None
