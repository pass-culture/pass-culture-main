...
class Returns200Test:
    ...
    def test_publication_filter(self, client: TestClient):
        ...
        # only offers with publicationDatetime between (now - 90 days) and now are received
        ...
        future_publication_offer = factories.EventOfferFactory(
            venue=venue, publicationDatetime=now + datetime.timedelta(days=10)
        )
        assert future_publication_offer.status == offer_mixin.OfferStatus.SCHEDULED
