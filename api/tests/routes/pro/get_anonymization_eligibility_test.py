...
class Returns200Test:
    @pytest.mark.parametrize(
        "is_only_pro,has_active_offerer,is_sole_user_with_ongoing_activities",
        [
            (True, True, False),  # eligible for anonymization
            (False, True, False),  # user has also non pro account
            (True, False, False),  # user has inactive offerer
    ...
    def test_returns_eligibility_status(
        ...
        mock_has_active_offerer.return_value = has_active_offerer
        ...
        assert response.json == {
            ...
            "hasActiveOfferer": has_active_offerer,
