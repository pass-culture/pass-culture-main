...
class ActivateFutureOffersTest:
    def test_activate_future_offers_and_notify_users_with_reminders(self, client):
        ...
        with patch(
            ...
            mock_notify_users_future_offer_activated.assert_has_calls(
                [call(offer_to_publish_3), call(offer_to_publish_2)], any_order=True
            )

        assert response.status_code == 202
        assert Offer.query.get(offer_to_publish_1.id).isActive
        assert Offer.query.get(offer_to_publish_2.id).isActive
    ...
    def test_deactivate_future_offers(self, client):
        ...
        assert response.status_code == 202
        assert not Offer.query.get(offer_published_1.id).isActive
        assert not Offer.query.get(offer_published_2.id).isActive
