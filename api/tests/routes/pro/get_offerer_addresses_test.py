...
class Return200Test:
    ...
    def test_get_offerer_addresses_success(self, client):
        ...
        response = client.with_session_auth(email=pro.email).get(f"/pro/offerers/{offerer.id}/addresses")
    ...
    def test_get_offerer_addresses_with_offers(self, client):
        ...
        with assert_num_queries(self.num_queries):
            response = client.get(f"/pro/offerers/{offerer_id}/addresses?onlyWithOffers=true")
            assert response.status_code == 200
            assert response.json == [
                {
                    "city": "Paris",
        ...
        with assert_num_queries(self.num_queries):
            response = response = client.get(f"/pro/offerers/{offerer_id}/addresses")
            assert response.status_code == 200
    ...
    def test_get_offerer_addresses_is_editable(self, client, linked_to_venue):
        ...
        with assert_num_queries(self.num_queries):
            response = client.get(f"/pro/offerers/{offerer_id}/addresses")
            assert response.status_code == 200
