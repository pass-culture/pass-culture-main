...
class Return200Test:
    ...
    def test_get_offers_stats(self, client):
        ...
        educational_factories.create_collective_offer_by_status(CollectiveOfferDisplayedStatus.REJECTED, venue=venue)
        educational_factories.create_collective_offer_template_by_status(
            CollectiveOfferDisplayedStatus.UNDER_REVIEW, venue=venue
        )
        educational_factories.create_collective_offer_template_by_status(
        ...
        educational_factories.create_collective_offer_template_by_status(
            CollectiveOfferDisplayedStatus.REJECTED, venue=venue
        )
        ...
        assert response.json == {
            ...
            "pendingEducationalOffers": 4,
        }

    def test_get_offers_stats_with_no_public_offers(self, client):
        user_offerer = offerers_factories.UserOffererFactory()
        venue = offerers_factories.VenueFactory(managingOfferer=user_offerer.offerer)
        ...
        client = client.with_session_auth(user_offerer.user.email)
        venue = user_offerer.offerer.managedVenues[0]
        with testing.assert_num_queries(self.num_queries):
            response = client.get(f"/venue/{venue.id}/offers-statistics")
            assert response.status_code == 200
    ...
    def test_get_offers_stats_with_no_collective_offers(self, client):
        user_offerer = offerers_factories.UserOffererFactory()
        venue = offerers_factories.VenueFactory(managingOfferer=user_offerer.offerer)
        for _ in range(2):
            offer = offers_factories.OfferFactory(venue=venue, validation=OfferValidationStatus.APPROVED)
    ...
    def test_get_offers_stats_with_no_offers(self, client):
        user_offerer = offerers_factories.UserOffererFactory()
        venue = offerers_factories.VenueFactory(managingOfferer=user_offerer.offerer)
        client = client.with_session_auth(user_offerer.user.email)
...
class TestReturn400:
    ...
    def test_get__offers_stats_returns_403_if_user_has_no_rights_on_venue(self, client):
    ...
    def test_get__offers_stats_returns_404_if_venue_is_not_found(self, check_user_has_access_to_offerer, client):
