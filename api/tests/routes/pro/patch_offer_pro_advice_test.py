...
class Returns400Test:
    ...
    @patch("pcapi.core.pro_advice.api.update_pro_advice")
    def test_update_pro_advice_raises_error(self, mock_update_pro_advice, client):
        mock_update_pro_advice.side_effect = pro_advice_exceptions.ProAdviceException(
            {"global": ["Impossible de modifier une recommandation sur cette offre"]}
        )
        user_offerer = offerers_factories.UserOffererFactory()
        venue = offerers_factories.VenueFactory(managingOfferer=user_offerer.offerer)
        ...
        assert response.status_code == 400
        assert response.json["global"] == ["Impossible de modifier une recommandation sur cette offre"]
