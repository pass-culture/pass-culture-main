...
class PostEventTest(PublicAPIVenueEndpointHelper):
    ...
    def test_event_minimal_body(self):
        ...
        assert created_offer.publicationDatetime == now_datetime_with_tz.replace(second=0, microsecond=0)
    ...
    def test_future_event(self):
        ...
        payload = self._get_base_payload(venue_provider.venueId)
        publication_date = datetime.now(UTC).replace(minute=0, second=0) + timedelta(days=30)
        ...
        assert created_offer.publicationDatetime == publication_date.replace(second=0, microsecond=0)
        assert created_offer.finalizationDatetime == now_datetime_with_tz
        assert not created_offer.bookingAllowedDatetime
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # errors on videoUrl
            (
                {"videoUrl": "https://www.youtube.com/shorts/watch?v=WXFgNutNTN8"},
                {
                    "videoUrl": [
