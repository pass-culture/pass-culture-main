...
from pcapi.routes.provider.collective.endpoints.offers import PATCH_NON_NULLABLE_FIELDS
...
class CollectiveOffersPublicPatchOfferTest(PublicAPIVenueEndpointHelper):
    ...
    def test_patch_offer(self):
        ...
        domain = factories.EducationalDomainFactory(nationalPrograms=[national_program])
        ...
        payload = {
            ...
            "location": {"type": models.CollectiveLocationType.SCHOOL.value},
        ...
        offer = db.session.query(models.CollectiveOffer).filter_by(id=stock.collectiveOffer.id).one()
    ...
    def test_non_nullable_field(self, field):
        plain_api_key, venue_provider = self.setup_active_venue_provider()
        offer = factories.PublishedCollectiveOfferFactory(venue=venue_provider.venue, provider=venue_provider.provider)
    ...
    def test_change_venue(self):
        ...
        offer = factories.PublishedCollectiveOfferFactory(venue=venue, provider=venue_provider.provider)
    ...
    def test_should_raise_400_because_booking_limit_is_after_start(self):
        ...
        offer = factories.CollectiveOfferFactory(venue=venue, provider=venue_provider.provider)
        stock = factories.CollectiveStockFactory(collectiveOffer=offer)
    ...
    def test_patch_offer_uai_and_institution_id(self):
        ...
        educational_institution = factories.EducationalInstitutionFactory(institutionId="UAI123")
        offer = factories.CollectiveOfferFactory(
            imageCredit="pouet", imageId="123456789", venue=venue, provider=venue_provider.provider
        )
        stock = factories.CollectiveStockFactory(collectiveOffer=offer)
    ...
    def test_patch_offer_invalid_offerer(self):
        ...
        offerers_factories.ApiKeyFactory(provider=venue_provider.provider)
        offer = factories.CollectiveOfferFactory(venue=venue, provider=provider_factories.ProviderFactory())
        stock = factories.CollectiveStockFactory(collectiveOffer=offer)
    ...
    def test_should_update_expired_booking(self, input_with_timezone):
        ...
        offer = factories.CollectiveOfferFactory(venue=venue_provider.venue, provider=venue_provider.provider)
    ...
    def test_different_educational_years(self):
        ...
        # 2022-2023
        factories.EducationalYearFactory(beginningDate=datetime(2022, 9, 1), expirationDate=datetime(2023, 8, 31))
        # 2023-2024
        factories.EducationalYearFactory(beginningDate=datetime(2023, 9, 1), expirationDate=datetime(2024, 8, 31))
        # offer on year 2022-2023
        offer = factories.CollectiveOfferFactory(venue=venue_provider.venue, provider=venue_provider.provider)
        stock = factories.CollectiveStockFactory(collectiveOffer=offer, startDatetime=datetime(2022, 10, 5))
    ...
    def test_end_before_stock_start(self, with_timezone):
        ...
        factories.CollectiveStockFactory(collectiveOffer=offer, startDatetime=start)
    ...
    def test_start_after_stock_end(self, with_timezone):
        ...
        factories.CollectiveStockFactory(collectiveOffer=offer, startDatetime=end - timedelta(days=1), endDatetime=end)
    ...
    def test_patch_offer_update_to_school_location(self):
        key, venue_provider = self.setup_active_venue_provider()
        venue = offerers_factories.VenueFactory(venueProviders=[venue_provider])
        collective_offer = factories.PublishedCollectiveOfferFactory(venue=venue, provider=venue_provider.provider)
        ...
        assert collective_offer.locationType == models.CollectiveLocationType.SCHOOL
    ...
    def test_patch_offer_update_to_to_be_defined_location(self):
        ...
        db.session.refresh(collective_offer)
        assert collective_offer.locationType == models.CollectiveLocationType.TO_BE_DEFINED
...
class UpdatePriceTest(PublicAPIVenueEndpointHelper):
    ...
    def test_update_price_no_booking(self):
        key, venue_provider = self.setup_active_venue_provider()
        offer = factories.CollectiveOfferFactory(venue=venue_provider.venue, provider=venue_provider.provider)
        stock = factories.CollectiveStockFactory(collectiveOffer=offer, price=200)
    ...
    def test_update_price_confirmed_booking_lower(self):
        ...
        offer = factories.CollectiveOfferFactory(venue=venue_provider.venue, provider=venue_provider.provider)
        stock = factories.CollectiveStockFactory(collectiveOffer=offer, price=200)
        factories.ConfirmedCollectiveBookingFactory(collectiveStock=stock)
...
class AllowedActionsTest(PublicAPIVenueEndpointHelper):
    ...
    @time_machine.travel(time_travel_str)
    @pytest.mark.parametrize("status", set(testing.STATUSES_ALLOWING_EDIT_DATES) - STATUSES_NOT_IN_PUBLIC_API)
    ...
    @pytest.mark.parametrize("status", set(testing.STATUSES_NOT_ALLOWING_EDIT_DATES) - STATUSES_NOT_IN_PUBLIC_API)
    ...
    @pytest.mark.parametrize("status", set(testing.STATUSES_NOT_ALLOWING_EDIT_INSTITUTION) - STATUSES_NOT_IN_PUBLIC_API)
    ...
    def test_unallowed_action_archived(self, field, value):
        key, venue_provider = self.setup_active_venue_provider()
        offer = factories.ArchivedPublishedCollectiveOfferFactory(
...
class DomainsAndNationalProgramTest(PublicAPIVenueEndpointHelper):
    ...
    def test_update_domains_unknown(self):
        key, venue_provider = self.setup_active_venue_provider()
        current_domain = factories.EducationalDomainFactory()
    ...
    def test_update_program_inactive(self):
        key, venue_provider = self.setup_active_venue_provider()
        current_program = factories.NationalProgramFactory()
        new_program = factories.NationalProgramFactory(isActive=False)
        current_domain = factories.EducationalDomainFactory(nationalPrograms=[current_program, new_program])
        offer = factories.PublishedCollectiveOfferFactory(
            venue=venue_provider.venue,
            provider=venue_provider.provider,
            nationalProgram=current_program,
    ...
    def test_update_domains_invalid(self):
        ...
        current_program = factories.NationalProgramFactory()
        current_domain = factories.EducationalDomainFactory(nationalPrograms=[current_program])
        new_domain = factories.EducationalDomainFactory()
        offer = factories.PublishedCollectiveOfferFactory(
            venue=venue_provider.venue,
            provider=venue_provider.provider,
            nationalProgram=current_program,
