...
class GetBookingTicketTest:
    ...
    def test_get_booking_withdrawal_informations(self, client):
        ...
        ticket = response.json["ongoing_bookings"][0]["ticket"]
        assert ticket["withdrawal"]["details"] == "Veuillez chercher votre billet au guichet"
    ...
    def test_get_booking_ticket_by_email(self, client, withdrawal_delay, email_sent):
        ...
        assert ticket["noTicket"] == False
        assert ticket["email"] == {"sent": email_sent}
    ...
    def test_get_booking_digital_ticket(self, client):
        ...
        assert ticket["token"] == None
        assert ticket["voucher"] == None
        booking_id = response.json["ongoing_bookings"][0]["id"]
    ...
    @pytest.mark.parametrize(
        "subcategory,withdrawal_type,token_only",
        [
            (subcategories.CONCERT.id, offer_models.WithdrawalTypeEnum.ON_SITE, True),
            (
    ...
    def test_get_internal_event_ticket(self, client, subcategory, withdrawal_type, token_only):
        user = users_factories.BeneficiaryGrant18Factory(email=self.identifier)
        ...
        ticket = response.json["ongoing_bookings"][0]["ticket"]
        assert ticket["noTicket"] == False
        assert ticket["voucher"] == {
            "data": f"PASSCULTURE:v3;TOKEN:{booking.token}",
        }
        assert ticket["token"] == {
            "data": booking.token,
    ...
    def test_get_external_event_ticket_visible(self, client):
        ...
        assert ticket["externalBooking"]["isVisible"] == True
