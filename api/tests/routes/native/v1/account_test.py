...
class AccountTest:
    ...
    def test_get_user_profile(self, client, app):
        ...
        EXPECTED_DATA = {
            ...
            "bonificationStatus": subscription_models.BonificationStatus.NOT_ELIGIBLE.value,
    ...
    def test_get_user_profile_bonification_status_is_eligible(self, client):
        ...
        response = client.with_token(user.email).get("/native/v1/me")
        assert response.status_code == 200
        assert response.json["bonificationStatus"] == "eligible"
    ...
    @pytest.mark.parametrize(
        "reason_code,expected_bonification_status",
        [
            (
                subscription_models.FraudReasonCode.NOT_IN_TAX_HOUSEHOLD,
                subscription_models.BonificationStatus.NOT_IN_TAX_HOUSEHOLD,
            ),
            (
                subscription_models.FraudReasonCode.QUOTIENT_FAMILIAL_TOO_HIGH,
                subscription_models.BonificationStatus.QUOTIENT_FAMILIAL_TOO_HIGH,
            ),
            (
                subscription_models.FraudReasonCode.CUSTODIAN_NOT_FOUND,
                subscription_models.BonificationStatus.CUSTODIAN_NOT_FOUND,
            ),
        ],
    )
    def test_get_user_profile_bonification_status_ko(self, client, reason_code, expected_bonification_status):
        user = users_factories.BeneficiaryFactory(age=18)
        ...
        response = client.with_token(user.email).get("/native/v1/me")
        assert response.status_code == 200
        assert response.json["bonificationStatus"] == expected_bonification_status.value
    ...
    def test_get_user_profile_bonification_status_granted(self, client):
        ...
        response = client.with_token(user.email).get("/native/v1/me")
        assert response.status_code == 200
        assert response.json["bonificationStatus"] == subscription_models.BonificationStatus.GRANTED.value
    ...
    def test_get_user_profile_bonification_status_too_many_retries(self, client):
        ...
        response = client.with_token(user.email).get("/native/v1/me")
        assert response.status_code == 200
        assert response.json["bonificationStatus"] == subscription_models.BonificationStatus.TOO_MANY_RETRIES.value
    ...
    def test_get_user_profile_bonification_status_is_not_eligible_for_under_17(self, client):
        ...
        response = client.with_token(user.email).get("/native/v1/me")
        assert response.status_code == 200
        assert response.json["bonificationStatus"] == subscription_models.BonificationStatus.NOT_ELIGIBLE.value
    ...
    def test_get_user_profile_bonification_status__unkown_ko(self, client):
        ...
        response = client.with_token(user.email).get("/native/v1/me")
        assert response.status_code == 200
        assert response.json["bonificationStatus"] == subscription_models.BonificationStatus.KO.value
