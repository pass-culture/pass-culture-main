...
def test_when_there_is_a_debit_note_to_generate_on_total_incident(app, css_font_http_request_mock):
    sixteen_days_ago = datetime.datetime.now() - datetime.timedelta(days=16)
    ...
    incident_event = finance_api._create_finance_events_from_incident(booking_total_incident, datetime.datetime.now())

    finance_api.price_event(incident_event[0])

    second_batch = finance_api.generate_cashflows_and_payment_files(cutoff=datetime.datetime.now())
...
def test_when_there_is_a_debit_note_to_generate_on_partial_incident(app, css_font_http_request_mock):
    ...
    incident_events = finance_api._create_finance_events_from_incident(
        booking_partial_incident, datetime.datetime.now()
    )
...
def test_generate_invoice_file_with_debit_note(app, tmp_path, monkeypatch, css_font_http_request_mock):
    ...
    cashflow_batch1 = finance_api.generate_cashflows_and_payment_files(cutoff=datetime.datetime.now())
    run_command(
        app,
        "generate_invoices",
        "--batch-id",
    ...
    ####################################################################################################
    cashflow_batch2 = finance_api.generate_cashflows_and_payment_files(cutoff=datetime.datetime.now())
    run_command(
        app,
        "generate_invoices",
        "--batch-id",
