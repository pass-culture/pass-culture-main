...
class ExternalFinanceCommandTest:
    ...
    def test_get_settlements_command(self, run_command, mocker):
        get_settlements_mock = mocker.patch("pcapi.core.finance.external.sync_settlements")
        run_command("get_settlements", raise_on_error=True)

        assert get_settlements_mock.call_count == 1
