...
class CegidFinanceBackendTest:
    ...
    def test_push_invoice(self, cegid_config, requests_mock):
        ...
        #################################################################
        # Mock the Vendor, VendorLocation and FinancialPeriod API calls #
        #################################################################
        request_vendor_matcher = requests_mock.register_uri(
            "GET",
            f"{cegid_config.CEGID_URL}/entity/INTERFACES/23.200.001/Vendor/{bank_account.id}",
            json={"VendorLocation": {"LocationID": {"value": "MAIN"}}},
            status_code=200,
        ...
        request_financial_period_matcher = requests_mock.register_uri(
            "GET",
            f"{cegid_config.CEGID_URL}/entity/INTERFACES/23.200.001/FinancialPeriod?$expand=Details",
            json=[{"rowNumber": 1, "Details": [{"rowNumber": 1, "FinancialPeriodID": {"value": ""}}]}],
            status_code=200,
            headers={"content-type": "application/json"},
        )
        ...
        # Vendor/FinancialPeriod related calls
        assert request_financial_period_matcher.call_count == 1
        assert request_vendor_matcher.call_count == 1
        ...
        assert request_json["Description"] == {"value": "Justificatif de remboursement"}
        ...
        assert details1["InventoryID"] == {"value": "ORIND18P000"}
        ...
        assert details2["InventoryID"] == {"value": "CGIND18P000"}
        ...
        assert request_json["PostPeriod"] == {"value": ""}
        # assert request_json["ReferenceNbr"] == {"value": "000000000"} TODO Find a way to put the actual invoice.reference
        assert request_json["Status"] == {"value": "Open"}
        assert request_json["TaxTotal"] == {"value": "0"}
        assert request_json["Terms"] == {"value": "30J"}
        assert request_json["Type"] == {"value": "Bill"}
