...
class FraudCheckLifeCycleTest:
    ...
    def test_start_fraud_check(self, check_type):
        ...
        fraud_check = fraud_api.start_fraudcheck(user, str(uuid.uuid4()), source_data)
    ...
    def test_start_fraud_check_already_pending(self, check_type):
        ...
        with pytest.raises(fraud_exceptions.ApplicationValidationAlreadyStarted):
            fraud_api.start_fraudcheck(user, application_id, source_data)
    ...
    def test_start_fraud_check_with_other_check_finished(self, check_type, closed_state):
        ...
        source_data = factory_class()
        fraud_api.start_fraudcheck(user, str(uuid.uuid4()), source_data)
        assert (
            fraud_models.BeneficiaryFraudCheck.query.filter_by(
                user=user,
                type=check_type,
    ...
    def test_mark_fraudcheck_failed(self, check_type):
        user = users_factories.UserFactory()
        application_id = str(uuid.uuid4())
        ...
        fraud_api.mark_fraudcheck_failed(
            user,
            application_id,
    ...
    def test_mark_fraudcheck_failed_unexistant_previous_check(self, check_type):
        user = users_factories.UserFactory()
        application_id = str(uuid.uuid4())
