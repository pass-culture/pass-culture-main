...
class AddPricingPointTest:
    ...
    def test_should_not_add_pricing_point_if_offerer_has_more_than_one_siret(self):
    ...
    def test_should_not_add_pricing_point_if_offerer_has_more_than_one_bank_account(self):
        venue_with_siret = offerers_factories.VenueFactory(siret="12345678900010")
        offerers_factories.VenuePricingPointLinkFactory(venue=venue_with_siret, pricingPoint=venue_with_siret)
        offerers_factories.VenueBankAccountLinkFactory(venue=venue_with_siret)
    ...
    def test_add_pricing_point_if_offerer_has_inactive_ba(self):
        venue_with_siret = offerers_factories.VenueFactory(siret="12345678900010")
        pricing_point_link = offerers_factories.VenuePricingPointLinkFactory(
            venue=venue_with_siret, pricingPoint=venue_with_siret
        )
        ...
        finished_timespan = [datetime.utcnow() - timedelta(days=3), datetime.utcnow() - timedelta(days=1)]
        venue_without_siret = offerers_factories.VenueFactory(
            managingOfferer=venue_with_siret.managingOfferer, siret=None, comment="Venue address that has no siret"
        )
        offerers_factories.VenueBankAccountLinkFactory(venue=venue_without_siret, timespan=finished_timespan)
...
class AddBankAccountTest:
    ...
    def test_should_not_add_wrong_bank_account(self):
        venue_with_siret = offerers_factories.VenueFactory(siret="12345678900010")
        venue_without_siret = offerers_factories.VenueFactory(
        ...
        offerers_factories.VenueBankAccountLinkFactory(bankAccount=bank_account, venue=venue_without_siret)
    ...
    def test_should_add_bank_account_everywhere(self):
        venue_with_siret = offerers_factories.VenueFactory(siret="12345678900010")
        bank_account = finance_factories.BankAccountFactory(offerer=venue_with_siret.managingOfferer)
        bank_account_link = offerers_factories.VenueBankAccountLinkFactory(
            bankAccount=bank_account, venue=venue_with_siret
