...
from pcapi.core.offerers.synchronize_venues_banners_with_google_places import (
    synchronize_venues_banners_with_google_places,
)
...
class SynchronizeVenuesBannersWithGooglePlacesTest:
    ...
    @patch(
        ...
        side_effect=[googlemaps.exceptions.ApiError(status="NOT_FOUND"), PlaceDetails(name="Nom du lieu", photos=[])],
    )
    def test_update_place_id_on_not_found(self, _mock_get_place_photos_and_owner, _mock_get_place_id):
        ...
        synchronize_venues_banners_with_google_places([venue])
