...
from pcapi.core.offers.defs.models import activity
from pcapi.core.offers.defs.models import digital
from pcapi.core.offers.defs.models import unselectable
...
class FormatModelToSchemaTest:
    ...
    @pytest.mark.parametrize("model", [pytest.param(model, id=model.__name__) for model in DIGITAL])
    def test_format_model_computes_digital_typology_as_expected(self, model):
        schema = tools.format_model_to_schema(set(), model)
        assert {"digital"} <= set(schema.properties.typology)

    @pytest.mark.parametrize("model", [pytest.param(model, id=model.__name__) for model in ACTIVITY])
    def test_format_model_computes_event_typology_as_expected(self, model):
        schema = tools.format_model_to_schema(set(), model)
        assert {"activity"} <= set(schema.properties.typology)

    @pytest.mark.parametrize("model", [pytest.param(model, id=model.__name__) for model in CANNOT_BE_CREATED])
