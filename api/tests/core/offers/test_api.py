...
class UpdateOfferTest:
    ...
    def test_success_should_duplicate_ean(self):
        ...
        body = offers_serialize.UpdateOffer(ean="1234567890125")
        api.update_offer(offer, body)
    ...
    def test_success_should_not_duplicate_ean_when_it_is_an_empty_string(self):
        ...
        body = offers_serialize.UpdateOffer(ean=None)
        api.update_offer(offer, body)
...
def base_update_opening_hours_payload():
    return {
        "MONDAY": [],
        "TUESDAY": [],
        "WEDNESDAY": [],
...
class UpdateEventOpeningHoursTest:
    """This class test focuses on the opening hours update. The
    startDate and endDate updates are rather trivial and should be
    tested by the basic and mandatory controller tests.
    """

    def test_update_opening_hours_remove_all_days(self):
        """Test that the event does not have any opening hours anymore"""
        original = {
            models.Weekday.MONDAY: [("09:00", "18:00")],
            models.Weekday.TUESDAY: [("09:00", "18:00")],
            models.Weekday.WEDNESDAY: [("09:00", "18:00")],
            models.Weekday.THURSDAY: [("09:00", "18:00")],
            models.Weekday.FRIDAY: [("09:00", "18:00")],
        }
        new_opening_hours = {
            "MONDAY": [],
            "TUESDAY": [],
        ...
        body, event = self.build_body_and_event_with_opening_hours(original, new_opening_hours)

        api.update_event_opening_hours(event=event, update_body=body)
        db.session.refresh(event)

        assert not event.weekDayOpeningHours

    def test_update_opening_hours_only_add_one_day(self):
        """Test that only one weekday is added with its specific opening hours"""
        original = {models.Weekday.SATURDAY: [("09:00", "18:00")]}
        new_opening_hours = {
        ...
        assert len(event.weekDayOpeningHours) == 2

        saturday_opening_hours = self.get_opening_hours(event, "SATURDAY")
        assert len(saturday_opening_hours.timeSpans) == 1
        assert saturday_opening_hours.timeSpans[0].lower == 9 * 60
        assert saturday_opening_hours.timeSpans[0].upper == 18 * 60

        sunday_opening_hours = self.get_opening_hours(event, "SUNDAY")
        assert len(sunday_opening_hours.timeSpans) == 1
        assert sunday_opening_hours.timeSpans[0].lower == 10 * 60
        assert sunday_opening_hours.timeSpans[0].upper == 19 * 60

    def test_update_opening_hours_update_all_timespans_from_one_day(self):
        """Test that the whole weekday time spans are updated"""
        original = {models.Weekday.TUESDAY: [("10:30", "12:00"), ("13:00", "18:00")]}
        new_opening_hours = {"TUESDAY": [{"open": "09:30", "close": "12:00"}, {"open": "14:00", "close": "20:00"}]}
        body, event = self.build_body_and_event_with_opening_hours(original, new_opening_hours)

        api.update_event_opening_hours(event=event, update_body=body)
        db.session.refresh(event)

        assert len(event.weekDayOpeningHours) == 1

        opening_hours = event.weekDayOpeningHours[0]
        assert opening_hours.weekday == models.Weekday.TUESDAY

        assert len(opening_hours.timeSpans) == 2

        time_spans = sorted(opening_hours.timeSpans, key=lambda ts: ts.lower)
        assert time_spans[0].lower == (9 * 60) + 30
        assert time_spans[0].upper == 12 * 60
        assert time_spans[1].lower == 14 * 60
        assert time_spans[1].upper == 20 * 60

    def test_update_opening_hours_update_only_one_specific_timespan_bound(self):
        """Test that only one of the two time span is updated"""
        original = {models.Weekday.TUESDAY: [("11:45", "19:00")]}
        new_opening_hours = {"TUESDAY": [{"open": "09:00", "close": "19:00"}]}
        body, event = self.build_body_and_event_with_opening_hours(original, new_opening_hours)
        ...
        assert len(opening_hours.timeSpans) == 1

        time_spans = sorted(opening_hours.timeSpans, key=lambda ts: ts.lower)
        assert time_spans[0].lower == 9 * 60
        assert time_spans[0].upper == 19 * 60

    def test_update_opening_hours_remove_only_one_specific_timespan_bound(self):
        """Test that only one of the two time span is removed"""
        original = {models.Weekday.TUESDAY: [("11:45", "14:00"), ("19:00", "23:00")]}
        new_opening_hours = {"TUESDAY": [{"open": "11:45", "close": "14:00"}]}
        body, event = self.build_body_and_event_with_opening_hours(original, new_opening_hours)
        ...
        time_spans = sorted(opening_hours.timeSpans, key=lambda ts: ts.lower)
        assert time_spans[0].lower == (11 * 60) + 45
        assert time_spans[0].upper == 14 * 60

    def test_update_opening_hours_add_only_one_timespan(self):
        """Test that the one time span is added without updating the first one"""
        original = {models.Weekday.TUESDAY: [("11:45", "14:00")]}
        new_opening_hours = {"TUESDAY": [{"open": "11:45", "close": "14:00"}, {"open": "19:00", "close": "23:00"}]}
        body, event = self.build_body_and_event_with_opening_hours(original, new_opening_hours)
        ...
        assert time_spans[0].lower == (11 * 60) + 45
        assert time_spans[0].upper == 14 * 60
        assert time_spans[1].lower == 19 * 60
        assert time_spans[1].upper == 23 * 60

    def test_update_opening_hours_create_one_day_update_another_remove_remaining(self):
        """Test that update, remove and create actions can happen together"""
        original = {
            models.Weekday.MONDAY: [("10:00", "12:00"), ("14:00", "18:00")],
            models.Weekday.TUESDAY: [("07:00", "12:00"), ("15:00", "19:00")],
            models.Weekday.WEDNESDAY: [("07:00", "12:00"), ("16:00", "20:00")],
            models.Weekday.THURSDAY: [("07:00", "12:00"), ("16:00", "20:00")],
            models.Weekday.FRIDAY: [("07:00", "12:00"), ("16:00", "20:00")],
            models.Weekday.SATURDAY: [("09:00", "12:00")],
            models.Weekday.SUNDAY: [],
        }

        new_opening_hours = {
            # remove: closed on mondays
            "MONDAY": [],
            "TUESDAY": [{"open": "07:00", "close": "12:00"}, {"open": "14:00", "close": "18:00"}],
            "WEDNESDAY": [{"open": "07:00", "close": "12:00"}, {"open": "14:00", "close": "18:00"}],
            "THURSDAY": [{"open": "07:00", "close": "12:00"}, {"open": "14:00", "close": "18:00"}],
            "FRIDAY": [{"open": "07:00", "close": "12:00"}, {"open": "14:00", "close": "18:00"}],
            # update: new time spans
            "SATURDAY": [{"open": "07:00", "close": "12:00"}, {"open": "14:00", "close": "18:00"}],
        ...
        assert len(event.weekDayOpeningHours) == 6

        # monday is closed, sunday has different opening hours timespans
        identical_weekdays = new_opening_hours.keys() - {"MONDAY", "SUNDAY"}
        for weekday in identical_weekdays:
            opening_hours = self.get_opening_hours(event, weekday)

            timespans = sorted(opening_hours.timeSpans, key=lambda ts: ts.lower)
            assert len(timespans) == 2

            assert timespans[0].lower == 7 * 60
            assert timespans[0].upper == 12 * 60
            assert timespans[1].lower == 14 * 60
            assert timespans[1].upper == 18 * 60

        sunday_opening_hours = self.get_opening_hours(event, "SUNDAY")
        timespans = sorted(sunday_opening_hours.timeSpans, key=lambda ts: ts.lower)
        assert len(timespans) == 1
        assert timespans[0].lower == 9 * 60
        assert timespans[0].upper == 17 * 60

    def build_body_and_event_with_opening_hours(self, original_weekdays_and_time_spans, new_opening_hours):
        event = factories.EventOpeningHoursFactory(weekday_opening_hours=False)

        for weekday, time_spans in original_weekdays_and_time_spans.items():
            ts = timespan_str_to_numrange(time_spans)
            factories.EventWeekDayOpeningHoursFactory(eventOpeningHours=event, timeSpans=ts, weekday=weekday)

        body = offers_serialize.UpdateEventOpeningHoursModel(
            startDatetime=_base_start_datetime.astimezone(pytz.utc),
            endDatetime=_base_end_datetime.astimezone(pytz.utc),
            openingHours={**base_update_opening_hours_payload(), **new_opening_hours},
        )

        return body, event

    def get_opening_hours(self, event, target_weekday):
        return next(oh for oh in event.weekDayOpeningHours if oh.weekday == models.Weekday[target_weekday])
