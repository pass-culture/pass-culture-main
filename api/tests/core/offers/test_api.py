...
class HeadlineOfferTest:
    ...
    def test_do_not_deactivate_headline_offer_with_product_mediation(self, mocked_async_index_offer_ids, caplog):
        product_without_mediation = products_factories.ProductFactory()
        product_with_mediation = products_factories.ProductFactory()
        products_factories.ProductMediationFactory(product=product_with_mediation)
...
class AddCriterionToOffersTest:
    ...
    def test_add_criteria_from_ean(self, mocked_async_index_offer_ids):
        # Given
        ean1 = Fake.ean13()
        product1 = products_factories.ProductFactory(ean=ean1)
    ...
    def test_add_criteria_from_visa(self, mocked_async_index_offer_ids):
        # Given
        visa = "222100"
        product = products_factories.ProductFactory(extraData={"visa": visa})
...
class RejectInappropriateProductTest:
    ...
    def test_should_reject_product_with_inappropriate_content(
        ...
        ean_2 = Fake.ean13()
        provider = providers_factories.PublicApiProviderFactory()
        product1 = products_factories.ThingProductFactory(
            subcategoryId=subcategories.LIVRE_PAPIER.id, ean=ean_1, lastProvider=provider
        )
        product2 = products_factories.ThingProductFactory(
        ...
        # When
        products_api.reject_inappropriate_products([ean_1], user, send_booking_cancellation_emails=False)

        # Then
        offers = db.session.query(models.Offer).all()
        bookings = db.session.query(bookings_models.Booking).all()

        product1 = db.session.query(products_models.Product).filter(products_models.Product.ean == ean_1).one()
        assert product1.gcuCompatibilityType == products_models.GcuCompatibilityType.PROVIDER_INCOMPATIBLE

        product2 = db.session.query(products_models.Product).filter(products_models.Product.ean == ean_2).one()
    ...
    def test_should_reject_product_with_inappropriate_content_and_send_email(
        ...
        # When
        products_api.reject_inappropriate_products([ean_1], user)
    ...
    def test_update_should_not_override_fraud_incompatibility(
        ...
        # When
        products_api.reject_inappropriate_products([ean], user, send_booking_cancellation_emails=False)
