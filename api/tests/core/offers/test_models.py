...
class OfferIsSearchableTest:
    def test_offer_is_future(self):
        ...
        # active, published (no publication date set) and bookable (no
        # booking date set) and has stock -> eligible for search
        # -> eligible for search
        ...
        assert {res.id for res in results} == {offer_3.id, offer_4.id, offer_5.id}
