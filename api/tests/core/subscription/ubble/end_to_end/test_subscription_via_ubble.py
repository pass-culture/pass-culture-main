...
class UbbleV2EndToEndTest:
    ...
    def test_beneficiary_activation_with_ubble_mocked_response(self, client, app, requests_mock):
        ...
        self._receive_and_ignore_id_verification_pending_webhook_notification(user, app)
        self._receive_and_ignore_id_capture_in_progress_webhook_notification(user, app)
        self._receive_and_handle_id_verification_refused_webhook_notification(user, client, app, requests_mock)

        assert user.eligibility == users_models.EligibilityType.UNDERAGE

        self._retry_ubble_workflow(user, client, requests_mock)
        self._receive_and_ignore_id_capture_in_progress_webhook_notification(user, app)
        self._receive_and_handle_id_checks_in_progress_webhook_notification(user, client, app, requests_mock)
        self._receive_and_handle_id_verification_approved_webhook_notification(user, client, app, requests_mock)
    ...
    def _receive_and_ignore_id_verification_pending_webhook_notification(self, user, app) -> None:
        with patch(
    ...
    def _receive_and_ignore_id_capture_in_progress_webhook_notification(self, user, app) -> None:
        with patch(
            "pcapi.core.subscription.ubble.api.update_ubble_workflow",
        ) as mocked_update:
            response = TestClient(app.test_client()).post(
    ...
    def _receive_and_handle_id_verification_refused_webhook_notification(
        self, user, client, app, requests_mock
    ) -> None:
        requests_mock.get(
    ...
    def _receive_and_handle_id_checks_in_progress_webhook_notification(self, user, client, app, requests_mock) -> None:
        requests_mock.get(
            f"{settings.UBBLE_API_URL}/v2/identity-verifications/{fixtures.ID_VERIFICATION_CREATION_RESPONSE['id']}",
    ...
    def _receive_and_handle_id_verification_approved_webhook_notification(
        self, user, client, app, requests_mock
    ) -> None:
        requests_mock.get(
            f"{settings.UBBLE_API_URL}/v2/identity-verifications/{fixtures.ID_VERIFICATION_CREATION_RESPONSE['id']}",
        ...
        (deprecated_ubble_fraud_check, ubble_fraud_check) = sorted(
            [
                fraud_check
                for fraud_check in user.beneficiaryFraudChecks
                if fraud_check.type == fraud_models.FraudCheckType.UBBLE
            ],
            key=lambda check: check.thirdPartyId,
        )
        assert "deprecated" in deprecated_ubble_fraud_check.thirdPartyId, [
            c.thirdPartyId for c in user.beneficiaryFraudChecks
        ]
        assert ubble_fraud_check.status == fraud_models.FraudCheckStatus.OK
