...
class GetNewlyEligibleUsersTest:
    ...
    def test_eligible_user(self):
        ...
        users = repository.get_newly_eligible_users_by_age(since=date(2017, 12, 31), age=18)
        assert set(users) == {user_just_18_in_eligible_area, user_just_18_in_ineligible_area}
        users = repository.get_newly_eligible_users_by_age(since=date(2017, 12, 30), age=18)
