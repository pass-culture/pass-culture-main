...
class NotifyProUsersBeforeAnonymizationTest:
    # - feature flag: WIP_ENABLE_PRO_NOTIFICATION_BEFORE_ANONYMIZATION
    # - users and joined data
    expected_num_queries = 2

    last_connection_date = datetime.datetime.utcnow() - relativedelta(years=3, days=-30)

    @pytest.mark.features(WIP_ENABLE_PRO_NOTIFICATION_BEFORE_ANONYMIZATION=True, WIP_ENABLE_PRO_ANONYMIZATION=True)
    @pytest.mark.parametrize(
    ...
    @pytest.mark.features(WIP_ENABLE_PRO_NOTIFICATION_BEFORE_ANONYMIZATION=False, WIP_ENABLE_PRO_ANONYMIZATION=True)
    def test_feature_disabled(self):
        offerers_factories.DeletedUserOffererFactory(
            user__lastConnectionDate=self.last_connection_date,
        )

        with assert_num_queries(1):  # FF only
            users_api.notify_pro_users_before_anonymization()
...
class AnonymizeProUserTest:
    ...
    @pytest.mark.features(WIP_ENABLE_PRO_NOTIFICATION_BEFORE_ANONYMIZATION=True, WIP_ENABLE_PRO_ANONYMIZATION=False)
    def test_feature_disabled(self):
