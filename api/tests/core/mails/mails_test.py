...
import pydantic
import pytest
from brevo import BadRequestError as BrevoBadRequestError
...
BREVO_INVALID_EMAIL_ERROR = BrevoBadRequestError(
...
class SendinblueBackendTest:
    ...
    expected_sent_data = serialization.SendTransactionalEmailRequest(
    ...
    @patch("pcapi.core.mails.tasks.send_transactional_email_secondary_task.delay")
    ...
    @patch("brevo.contacts.client.ContactsClient.delete_contact", side_effect=BREVO_INVALID_EMAIL_ERROR)
    def test_delete_contact_bad_request_invalid_email(self, mock_delete_contact):
        contact_email = "invalid············@example.com"

        backend = self._get_backend_for_test()
        backend(use_pro_subaccount=False).delete_contact(contact_email)

        mock_delete_contact.assert_called_once()
    ...
    @patch("brevo.contacts.client.ContactsClient.get_contact_info", side_effect=BREVO_INVALID_EMAIL_ERROR)
    def test_get_contact_url_bad_request_invalid_email(self, mock_get_contact_info):
        contact_email = "invalid············@example.com"
    ...
    @patch("brevo.contacts.client.ContactsClient.get_contact_info", side_effect=BREVO_INVALID_EMAIL_ERROR)
    def test_get_raw_contact_data_bad_request_invalid_email(self, mock_get_contact_info):
        contact_email = "invalid············@example.com"
...
class ToDevSendinblueBackendTest(SendinblueBackendTest):
    expected_sent_data_to_dev = serialization.SendTransactionalEmailRequest(
...
class SendTest:
    ...
    @pytest.mark.settings(IS_TESTING=True, EMAIL_BACKEND="pcapi.core.mails.backends.sendinblue.ToDevSendinblueBackend")
    @patch("pcapi.core.mails.tasks.send_transactional_email_secondary_task.delay")
    def test_payload_json_serializable(self, mock_send_transactional_email_secondary_task, caplog):
        mock_template_send_ehp_false = models.Template(
        ...
        assert mock_send_transactional_email_secondary_task.call_count == 1
        assert mock_send_transactional_email_secondary_task.call_args.args[0]["params"]["CREDIT"] == decimal.Decimal(
            "139.90"
        )
