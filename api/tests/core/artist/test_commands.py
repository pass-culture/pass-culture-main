...
class UpdateArtistsFromDeltaTest:
    ...
    def test_updates_and_creates_artist_aliases(self, mock_alias_delta_query):
        ...
        cluster_id = "MULTI_Q1274944"
        alias_to_keep = ArtistAliasFactory(
            artist_id=artist.id, artist_alias_name=alias_name_to_delete, artist_cluster_id="CLUSTER"
        )
        alias_to_delete = ArtistAliasFactory(
            artist_id=artist.id, artist_alias_name=alias_name_to_delete, artist_cluster_id=cluster_id
        )
        ...
        mock_alias_delta_query.return_value = [
            DeltaArtistAliasModel(
                artist_id=artist.id, artist_alias_name=new_alias, artist_cluster_id=cluster_id, action=DeltaAction.ADD
            ),
            DeltaArtistAliasModel(
                artist_id=artist.id,
                artist_alias_name=alias_name_to_delete,
                artist_cluster_id=cluster_id,
                action=DeltaAction.REMOVE,
            ),
        ]
        ...
        assert (
            ...
            .filter_by(artist_id=artist.id, artist_alias_name=new_alias, artist_cluster_id=cluster_id)
            .first()
            is not None
        )
