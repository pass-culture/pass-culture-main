...
class ChurnedProEmailTest:
    ...
    def test_get_churned_pro_email_with_subaccount(self, mock_import_contacts, mock_run_query):
        mock_run_query.return_value = [
            {"venue_booking_email": "alice@example.com"},
            {"venue_booking_email": "beatrice@example.com"},
        ]
        ...
        mock_import_contacts.assert_called_once_with(
            RequestContactImport(
                file_url=None,
                file_body="EMAIL\nalice@example.com\nbeatrice@example.com",
                list_ids=[settings.SENDINBLUE_PRO_SUBACCOUNT_NO_ACTIVE_OFFERS_40_DAYS_ID],
                notify_url=f"{settings.API_URL}/webhooks/sendinblue/importcontacts/{settings.SENDINBLUE_PRO_SUBACCOUNT_NO_ACTIVE_OFFERS_40_DAYS_ID}/1",
                new_list=None,
                email_blacklist=False,
                sms_blacklist=False,
                update_existing_contacts=True,
    ...
    def test_get_no_bookings_pro_email(self, mock_import_contacts, mock_run_query):
        ...
        mock_import_contacts.assert_called_once_with(
            RequestContactImport(
                file_url=None,
                file_body="EMAIL\nalice@example.com\nbeatrice@example.com",
                list_ids=[settings.SENDINBLUE_PRO_SUBACCOUNT_NO_BOOKINGS_40_DAYS_ID],
                notify_url=f"{settings.API_URL}/webhooks/sendinblue/importcontacts/{settings.SENDINBLUE_PRO_SUBACCOUNT_NO_BOOKINGS_40_DAYS_ID}/1",
                new_list=None,
                email_blacklist=False,
                sms_blacklist=False,
                update_existing_contacts=True,
