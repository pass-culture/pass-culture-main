...
def test_synchronize_adage_ids_on_venues_with_timestamp_filter(mock_send_eac_email, db_session):
    ...
    yesterday = (datetime.now() - timedelta(days=1)).isoformat()  # pylint: disable=datetime-now
    a_month_ago = (datetime.now() - timedelta(days=31)).isoformat()  # pylint: disable=datetime-now
