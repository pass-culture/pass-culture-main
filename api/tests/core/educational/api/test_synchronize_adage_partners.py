...
def test_synchronize_partners():
    ...
    with (
        ...
        adage.synchronize_adage_partners(partners)
        db.session.commit()
    ...
    assert db.session.query(offerers_models.Venue).filter(offerers_models.Venue.adageId == str(adage_id_7)).count() == 0
...
def test_synchronize_partners_offerers():
    ...
    expected = [{"siren": venue_2.managingOfferer.siren}, {"siren": venue_5.managingOfferer.siren}]
    assert [call.kwargs for call in mock_get_adage_offerer.call_args_list] == expected
