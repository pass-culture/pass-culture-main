...
class NotifyProUserOneDayTest:
    ...
    def test_notify_pro_users_one_day(self, mock_mail_sender) -> None:
        ...
        booking1 = educational_factories.CollectiveBookingFactory(
            stock__offer__name="booking1",
            stock__offer__bookingEmails=["booking1@example.com", "booking1-2@example.com"],
            stock__beginningDatetime=datetime.datetime(2020, 1, 6),
            status=CollectiveBookingStatus.CONFIRMED,
        )
        # cancelled should not send email
        educational_factories.CollectiveBookingFactory(
            stock__offer__name="booking2",
            stock__offer__bookingEmails=["booking2+1@example.com", "booking2+2@example.com"],
            stock__beginningDatetime=datetime.datetime(2020, 1, 6),
        ...
        booking3 = educational_factories.CollectiveBookingFactory(
            stock__offer__name="booking3",
            stock__offer__bookingEmails=["booking3+2@example.com", "booking3+1@example.com"],
            stock__beginningDatetime=datetime.datetime(2020, 1, 6),
            status=CollectiveBookingStatus.CONFIRMED,
        )
        # cancelled should not send email (linked to a good one)
        educational_factories.CollectiveBookingFactory(
            stock__offer__name="booking4",
            stock__offer__bookingEmails=["booking4+1@example.com", "booking4+2@example.com"],
            stock__beginningDatetime=datetime.datetime(2020, 1, 6),
            stock=booking3.stock,
            status=CollectiveBookingStatus.CANCELLED,
        )
        # no emails register, should not send email
        educational_factories.CollectiveBookingFactory(
            stock__offer__name="booking5",
            stock__offer__bookingEmails=[],
            stock__beginningDatetime=datetime.datetime(2020, 1, 6),
            status=CollectiveBookingStatus.CONFIRMED,
        )
        # old booking should not be selected
        educational_factories.CollectiveBookingFactory(
            stock__offer__name="booking6",
            stock__offer__bookingEmails=["booking6+1@example.com", "booking6+2@example.com"],
            stock__beginningDatetime=datetime.datetime(2019, 1, 6),
            status=CollectiveBookingStatus.CONFIRMED,
        )
        # too far in the future to be selected
        educational_factories.CollectiveBookingFactory(
            stock__offer__name="booking7",
            stock__offer__bookingEmails=["booking7+1@example.com", "booking7+2@example.com"],
            stock__beginningDatetime=datetime.datetime(2021, 1, 6),
            status=CollectiveBookingStatus.CONFIRMED,
        )
        ...
        for args in mock_mail_sender.call_args_list:
            ...
            if data.params["OFFER_NAME"] == booking1.stock.offer.name:
                assert data.params == {
                    "OFFER_NAME": booking1.stock.offer.name,
                    "VENUE_NAME": booking1.stock.offer.venue.name,
                    "EVENT_HOUR": "1h",
                    "QUANTITY": 1,
                    "PRICE": str(booking1.stock.price),
                ...
                assert args.kwargs["recipients"] == [booking1.stock.offer.bookingEmails[0]]
                assert args.kwargs["bcc_recipients"] == booking1.stock.offer.bookingEmails[1:]
            elif data.params["OFFER_NAME"] == booking3.stock.offer.name:
                assert data.params == {
                    "OFFER_NAME": booking3.stock.offer.name,
                    "VENUE_NAME": booking3.stock.offer.venue.name,
                    "EVENT_HOUR": "1h",
                    "QUANTITY": 1,
                    "PRICE": str(booking3.stock.price),
                ...
                assert args.kwargs["recipients"] == [booking3.stock.offer.bookingEmails[0]]
                assert args.kwargs["bcc_recipients"] == booking3.stock.offer.bookingEmails[1:]
