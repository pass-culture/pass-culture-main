...
class NotifyUsersFutureOfferActivatedTest:
    ...
    def test_notify_users_future_offer_activate_exception_in_send(self, caplog):
        ...
        with patch("pcapi.tasks.batch_tasks.track_event_bulk_task.delay", side_effect=Exception):
            with caplog.at_level(logging.ERROR):
                notify_users_future_offer_activated(offer)

        assert len(push_testing.requests) == 0
        assert (
            caplog.records[0].message == f"Failed to register track_offer_added_to_reminders_event for offer {offer.id}"
        )
