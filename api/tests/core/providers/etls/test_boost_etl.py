...
class BoostExtractTransformLoadProcessTest:
    ...
    def test_load_should_update_product_offer_and_stocks(self, update_finance_event_pricing_date_mock, requests_mock):
        ...
        stock_1 = offers_factories.EventStockFactory(
            ...
            offer=offer_1,
            priceCategory__label="PCU",
            priceCategory__price=Decimal("5.0"),
            price=Decimal("5.0"),
            lastProvider=venue_provider.provider,
            idAtProviders=f"161%{venue_id}%Boost#15971",
        )
        # Force recalculating the expiration datetime because user's credit at booking creation is calculated depending
        # on database's datetime which is not affected by the time machine travelling hack.
        # For instance, the credit should expire at user's 21st birthday as he's supposed to be 18 years old when
        # generated by BeneficiaryFactory
        expiration_date = time_machine.escape_hatch.datetime.datetime.utcnow() + relativedelta(years=3)
        user = users_factories.BeneficiaryFactory(deposit__expirationDate=expiration_date)
        bookings_factories.BookingFactory(stock=stock_1, user=user)
