...
@pytest.mark.usefixtures("db_session")
class RequireBrevoSecretParamTest:
    def _make_app(self):
        app = flask.Flask(__name__)
    ...
    @pytest.mark.settings(BREVO_WEBHOOK_SECRET_QUERY_PARAM="valid")
    @pytest.mark.parametrize(
        "url,expected_response_status",
        [
            ("/test?token=valid", 200),
            ("/test?token=invalid", 401),
            ("/test", 401),
            ("/test?secret=valid", 401),
        ],
    )
    def test_require_token(self, url, expected_response_status):
        app = self._make_app()
        client = TestClient(app.test_client())
