...
from pcapi.scripts.transform_virtual_venues.main import FetchSiretError
...
class TransformVirtualVenueTest:
    ...
    @patch("pcapi.scripts.transform_virtual_venues.main._get_ban_address_or_none")
    def test_does_not_transform_virtual_venue_if_no_address_found(self, mock_get_ban_address):
        venue = offerers_factories.VirtualVenueFactory(managingOfferer__siren="012345678")

        mock_get_ban_address.return_value = None
    ...
    @patch("pcapi.connectors.entreprise.api.get_head_quarter")
    def test_does_not_transform_virtual_venue_if_failed_to_fetch_siret_info(self, mock_get_head_quarter):
        build_address()
        venue = offerers_factories.VirtualVenueFactory(managingOfferer__siren="012345678")

        mock_get_head_quarter.side_effect = RuntimeError("test")

        with pytest.raises(FetchSiretError):
            transform_virtual_venue(venue)
...
class TransformVenuesFromOfferersWithOneUniqueVirtualVenueTest:
    def test_transform_venues(self):
        ...
        transformed_venue_ids = transform_venues_from_offerers_with_one_unique_virtual_venue()
    ...
    def test_rollback_changes_for_one_venue_if_error_and_updated_others(self):
        ...
        original_func = entreprise_api.get_head_quarter

        with patch("pcapi.connectors.entreprise.api.get_head_quarter") as mock_transform_virtual_venue:
            ...
            mock_transform_virtual_venue.side_effect = [
                original_func(expected_venues[0].managingOfferer.siren),
                RuntimeError("test"),
                original_func(expected_venues[2].managingOfferer.siren),
            ]

            transformed_venue_ids = transform_venues_from_offerers_with_one_unique_virtual_venue()
            assert transformed_venue_ids == {expected_venues[0].id, expected_venues[2].id}

            assert_updated(expected_venues[0])
            assert_not_updated(expected_venues[1])
            assert_updated(expected_venues[2])
