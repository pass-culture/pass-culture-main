...
class TransformVirtualVenueTest:
    ...
    @patch("pcapi.scripts.transform_virtual_venues.main.get_backend")
    def test_does_not_transform_virtual_venue_if_failed_to_fetch_siret_info(self, mock_get_backend):
        build_address()
        venue = offerers_factories.VirtualVenueFactory(managingOfferer__siren="012345678")

        mock_get_backend.return_value.get_head_quarter.side_effect = RuntimeError("test")
