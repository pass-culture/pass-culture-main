...
from pcapi.scripts.change_virtual_to_physical.main import main
from pcapi.scripts.change_virtual_to_physical.main import transform_venue


pytestmark = pytest.mark.usefixtures("db_session")


class TransformVenueTest:
    ...
    def test_transform_virtual_venue_to_physical_venue(self, get_head_quarter_info):
        ...
        assert action.extraData["modified_info"] == {
            "activity": {"new_info": Activity.OTHER.name, "old_info": Activity.ARTISTIC_COMPANY.name},
...
class MainTest:
    @patch("pcapi.scripts.change_virtual_to_physical.main.transform_venue")
    @patch("pcapi.scripts.change_virtual_to_physical.main._read_venue_ids")
    def test_processes_venues_from_csv(self, read_venue_ids_mock, transform_venue_mock):
        venue1 = offerers_factories.VenueFactory(isVirtual=True, siret=None, comment="virtual")
        venue2 = offerers_factories.VenueFactory(isVirtual=True, siret=None, comment="virtual")
        offerers_factories.VenueFactory(isVirtual=True, siret=None, comment="virtual")

        author = users_factories.UserFactory()
        read_venue_ids_mock.return_value = [venue1.id, venue2.id]

        venues = main(not_dry=True, author=author)

        assert len(venues) == 2
        assert transform_venue_mock.call_count == 2
        transform_venue_mock.assert_any_call(venue1, author)
        transform_venue_mock.assert_any_call(venue2, author)
