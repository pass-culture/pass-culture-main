...
def test_chronicles_count() -> None:
    ...
    query = chronicles_count_query().order_by("product_id")
...
def test_headlines_count() -> None:
    ...
    query = headlines_count_query().order_by("product_id")
    result = db.session.execute(query).all()
...
def test_update_product_count() -> None:
    ...
    update_product_count(likes_count_query(), "likesCount", True)
