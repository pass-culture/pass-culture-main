...
def test_recredit_is_substracted() -> None:
    ...
    recredit_to_delete = RecreditFactory(
        deposit=user.deposit, recreditType=RecreditType.PREVIOUS_DEPOSIT, amount=Decimal("123.45")
    )
    user.deposit.amount += recredit_to_delete.amount

    with atomic():
        substract_from_deposit_because_of_past_expenses_on_another_account(user.id, author_id=author.id)
