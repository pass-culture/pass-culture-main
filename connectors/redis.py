...
ALGOLIA_INDEXING_BATCH_SIZE = int(os.environ.get('ALGOLIA_INDEXING_BATCH_SIZE', '1000'))
...
def get_offer_ids(client: Redis) -> List[int]:
    try:
        offer_ids = client.lrange(REDIS_LIST_OFFER_IDS_NAME, 0, ALGOLIA_INDEXING_BATCH_SIZE)
        return offer_ids
    except redis.exceptions.RedisError as error:
        logger.error(f'[REDIS] {error}')
...
def get_venue_ids(client: Redis) -> List[int]:
    try:
        venue_ids = client.lrange(REDIS_LIST_VENUE_IDS_NAME, 0, ALGOLIA_INDEXING_BATCH_SIZE)
        return venue_ids
    except redis.exceptions.RedisError as error:
        logger.error(f'[REDIS] {error}')
...
def delete_offer_ids(client: Redis) -> None:
    try:
        client.ltrim(REDIS_LIST_OFFER_IDS_NAME, ALGOLIA_INDEXING_BATCH_SIZE, -1)
...
def delete_venue_ids(client: Redis) -> None:
    try:
        client.ltrim(REDIS_LIST_VENUE_IDS_NAME, ALGOLIA_INDEXING_BATCH_SIZE, -1)
