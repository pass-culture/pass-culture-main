...
describe('src | components | pages | Terms', () => {
  it('should display title and legal disclaimer', () => {
    // when
    const wrapper = shallow(<Terms />)

    // then
    const title = wrapper.find({ children: 'Mentions légales' })
    const reactMarkdown = wrapper.find(ReactMarkdown)
    const sentence = wrapper.find({ children: 'pass Culture - ##LAST_DEPLOYED_COMMIT##' })
    expect(title).toHaveLength(1)
    expect(reactMarkdown.prop('source')).toBe(`
# Titre section
