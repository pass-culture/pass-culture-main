...
def get_offerers_with_offer_available_on_discovery_count(departement_code: str = None) -> int:
    ...
    query = Offerer.query.join(VenueSQLEntity).join(Offer).filter(Offer.id.in_(active_offers_ids))

    if departement_code:
        query = query.filter(VenueSQLEntity.departementCode == departement_code)

    return query.distinct(Offerer.id).count()
...
def get_offerers_with_offer_available_on_discovery_count_v2(departement_code: str = None) -> int:
    ...
    discovery_view_query = (
        keep_only_offers_in_venues_or_national(discovery_view_query, venues_ids)
        .with_entities(DiscoveryView.venueId)
        .subquery()
    )

    return (
        VenueSQLEntity.query.filter(VenueSQLEntity.id.in_(discovery_view_query))
...
def _get_all_physical_venue_ids() -> selectable.Alias:
    return (
        VenueSQLEntity.query.filter(VenueSQLEntity.departementCode.isnot(None))
...
def _get_physical_venue_ids_for_departement(departement_code: str) -> selectable.Alias:
    return VenueSQLEntity.query.filter_by(departementCode=departement_code).with_entities(VenueSQLEntity.id).subquery()
...
def get_offers_available_on_discovery_count(departement_code: str = None) -> int:
    ...
    if departement_code:
        query = query.join(VenueSQLEntity).filter(VenueSQLEntity.departementCode == departement_code)
...
def get_offers_with_non_cancelled_bookings_count(departement_code: str = None) -> int:
    query = Offer.query.join(StockSQLEntity).join(Booking)

    if departement_code:
        query = query.join(VenueSQLEntity).filter(VenueSQLEntity.departementCode == departement_code)
    return (
        query.filter(Booking.isCancelled.is_(False))
        .filter(Offer.type != str(ThingType.ACTIVATION))
        .filter(Offer.type != str(EventType.ACTIVATION))
        .distinct(Offer.id)
        .count()
...
def get_all_used_or_finished_bookings(departement_code: str) -> int:
    query = booking_repository._query_keep_only_used_and_non_cancelled_bookings_on_non_activation_offers().join(
        UserSQLEntity
    )
    if departement_code:
...
def count_all_cancelled_bookings(departement_code: str = None) -> int:
    return (
        booking_repository.count_cancelled_by_departement(departement_code)
        if departement_code
        else query_count_all_cancelled_bookings()
    )


def get_offer_counts_grouped_by_type_and_medium(
    query_get_counts_per_type_and_digital: Callable, counts_column_name: str
) -> pandas.DataFrame:
    ...
    counts_for_other_offers = pandas.DataFrame(
        {"Catégorie": ["Autres", "Autres"], "Support": ["Numérique", "Physique"], counts_column_name: [0, 0]}
    )
    ...
    for offer_counts in offer_counts_per_type_and_digital:
        ...
        if has_matching_row:
            offers_by_type_and_digital_table.loc[row_matching_type_and_medium, counts_column_name] = counts
    ...
    offers_by_type_and_digital_table = offers_by_type_and_digital_table.append(
        counts_for_other_offers, ignore_index=True
    )
    offers_by_type_and_digital_table.sort_values(
        by=[counts_column_name, "Catégorie", "Support"], ascending=[False, True, True], inplace=True
    )
...
def query_get_offer_counts_grouped_by_type_and_medium_for_departement(
...
def query_get_booking_counts_grouped_by_type_and_medium_for_departement(
    departement_code: str,
) -> List[Tuple[str, bool, int]]:
    return db.engine.execute(
        text(
