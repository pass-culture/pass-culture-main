...
def send_payments_details(payments: List[Payment], recipients: List[str]) -> None:
    ...
    if all(map(lambda x: x.currentStatus.status == TransactionStatus.ERROR, payments)):
        ...
        try:
            ...
            logger.exception("[BATCH][PAYMENTS] Error while sending payment details email to MailJet", exception)
...
def send_wallet_balances(recipients: List[str]) -> None:
    ...
    try:
        ...
        logger.exception("[BATCH][PAYMENTS] Error while sending users wallet balances email to MailJet", exception)
...
def send_payments_report(payments: List[Payment], recipients: List[str]) -> None:
    ...
    try:
        ...
        logger.exception("[BATCH][PAYMENTS] Error while sending payments reports to MailJet", exception)
