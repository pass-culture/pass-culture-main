...
def upgrade():
    op.alter_column(
        "deposit", "source", existing_type=sa.VARCHAR(length=12), type_=sa.String(length=300), existing_nullable=False
    )
...
def downgrade():
    ...
    op.alter_column(
        "deposit", "source", existing_type=sa.VARCHAR(length=300), type_=sa.String(length=12), existing_nullable=False
    )
