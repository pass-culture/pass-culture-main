...
def upgrade():
    ...
    op.execute("COMMIT")
    op.add_column(
        "venue_provider", sa.Column("fieldsUpdated", sa.ARRAY(sa.String(100)), nullable=False, server_default="{}")
    )
    op.execute("COMMIT")
    op.add_column(
    ...
    op.execute("COMMIT")
    op.add_column(
        "mediation", sa.Column("fieldsUpdated", sa.ARRAY(sa.String(100)), nullable=False, server_default="{}")
    )
    op.execute("COMMIT")
