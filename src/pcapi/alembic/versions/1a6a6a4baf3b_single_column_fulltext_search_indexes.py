...
def upgrade():
    ...
    op.execute(
        """CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_venue_fts_address" ON venue USING gin (to_tsvector('french_unaccent'::regconfig, address::text));"""
    )
    op.execute(
        """CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_venue_fts_name" ON venue USING gin (to_tsvector('french_unaccent'::regconfig, name::text));"""
    )
    op.execute(
        """CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_venue_fts_publicName" ON venue USING gin (to_tsvector('french_unaccent'::regconfig, "publicName"::text));"""
    )
    op.execute(
        """CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_venue_fts_siret" ON venue USING gin (to_tsvector('french_unaccent'::regconfig, siret::text));"""
    )
    op.execute(
        """CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_offerer_fts_address" ON offerer USING gin (to_tsvector('french_unaccent'::regconfig, address::text));"""
    )
    op.execute(
        """CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_offerer_fts_name" ON offerer USING gin (to_tsvector('french_unaccent'::regconfig, name::text));"""
    )
    op.execute(
        """CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_offerer_fts_siret" ON offerer USING gin (to_tsvector('french_unaccent'::regconfig, siren::text));"""
    )
...
def downgrade():
    op.execute("COMMIT")  # Close the automatically opened transaction so we can create/drop indexes "concurrently"
    op.execute(
        """CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_event_fts" ON product USING gin (to_tsvector('french_unaccent'::regconfig, (COALESCE(name, ''::character varying)::text || ' '::text) || COALESCE(description, ''::text)));"""
    )
    op.execute(
        """CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_offer_fts" ON offer USING gin (to_tsvector('french_unaccent'::regconfig, (((((COALESCE(name, ''::character varying)::text || ' '::text) || COALESCE(("extraData" -> 'author'::text)::text, ''::text)) || ' '::text) || COALESCE(("extraData" -> 'byArtist'::text)::text, ''::text)) || ' '::text) || COALESCE(description, ''::text)));"""
    )
    op.execute(
        """CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_offerer_fts" ON offerer USING gin (to_tsvector('french_unaccent'::regconfig, (((COALESCE(name, ''::character varying)::text || ' '::text) || COALESCE(address, ''::character varying)::text) || ' '::text) || COALESCE(siren, ''::character varying)::text));"""
    )
    op.execute(
        """CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_venue_fts" ON venue USING gin (to_tsvector('french_unaccent'::regconfig, (((COALESCE(name, ''::character varying)::text || ' '::text) || COALESCE(address, ''::character varying)::text) || ' '::text) || COALESCE(siret, ''::character varying)::text));"""
    )
