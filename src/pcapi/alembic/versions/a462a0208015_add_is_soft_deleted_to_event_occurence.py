...
def upgrade():
    op.add_column(
        "event_occurrence", sa.Column("isSoftDeleted", sa.BOOLEAN, nullable=False, server_default=expression.false())
    )
...
def downgrade():
    ...
    op.add_column(
        "event_occurrence", sa.Column("isActive", sa.BOOLEAN, nullable=False, server_default=expression.true())
    )
