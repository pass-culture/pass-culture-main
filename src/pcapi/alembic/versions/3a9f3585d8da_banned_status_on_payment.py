...
def upgrade():
    ...
    op.execute(
        "ALTER TABLE payment_status ALTER COLUMN status TYPE tmp_transactionstatus"
        " USING status::text::tmp_transactionstatus"
    )
    previous_enum.drop(op.get_bind(), checkfirst=False)
    new_enum.create(op.get_bind(), checkfirst=False)
    op.execute(
        "ALTER TABLE payment_status ALTER COLUMN status TYPE transactionstatus" " USING status::text::transactionstatus"
    )
