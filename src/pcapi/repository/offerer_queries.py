...
def find_all_offerers_with_managing_user_information():
    query = (
        db.session.query(
            ...
            UserSQLEntity.lastName,
            UserSQLEntity.email,
            UserSQLEntity.phoneNumber,
...
def find_all_offerers_with_managing_user_information_and_not_virtual_venue():
    ...
    result = (
        query.filter(VenueSQLEntity.isVirtual == False)
        .order_by(Offerer.name, VenueSQLEntity.name, UserSQLEntity.email)
        .all()
    )
    return result
...
def find_all_pending_validation():
    ...
    result = (
        Offerer.query.join(UserOfferer)
        .join(VenueSQLEntity)
        .join(UserSQLEntity)
        .filter(
            or_(
                user_offerer_pending_validation,
                offerer_pending_validation,
                venue_pending_validation,
...
def find_filtered_offerers(
    ...
    if (
        has_not_virtual_venue is not None
        or has_validated_venue is not None
        or offer_status is not None
        or has_venue_with_siret is not None
    ):
