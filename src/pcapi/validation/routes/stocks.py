...
def _forbid_dates_on_stock_for_thing_offer(request_data: dict) -> None:
    if "beginningDatetime" in request_data:
        raise ApiErrors({"global": ["Impossible de mettre une date de début si l'offre ne porte pas sur un événement"]})
...
def get_only_fields_with_value_to_be_updated(existing_stock_data: Dict, new_stock_data: Dict) -> List:
    ...
    fields_with_changed_data = [
        key for key in stock_related_keys if filtered_new_stock_data[key] != filtered_existing_stock_data[key]
    ]
