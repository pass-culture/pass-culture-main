...
def create_industrial_recommendations(offers_by_name, users_by_name):
    ...
    for (user_name, user) in users_by_name.items():
        ...
        user_has_recommendation_on_something_else_than_activation_offers = any(
            [user_tag in user_name for user_tag in ["has-confirmed-activation", "has-booked-some", "has-no-more-money"]]
        )
        ...
        active_offer_ids = [
            offer.id
            for offer in get_offers_for_recommendation(user=user, departement_codes=departement_codes, limit=None)
        ]
        ...
        recommended_offer_ids = remove_every(active_offer_ids, ACTIVE_OFFERS_WITH_RECOMMENDATION_PER_USER_REMOVE_MODULO)
        ...
        for (offer_name, offer) in list(offers_by_name.items()):
            ...
            recommendations_by_name[recommendation_name] = create_recommendation(
        ...
        if recommendation.offer:
            ...
            if not offer.mediations:
                recommendations_with_asset[recommendation.id] = store_public_object_from_sandbox_assets(
                    "thumbs", offer.product, offer.type
                )
