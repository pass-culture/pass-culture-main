...
export async function putData(
  ...
  if (diff) {
    ...
    if (IS_DEXIE) {
      await db.differences(diffElement)
    } else {
      db.differences.data.push(diffElement)
    }
  }
  // get again
...
export async function pushPull(state = {}) {
  ...
  if (state.data) {
    console.log('ON A DE LA DATA !!!', state.data)
  }
  ...
  return Promise.all(
    config.collections.map(async ({
      ...
      // push
      if (isSync) {
        let differences = db.differences
          .filter(difference => difference.name === name)
        if (IS_DEXIE) {
          differences = await differences.toArray()
        } else {
        ...
        if (IS_DEXIE) {
          await db.differences.filter(difference => difference.name === name)
                              .delete()
        } else {
