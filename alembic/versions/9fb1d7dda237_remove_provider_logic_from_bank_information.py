...
def upgrade():
    op.drop_constraint(
        'check_providable_with_provider_has_idatproviders', 'bank_information')
    ...
    op.alter_column('bank_information', 'dateModifiedAtLastProvider',
                    new_column_name='dateModified')


def downgrade():
    op.add_column('bank_information', sa.Column(
        'idAtProviders', sa.String(70), unique=True))
    op.add_column('bank_information', sa.Column('fieldsUpdated', sa.ARRAY(
        sa.String(100)), nullable=False, server_default="{}"))
    op.add_column('bank_information', sa.Column('lastProviderId',
                                                sa.BigInteger, ForeignKey('provider.id'), nullable=True))
    ...
    op.alter_column('bank_information', 'dateModified',
