...
CREATE FUNCTION public.audit_table(target_table regclass, ignored_cols text[])
 RETURNS void
 LANGUAGE plpgsql
AS $function$
DECLARE
...
CREATE FUNCTION public.audit_table(target_table regclass)
 RETURNS void
 LANGUAGE sql
AS $function$
...
FROM pg_class
WHERE oid = 'bank_information'::regclass;

SELECT public.audit_table(oid)
FROM pg_class
WHERE oid = 'booking'::regclass;

SELECT public.audit_table(oid)
FROM pg_class
WHERE oid = 'event'::regclass;

SELECT public.audit_table(oid)
FROM pg_class
WHERE oid = 'mediation'::regclass;

SELECT public.audit_table(oid)
FROM pg_class
WHERE oid = 'offer'::regclass;

SELECT public.audit_table(oid)
FROM pg_class
WHERE oid = 'offerer'::regclass;

SELECT public.audit_table(oid)
FROM pg_class
WHERE oid = 'stock'::regclass;

SELECT public.audit_table(oid)
FROM pg_class
WHERE oid = 'venue'::regclass;

SELECT public.audit_table(oid)
FROM pg_class
WHERE oid = 'venue_provider'::regclass;
