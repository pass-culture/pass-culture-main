...
class MailjetNotificationService(NotificationService):
    def send_booking_cancellation_emails_to_user_and_offerer(self, booking: BookingWithOfferer,
                                                             is_offerer_cancellation: bool, is_user_cancellation: bool,
                                                             send_email: Callable[..., bool]):
        booking_with_offerer_sql_entity = booking_with_offerer.to_model(booking)
        try:
            send_booking_cancellation_emails_to_user_and_offerer(booking_with_offerer_sql_entity, is_offerer_cancellation,
