...
def restize_value_error(e):
    if len(e.args)>1 and e.args[1]=='enum':
        return json.dumps([{'field':e.args[2], 'errtype':'format', 'error': 'should be one of '+",".join(map(lambda x : '"'+x+'"', e.args[3]))}]), 400
    else:
...
def restize_type_error(e):
    if e.args and len(e.args)>1 and e.args[1]=='geography':
        return json.dumps([{'field':e.args[2], 'errtype':'format', 'error': 'should be a list of floating point numbers like this : [2.22, 3.22]'}]), 400
    elif e.args and len(e.args)>1 and e.args[1] and e.args[1]=='decimal':
        return json.dumps([{'field':e.args[2], 'errtype':'format', 'error': 'should be a decimal number'}]), 400
    elif e.args and len(e.args)>1 and e.args[1] and e.args[1]=='integer':
        return json.dumps([{'field':e.args[2], 'errtype':'format', 'error': 'should be an integer'}]), 400
    else:
        return something_went_wrong(e)
...
def restize_integrity_error(e):
    if hasattr(e, 'orig') and hasattr(e.orig, 'pgcode') and e.orig.pgcode=='23505':
        ...
        return json.dumps([{'field': field, 'errtype':'duplicate', 'error': 'already exists in our database'}]), 400
    elif hasattr(e, 'orig') and hasattr(e.orig, 'pgcode') and e.orig.pgcode=='23503':
        field = re.search('Key \((.*?)\)=', str(e._message), re.IGNORECASE).group(1)
        return json.dumps([{'field': field, 'errtype':'integrity', 'error': 'there is no object with this id in our database'}]), 400
        ...
        return json.dumps([{'field': field, 'errtype':'missing', 'error': 'Champ obligatoire'}]), 400
    else:
        return something_went_wrong(e)
